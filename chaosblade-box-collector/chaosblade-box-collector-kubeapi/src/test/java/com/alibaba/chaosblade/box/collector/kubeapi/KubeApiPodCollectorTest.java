...
@ContextConfiguration(classes = KubeApiPodCollectorTest.Config.class)
public class KubeApiPodCollectorTest {

    @Configuration
    @ComponentScan("com.alibaba.chaosblade.box.collector.kubeapi")
    public static class Config {
    }

    @Autowired
    private KubeApiPodCollector podCollector;
    ...
    public void testAttack() throws Exception {
        CompletableFuture<List<Pod>> collect = podCollector.collect(Query.builder()
                .nodeName("node-master")
                .build());
        List<Pod> pods = collect.get();
        Assert.assertEquals(pods.size(), 1);
        Assert.assertEquals("tomcat-xyz", pods.get(0).getName());
    }
