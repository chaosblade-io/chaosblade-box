...
public class ApplicationDeviceRepository implements IRepository<String, ApplicationDeviceDO> {
  ...
  @Autowired private ApplicationDeviceMapper applicationDeviceMapper;
  ...
  public Optional<ApplicationDeviceDO> findByAppIdAndAppConfigurationId(
      Long appId, String configurationId) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
  ...
  public List<ApplicationDeviceDO> findByConfigurationIdIn(
      Long appId, Set<String> appConfigurationId, Integer deviceType) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("app_id", appId);
  ...
  public List<ApplicationDeviceDO> findDeviceByLastHealthPingTimeLt(
      Long heartBeatExpireTimeInMilles) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
   ...
   * @return
   */
  public IPage<ApplicationDeviceDO> findLiveDeviceByAppIdAndGroupAndKey(
      ApplicationDO chaosApplicationDO, List<String> groups, String key, int page, int size) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
    ...
    if (AppType.HOST.getType() == chaosApplicationDO.getAppType()) {
      queryWrapper.in(
          "dimension",
          ApplicationDimension.HOST.getValue(),
          ApplicationDimension.NODE.getValue(),
  ...
  public ApplicationDeviceDO findAnyOneByAppId(String appId) {
    ...
    return applicationDeviceMapper.selectOne(
        new QueryWrapper<>(applicationDeviceDO).last("limit 1"));
  }
   ...
   * @return
   */
  public IPage<ApplicationDeviceDO> findLiveDeviceByAppIdAndGroupAndKeyAndTags(
    ...
    // 性能考虑将方法区分开
    if (AppType.HOST.getType() == chaosApplicationDO.getAppType()) {
      return applicationDeviceMapper.selectPageByTagsForHost(
          MyBatisUtil.getPage(page, size), query);
    } else {
   ...
   * @return
   */
  public IPage<ApplicationDeviceDO> searchLiveDevice(
      ApplicationDeviceQuery deviceQuery, int page, int size) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
    ...
    if (!Strings.isNullOrEmpty(deviceQuery.getPartName())) {
      queryWrapper.nested(
          nestedWrapper ->
              nestedWrapper
                  .or(wrapper -> wrapper.like("private_ip", deviceQuery.getPartName()))
  ...
  public List<ApplicationDeviceDO> findLiveDeviceByAppNameAndGroupAndIp(
      String appName, List<String> groups, List<String> ips) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
  ...
  public List<ApplicationDeviceDO> findByAppInfoAndConfigurationIds(
      String appId, Set<String> groupName, List<String> configurationIds) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("app_id", appId);
  ...
  public IPage<ApplicationDeviceDO> searchDeviceByAppIdOrAppNamePart(
      ApplicationDeviceQuery deviceQuery, int page, int size) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
  ...
  public List<ApplicationDeviceDO> findByClusterId(
      String user_id, String clusterId, boolean online) {
    QueryWrapper<ApplicationDeviceDO> queryWrapper = new QueryWrapper<>();
