...
/** @author haibin */
@Component
public class ConfigFileMiniAppInvokeErrorMessageDescProvider
    implements MiniAppInvokeErrorMessageDescProvider, InitializingBean {
  ...
  @Override
  public ChaosbladeErrorMessageDesc getDescByErrorMessage(
      MiniAppInvokeContext miniAppInvokeContext, Response response) {
    Optional<ChaosbladeErrorMessageDesc> chaosbladeErrorMessageDesc =
        chaosbladeErrorMessageDescs.stream()
            .filter(
                chaosbladeErrorMessageDesc1 ->
                    response.getError().contains(chaosbladeErrorMessageDesc1.getPartError()))
            .findFirst();
  ...
  public void init() {
    try {
      if (getAgentCodePath() != null) {
        Type type = new TypeReference<Map<String, CodeMetaData>>() {}.getType();
    ...
    try {
      if (getErrorMessagePath() != null) {
        Type type = new TypeReference<List<ChaosbladeErrorMessageDesc>>() {}.getType();
        chaosbladeErrorMessageDescs = ObjectUtil.parseResourceObject(getErrorMessagePath(), type);
        logger.info(
            "load chaos-blade error-message file success,content size:" + codeToMetaData.size());
      }
    } catch (Throwable throwable) {
      logger.error("load chaos-blade error message failed", throwable);
    }
