...
public class ExperimentRepository
    implements IRepository<String, ExperimentDO>, IQuery<ExperimentDO, ExperimentQuery> {

  @Resource private ExperimentMapper experimentMapper;

  @Override
  public Optional<ExperimentDO> findById(String experimentId) {
    QueryWrapper<ExperimentDO> queryWrapper =
        new QueryWrapper<ExperimentDO>().eq("experiment_id", experimentId).eq("is_delete", false);
  ...
  public IPage<ExperimentDO> findByPage(
      ExperimentPageQuery experimentPageQuery, int pageNo, int pageSize) {
  ...
  private QueryWrapper<ExperimentDO> buildExperimentQueryWrapper(ExperimentQuery query) {
    ...
    queryWrapper
        ...
        .in(
            ...
            "result_state",
            !CollectionUtil.isNullOrEmpty(query.getResults())
                ? query.getResults().stream().map(ResultEnum::getValue).collect(Collectors.toList())
                : Lists.newArrayList())
        .in(
            null != query.getExperimentIds() && !query.getExperimentIds().isEmpty(),
            "experiment_id",
  ...
  public List<ExperimentDO> findBetweenByExperimentIdsIn(
  ...
  public int countByExpertiseId(String expertiseId) {
    QueryWrapper<ExperimentDO> queryWrapper =
        new QueryWrapper<ExperimentDO>().eq("is_template", false).eq("outer_id", expertiseId);
    return experimentMapper.selectCount(queryWrapper);
  }

  public List<ExperimentSummaryInfo> getExperimentSummaryInfoIn30Days(
      ChaosUser user, ExperimentQueryRequest request) {
    return experimentMapper.getExperimentSummaryInfoIn30Days(
        user.getUserId(), request.getNamespace());
  }

  public List<ExperimentSummaryInfo> getExperimentSummaryInfo(
      ChaosUser user, ExperimentQueryRequest request) {
    return experimentMapper.getExperimentSummaryInfo(user.getUserId(), request.getNamespace());
  }
  ...
  public List<ExperimentDO> findByUserId(String userId) {
    QueryWrapper<ExperimentDO> queryWrapper = new QueryWrapper<ExperimentDO>();
    queryWrapper.eq("user_id", userId).eq("is_delete", false);
    return experimentMapper.selectList(queryWrapper);
  }
}
