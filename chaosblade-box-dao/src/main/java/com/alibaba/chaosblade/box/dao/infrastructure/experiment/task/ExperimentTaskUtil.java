...
public class ExperimentTaskUtil {
  ...
  public long calculateProgressPercent(List<ActivityTaskDO> activityTaskDOS) {
    if (activityTaskDOS == null || activityTaskDOS.isEmpty()) {
      return 0;
    }
    int activitySize = activityTaskDOS.size();
    Set<String> finishedOrRunningActivityIds =
        activityTaskDOS.stream()
            .map(
                activityTaskDO -> {
                  if (activityTaskDO.isReady()) {
                    return null;
                  }
  ...
  public void fillBaseInfo(
      BaseExperimentTask baseExperimentTask,
    ...
    if (experimentTaskDO.getExperimentTaskContext() != null) {
      experimentTaskExtInfo.setSchedulerConfig(
          experimentTaskDO.getExperimentTaskContext().getSchedulerConfig());
    ...
    if (!experimentTaskDO.isFinished()) {
      activityTaskRepository
          .findById(experimentTaskDO.getActivityTaskId())
          .ifPresent(
  ...
  public ExperimentTaskStat doStatistic(List<ExperimentTaskDO> experimentTaskDOS) {
    ...
    for (ExperimentTaskDO experimentTaskDO : experimentTaskDOS) {
      ...
      if (experimentTaskDO.getGmtEnd() != null) {
        countWithEndTime++;
        totalCost +=
            (experimentTaskDO.getGmtEnd().getTime() - experimentTaskDO.getGmtCreate().getTime());
      }
    }
    ...
    if (countWithEndTime != 0) {
      experimentTaskStat.setAverageCostInSecond(
          TimeUnit.MILLISECONDS.toSeconds(totalCost / countWithEndTime));
    }
