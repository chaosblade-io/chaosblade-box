...
public class UserApplicationRepository {
  ...
  public ApplicationDeviceDO registerOrUpdateDevice(ApplicationDeviceDO requestDeviceDO) {
    ...
    // 删除老的记录
    deleteDevice(
        requestDeviceDO.getUserId(),
        ApplicationDimension.HOST,
        requestDeviceDO.getConfigurationId());
    // 处理应用信息
    ApplicationDO chaosApplicationRequestDO =
        buildApplicationDOFromDeviceDO(deviceType, requestDeviceDO);
    ApplicationDO chaosApplicationDO = registerApplication(chaosApplicationRequestDO);
    // 处理分组信息
    registerGroup(buildGroupFromDeviceDO(chaosApplicationDO, requestDeviceDO));
    // 处理设备信息
    return registerDevice(
        requestDeviceDO, deviceType, chaosApplicationRequestDO, chaosApplicationDO);
  }

  private ApplicationDeviceDO registerDevice(
      ApplicationDeviceDO requestDeviceDO,
      DeviceType deviceType,
      ApplicationDO chaosApplicationRequestDO,
      ApplicationDO chaosApplicationDO) {
    ApplicationDeviceDO chaosApplicationDeviceDO =
        findDeviceByDeviceType(deviceType, requestDeviceDO);
   ...
   * @return
   */
  private ApplicationDeviceDO findDeviceByDeviceType(
      DeviceType deviceType, ApplicationDeviceDO requestDeviceDO) {
  ...
  private ApplicationGroupDO buildGroupFromDeviceDO(
      ApplicationDO chaosApplicationDO, ApplicationDeviceDO chaosApplicationDeviceDO) {
   ...
   * @return
   */
  private ApplicationDO buildApplicationDOFromDeviceDO(
      DeviceType deviceType, ApplicationDeviceDO requestChaosApplicationDeviceDO) {
  ...
  private void registerGroup(ApplicationGroupDO requestApplicationGroupDO) {
    ...
    ApplicationGroupDO chaosApplicationGroupDO =
        applicationGroupMapper.selectOne(new QueryWrapper<>(query));
  ...
  public boolean deleteDevice(
      String userId, ApplicationDimension dimension, String configurationId) {
    ...
    return applicationDeviceMapper.update(updateObject, new QueryWrapper<>(applicationDeviceDO))
        > 0;
  }
}
