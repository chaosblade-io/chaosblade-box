...
public class ChaosBladeActivityAsyncCallback implements ActivityAsyncCallback {
  ...
  @Autowired private AsyncCallBackHostInterceptor asyncCallBackHostInterceptor;
  ...
  public void execute(AsyncCallBackContext asyncCallBackContext) {
    ...
    if (Strings.isNullOrEmpty(chaosBladeExpUidDO.getExpUid())) {
      RecordsRepo.getMiniAppInvocationRecorder()
          .log(
              asyncCallBackContext.getExperimentMiniAppTaskDO(),
              "Current expId missed,try later",
              true);
      return;
    }
  ...
  private void handleInKubernetesMode(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO,
    ...
    if (isK8sScene(experimentMiniAppTaskDO.getAppCode())) {
      if ("Running".equals(asyncCallBackContext.getStatus())) {
        ...
        K8sResultBean k8sResultBean =
            new K8sResultBean(chaosBladeExpUidDO.getExpUid(), true, null, new ArrayList<>());
        pushTasks(experimentMiniAppTaskDO, k8sResultBean);
      } else {
        Response<K8sResultBean> k8sResultBeanResponse = null;
        try {
          k8sResultBeanResponse =
              chaosBladeInvoker.queryK8sExp(
                  host,
                  chaosBladeExpUidDO.getExpUid(),
                  PhaseType.ATTACK.equals(experimentMiniAppTaskDO.getPhase()));
  ...
  private void pushTasks(ExperimentMiniAppTaskDO experimentMiniAppTaskDO, Object resultBean) {
    ...
    if (experimentMiniAppTaskDO.isFinished()) {
      boolean success =
          activityTargetExecutionResultRepository.updateNotFinishedTask(experimentMiniAppTaskDO);
      if (success) {
  ...
  private void handleInjectionFinishedInKubernetesMode(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO, K8sResultBean k8sResultBean) {
  ...
  private void handleInjectionTimeoutInKubernetesMode(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO, K8sResultBean k8sResultBean) {
    if (!isCompleted(experimentMiniAppTaskDO, k8sResultBean)) {
  ...
  private void handleInHostMode(
    ...
    if (!isK8sScene(experimentMiniAppTaskDO.getAppCode())) {
      ...
      try {
        expStatus =
            chaosBladeInvoker.getExpStatus(
  ...
  private void handleInjectionTimeOutInHostMode(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO, ExpStatusBean expStatusBean) {
  ...
  private void handleInjectionFinishedInHostMode(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO, ExpStatusBean expStatusBean) {
    if (ChaosBladeStatus.Revoked.name().equals(expStatusBean.getStatus())) {
      markRunFailed(
          experimentMiniAppTaskDO,
          CommonErrorCode.B_CHAOS_BLADE_ASYNC_REVOKED.name(),
    ...
    if (expStatusBean.isRunning()) {
      log.info(
          "[ChaosBladeActivityAsyncCallback] expStatusBean.isRunning() : {}",
  ...
  private boolean runTimeOut(ExperimentMiniAppTaskDO experimentMiniAppTaskDO) {
    boolean isTimeout =
        System.currentTimeMillis() - experimentMiniAppTaskDO.getStartTime().getTime()
            > MAX_INSTALL_TIME_MINUTES * 60 * 1000;
    if (isTimeout) {
      RecordsRepo.getMiniAppInvocationRecorder()
          .log("Execute timeout after:" + MAX_INSTALL_TIME_MINUTES + " minutes");
    }
    return isTimeout;
  }
