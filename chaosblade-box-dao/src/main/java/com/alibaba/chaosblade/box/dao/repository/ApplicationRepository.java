...
public class ApplicationRepository implements IRepository<Long, ApplicationDO> {
  ...
  public ApplicationDO findByAppIdOrAppName(
      Long appId, String appName, String userIdForAppName, String namespaceForAppName) {
    ...
    if (!Strings.isNullOrEmpty(appName)) {
      applicationDO =
          findByUserIdAndAppNameAndUserNamespace(userIdForAppName, appName, namespaceForAppName);
    }
  ...
  public ApplicationDO findByUserIdAndAppNameAndUserNamespace(
      String userId, String appName, String namespace) {
    if (Strings.isNullOrEmpty(appName)
  ...
  public List<ApplicationDO> searchApplicationByNamespace(
      String userId, String namespace, boolean filterDisabled) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("user_id", userId);
    queryWrapper.eq("namespace", namespace);
    if (filterDisabled) {
  ...
  public List<ApplicationDO> fuzzySearchApplication(
      String userId, String namespace, String key, boolean filterDisabled) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("user_id", userId);
  ...
  public IPage<ApplicationDO> pageableFuzzySearchApplication(
      String userId,
      String namespace,
      String key,
      int page,
    ...
    queryWrapper.in(
        !CollectionUtils.isEmpty(appTypes),
        "app_type",
        appTypes.stream().map(AppType::getType).collect(Collectors.toList()));
    return applicationMapper.selectPage(MyBatisUtil.getPage(page, pageSize), queryWrapper);
  }

  public List<ApplicationDO> fuzzySearchApplication(
      String userId, String namespace, String key, boolean filterDisabled, List<Long> appIds) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.like("app_name", key);
    queryWrapper.eq("namespace", namespace);
    if (filterDisabled) {
  ...
  public IPage<ApplicationDO> findApplicationsByUserId(
      String userId, String namespace, boolean filterDisabled, int page, int size) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("user_id", userId);
    queryWrapper.eq("namespace", namespace);
    if (filterDisabled) {
  ...
  public IPage<ApplicationDO> findApplicationsByUserIdAndAuthApp(
      String userId,
      String namespace,
  ...
  public List<AppNameAndIdPair> findApplicationByUserIdReturnNames(
      String userId, String namespace) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("user_id", userId);
    queryWrapper.eq("namespace", namespace);
    return applicationMapper.selectList(queryWrapper).stream()
        .map(
            new Function<ApplicationDO, AppNameAndIdPair>() {
              @Override
  ...
  public List<AppNameAndIdPair> findApplicationByUserIdAndScopeTypeReturnNames(
      String userId, String namespace, String appType, String key, boolean filterDisabled) {
    QueryWrapper<ApplicationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("user_id", userId);
    queryWrapper.eq("namespace", namespace);
  ...
  public int deleteApplicationByUserId(String userId, boolean logical) {
    ...
    if (logical) {
      ...
      return applicationMapper.update(
          applicationDO, new QueryWrapper<ApplicationDO>().eq("user_id", userId));
    } else {
  ...
  public IPage<ApplicationDO> pageUserApplications(
      String userId,
      String namespace,
      String appType,
      String key,
