...
public class ExperimentFlowDefinitionMeta {
  ...
  public static class ExperimentActivitiesMap {
    ...
    public void addExperimentActivityDO(
        PhaseType phaseType, ExperimentActivityDO experimentActivityDO) {
      if (experimentActivityDO.getActivityId() != null) {
        if (activities.contains(experimentActivityDO.getActivityId())) {
          return;
        }
        activities.add(experimentActivityDO.getActivityId());
      }
      phaseTypeListMap
          .computeIfAbsent(phaseType, phaseType1 -> new ArrayList<>())
          .add(experimentActivityDO);
      if (experimentActivityDO.getFlowId() != null) {
        flowIdToExperimentActivityDO
            .computeIfAbsent(experimentActivityDO.getFlowId(), s -> new ArrayList<>())
            .add(experimentActivityDO);
      }
    }
    ...
    public List<ExperimentActivityDO> getExperimentActivityDOs() {
      return phaseTypeListMap.values().stream()
          .flatMap(
              (Function<List<ExperimentActivityDO>, Stream<ExperimentActivityDO>>)
                  Collection::stream)
          .collect(Collectors.toList());
    }

    public List<ExperimentActivityDO> getExperimentActivityDOSByPhase(PhaseType phaseType) {
      return phaseTypeListMap.computeIfAbsent(
          phaseType,
          new Function<PhaseType, List<ExperimentActivityDO>>() {
            @Override
