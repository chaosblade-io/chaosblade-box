...
public class ChaosBladeExpUidRepository implements IRepository<String, ChaosBladeExpUidDO> {
  ...
  @Autowired private ChaosBladeExpUidMapper chaosBladeExpUidMapper;
  ...
  public ChaosBladeExpUidDO createChaosBladeExpRecord(
      String expId,
    ...
    String configurationId =
        Strings.isNullOrEmpty(experimentMiniAppTaskDO.getAppConfigurationId())
    ...
    chaosBladeExpUidDO.addAttribute(
        ChaosBladeExpUidDO.ATTRIBUTE_DEVICE_CONFIGURATION_ID,
        experimentMiniAppTaskDO.getDeviceConfigurationId());
  ...
  public ChaosBladeExpUidDO findLastByExperimentTaskIdAndHostAndAppCodeAndNotExpired(
      String experimentTaskId, String host, String appCode) {
    QueryWrapper<ChaosBladeExpUidDO> chaosBladeExpUidDOQueryWrapper = new QueryWrapper<>();
    chaosBladeExpUidDOQueryWrapper
  ...
  public ChaosBladeExpUidDO findLastByActivityTaskIdAndHost(String activityTaskId, String host) {
    QueryWrapper<ChaosBladeExpUidDO> chaosBladeExpUidDOQueryWrapper = new QueryWrapper<>();
    chaosBladeExpUidDOQueryWrapper.eq(ACTIVITY_TASK_ID, activityTaskId).eq(HOST, host);
    wrapperLast(chaosBladeExpUidDOQueryWrapper);
    return chaosBladeExpUidMapper.selectOne(chaosBladeExpUidDOQueryWrapper);
  }
  ...
  public ChaosBladeExpUidDO findByExpUidAndExperimentTaskId(
      String expUid, String experimentTaskId) {
    QueryWrapper<ChaosBladeExpUidDO> chaosBladeExpUidDOQueryWrapper = new QueryWrapper<>();
    chaosBladeExpUidDOQueryWrapper.eq(EXP_UID, expUid);
  ...
  public List<ChaosBladeExpUidDO> findWithNotExpireAndExperimentTaskFinished() {
    ...
    chaosBladeExpUidDOQueryWrapper
        ...
        .inSql(
            EXPERIMENT_TASK_ID,
            "SELECT task_id " + "  from t_chaos_experiment_task " + " where run_state= 4");
    return chaosBladeExpUidMapper.selectList(chaosBladeExpUidDOQueryWrapper);
  }

  public ChaosBladeExpUidDO findLastByExpUidOrSubExpId(String expUid) {
    QueryWrapper<ChaosBladeExpUidDO> chaosBladeExpUidDOQueryWrapper = new QueryWrapper<>();
    chaosBladeExpUidDOQueryWrapper.and(
        wrapper -> wrapper.eq(EXP_UID, expUid).or().eq("sub_exp_uid", expUid));
    wrapperLast(chaosBladeExpUidDOQueryWrapper);
    return chaosBladeExpUidMapper.selectOne(chaosBladeExpUidDOQueryWrapper);
  }
}
