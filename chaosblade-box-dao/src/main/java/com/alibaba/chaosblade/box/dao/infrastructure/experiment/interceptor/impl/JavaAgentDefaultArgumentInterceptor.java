...
/** @author sunju */
@Component
public class JavaAgentDefaultArgumentInterceptor
  ...
  @Resource private SceneDaoFunctionService sceneDaoFunctionService;

  private Map<String, String> setDefaultValueIfEmpty(
      Integer type,
  ...
  public void preInterceptor(
    ...
    if (null != originDefinition && phaseType == PhaseType.PREPARE) {
      if (JAVA_AGENT_INSTALL_APP_CODE.equals(originDefinition.getAppCode())) {
        SceneFunctionDO sceneFunction =
            sceneDaoFunctionService.querySceneFunctionByCode(JAVA_AGENT_INSTALL_APP_CODE);
        Map<String, List<SceneFunctionParameterDO>> grouping =
            sceneFunction.getParameters().stream()
                .collect(
                    Collectors.groupingBy(SceneFunctionParameterDO::getAlias, Collectors.toList()));
        ExperimentNodeArgumentsDefinition args = originDefinition.getArguments();
        args.setAction(
            setDefaultValueIfEmpty(
                SceneFunctionParameterDO.TYPE_ACTION, args.getAction(), grouping));
        args.setMatcher(
            setDefaultValueIfEmpty(
                SceneFunctionParameterDO.TYPE_MATCHER, args.getMatcher(), grouping));
        args.setUserArgs(
            setDefaultValueIfEmpty(
                SceneFunctionParameterDO.TYPE_USER, args.getUserArgs(), grouping));
      }
    }
  }
}
