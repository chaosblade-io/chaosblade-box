...
public class MiniAppTaskManagerImpl implements MiniAppTaskManager {
  ...
  @Override
  public ExperimentMiniAppTaskDO findOrCreateMiniAppTaskIfNotExist(
      ActivityTaskDO activityTaskDO, Host host) {
    ...
    ExperimentMiniAppTaskDO experimentMiniAppTaskDO =
        activityTargetExecutionResultRepository.findOne(activityTargetExecutionResultQuery);
  ...
  private ExperimentMiniAppTaskDO createMiniAppTask(ActivityTaskDO activityTaskDO, Host host) {
    ExperimentMiniAppTaskDO experimentMiniAppTaskDO =
        initExperimentMiniAppTaskDO(activityTaskDO, host);
  ...
  public long rejectMiniAppTasksByActivityTaskId(String activityTaskId, String rejectReason) {
    return activityTargetExecutionResultRepository.findByActivityTaskId(activityTaskId).stream()
        .filter(miniAppExecutionRecordDO -> !miniAppExecutionRecordDO.isFinished())
        .peek(miniAppExecutionRecordDO -> rejectMiniAppTask(miniAppExecutionRecordDO, rejectReason))
        .count();
  }
  ...
  public List<MiniAppTaskSummary> findMiniAppTasksByActivityTaskId(String activityTaskId) {
    return activityTargetExecutionResultRepository.findByActivityTaskIdReturnSummary(activityTaskId)
        .stream()
        .map(this::convertMiniAppTaskSummary)
        .collect(Collectors.toList());
  }

  @Override
  public MiniAppTask findMiniAppTask(String miniAppTaskId) {
    return activityTargetExecutionResultRepository
        .findById(miniAppTaskId)
        .map(this::convert)
        .orElse(null);
  }
  ...
  private MiniAppTaskSummary convertMiniAppTaskSummary(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO) {
    ...
    Date startTime =
        experimentMiniAppTaskDO.getStartTime() == null
            ? experimentMiniAppTaskDO.getGmtCreate()
  ...
  private MiniAppTask convert(ExperimentMiniAppTaskDO experimentMiniAppTaskDO) {
    ...
    ChaosBladeResultCodeRepo.getByErrorCode(experimentMiniAppTaskDO.getErrorCode())
        .ifPresent(codeMetaData -> miniAppTask.setSolution(codeMetaData.getSolution()));
    ChaosBladeResultCodeRepo.getByCode(experimentMiniAppTaskDO.getAgentCode())
        .ifPresent(
            codeMetaData ->
                miniAppTask.setSolution(
                    ChaosBladeSolutionUtil.parseSolution(
                        codeMetaData.getSolution(), experimentMiniAppTaskDO.getErrorMessage())));
    ChaosRequestContextHolder.getLoginUser()
        .ifPresent(
            chaosUser -> {
              miniAppTask
                  .getExtraInfo()
                  .put(ORIGIN_REQUEST, experimentMiniAppTaskDO.getOriginRequest());
              miniAppTask
                  .getExtraInfo()
                  .put(ORIGIN_RESPONSE, experimentMiniAppTaskDO.getOriginResponse());
            });
    return miniAppTask;
  }
}
