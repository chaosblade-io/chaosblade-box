...
public class JvmHitCountMetricProvider implements MetricProvider {
  ...
  @Autowired private MetricManager metricManager;

  @Autowired private ChaosBladeInvoker chaosBladeInvoker;

  @Override
  public List<ChaosMetricEntity> provide(
    ...
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        (ChaosBladeExpUidDO)
            metricRequest.getAttribute(CommonConstant.METRIC_CHAOSBLADE_EXP_OBJECT);
    if (chaosBladeExpUidDO == null) {
      log.warn("chaosblade expObject is null,expId:" + expId);
      return new ArrayList<>();
    }
    Response<Long> response =
        chaosBladeInvoker.getHitCountInJvm(
            new ChaosBladeExpQueryRequest(
                host, expId, MiniAppUtils.isK8S(chaosBladeExpUidDO.getAppCode())));
    if (response.isSuccess()) {
      ...
      chaosMetricDO.setValue(
          response.getResult() == null ? 0f : Float.parseFloat(response.getResult() + ""));
    ...
    return metricManager
        .findMetricsByConfigurationIdAndExpId(
            host.getDeviceConfigurationId(),
            metricRequest.getFrom(),
            metricRequest.getTo(),
            metricDefinition.getKey(),
            expId)
        .stream()
        .map(
            chaosMetricDO -> {
  ...
  @Override
  public Map<Host, List<ChaosMetricEntity>> batchProvider(
      List<Host> hosts, MetricDefinition metricDefinition, MetricRequest metricRequest) {
    return null;
  }
}
