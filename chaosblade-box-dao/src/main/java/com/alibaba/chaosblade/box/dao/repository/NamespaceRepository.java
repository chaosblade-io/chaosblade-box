...
public class NamespaceRepository {
  ...
  @Resource private NamespaceMapper namespaceMapper;

  public Optional<List<NamespaceDO>> getUserNamespace(String userId) {
    if (!Strings.isNullOrEmpty(userId)) {
      QueryWrapper<NamespaceDO> queryWrapper =
          new QueryWrapper<NamespaceDO>().eq("user_id", userId).eq("is_delete", NOT_DELETE_FLAG);
  ...
  public boolean addNamespace(NamespaceDO namespace) {
    ...
    checkArgument(
        !Strings.isNullOrEmpty(namespace.getName()), "Namespace's name can not be null or empty");
    //        checkArgument(!Strings.isNullOrEmpty(namespace.getNamespace()), "Namespace's name can
    // not be null or empty");
    checkArgument(
        !Strings.isNullOrEmpty(namespace.getUserId()), "User id can not be null or empty");

    namespace.setIsDelete(NOT_DELETE_FLAG);
    namespace.setDescription(namespace.getDescription());

    List<NamespaceDO> namespaces =
        namespaceMapper.selectList(
            new QueryWrapper<NamespaceDO>()
    ...
    if (CollectionUtil.isNullOrEmpty(namespaces)) {
      ...
      namespaces.stream()
          .filter(namespaceDO -> DELETED_FLAG.equals(namespaceDO.getIsDelete()))
          .forEach(
              namespaceDO -> {
