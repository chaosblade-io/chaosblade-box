...
public class WorkspaceRepository implements IRepository<String, WorkspaceDO> {
  ...
  public List<WorkspaceDO> find(WorkspaceQuery query) {
    QueryWrapper<WorkspaceDO> queryWrapper =
        new QueryWrapper<WorkspaceDO>()
            .in(
            ...
            .in(
                null != query.getUserIds() && !query.getUserIds().isEmpty(),
                "user_id",
                query.getUserIds())
  ...
  public IPage<WorkspaceDO> findPageable(int page, int size) {
    return workspaceMapper.selectPage(
        MyBatisUtil.getPage(page, size), new QueryWrapper<WorkspaceDO>().eq("is_delete", false));
  }
  ...
  public Optional<WorkspaceDO> findById(String workspaceId) {
    QueryWrapper<WorkspaceDO> queryWrapper =
        new QueryWrapper<WorkspaceDO>().eq("workspace_id", workspaceId).eq("is_delete", false);
    return Optional.ofNullable(workspaceMapper.selectOne(queryWrapper));
  }

  @Override
  public boolean update(WorkspaceDO workspace) {
    UpdateWrapper<WorkspaceDO> updateWrapper =
        new UpdateWrapper<WorkspaceDO>().eq("workspace_id", workspace.getWorkspaceId());
