...
public class ActivityTaskExecutionCommand extends BasePoolCommand<ActivityTaskExecutionResponse>
  ...
  @Getter private final ExperimentExecuteContext experimentExecuteContext;

  @Autowired private ActivityRepository activityRepository;

  @Autowired private ExperimentRepository experimentRepository;

  @Autowired private ExperimentTaskContextManager experimentTaskContextManager;
  ...
  @Autowired private List<ActivityExecutionInterceptor> activityExecutionInterceptors;
  ...
  public ActivityTaskExecutionCommand(ExperimentExecuteContext experimentExecuteContext) {
  ...
  public ActivityTaskExecutionResponse execute() {
    ActivityTaskExecutionResponse activityTaskExecutionResponse =
        new ActivityTaskExecutionResponse();
    try {
      // 演练上下文需要提前准备
  ...
  private void onReturn(ActivityTaskExecutionResponse activityTaskExecutionResponse) {
    for (ActivityExecutionInterceptor activityExecutionInterceptor :
        activityExecutionInterceptors) {
      activityExecutionInterceptor.onReturn(
          currentActivityTaskDO,
  ...
  private void onError(Throwable throwable) {
    for (ActivityExecutionInterceptor activityExecutionInterceptor :
        activityExecutionInterceptors) {
      activityExecutionInterceptor.onError(
          currentActivityTaskDO, throwable, this.experimentExecuteContext.getContextData());
    }
  }
  ...
  private void beforeRun() {
    for (ActivityExecutionInterceptor activityExecutionInterceptor :
        activityExecutionInterceptors) {
      activityExecutionInterceptor.onStarted(
          this.currentActivityTaskDO, this.experimentExecuteContext.getContextData());
    }
  }

  private boolean forbidRun() {
    return activityExecutionInterceptors.stream()
        .anyMatch(
            activityExecutionInterceptor ->
                activityExecutionInterceptor.forbid(
                    currentActivityTaskDO, experimentExecuteContext.getContextData()));
  }
  ...
  private ActivityExecuteResult runActivity() {
    ...
    try {
      Activity activity =
          new Activity(
              experimentExecuteContext
                  .getContextData()
                  .getAsString(
                      ActivityTaskExecutionAttributes.ATTRIBUTE_KEY_EXPERIMENT_ACTIVITY_ID),
              getActivityHosts(),
              experimentExecuteContext
                  .getContextData()
                  .getObject(
                      ActivityTaskExecutionAttributes.ATTRIBUTE_KEY_ACTIVITY_TASK_DO,
                      ActivityTaskDO.class),
              experimentExecuteContext
                  .getContextData()
                  .getObject(
                      ActivityTaskExecutionAttributes.ATTRIBUTE_KEY_EXPERIMENT_ACTIVITY_DEFINITION,
      ...
      if (!(throwable instanceof ActivityInvokeInterruptException)) {
        logger.error(
            "run activity error,taskId:{}", this.currentActivityTaskDO.getTaskId(), throwable);
      }
                  ...
                  experimentMiniAppTaskDOList.stream()
                      .anyMatch(
                          experimentMiniAppTaskDO ->
                              Objects.equals(experimentMiniAppTaskDO.getHostIp(), host.getIp())))
  ...
  private void fillActivityInfo(
    ...
    ExperimentActivityDO experimentActivityDO =
        activityRepository.findById(activityTaskDO.getActivityId()).get();
   ...
   * @param runParam
   */
  private void overrideActivityDefinition(
      ExperimentActivityDefinition experimentActivityDefinition, ActivityRunParam runParam) {
  ...
  private void processEntryActivityTask(ExperimentTaskRunnableSettings settings) {
    ...
    try {
      settings.setMiniAppContext(
          experimentTaskContextManager
              .getExperimentTaskContext(experimentTaskDO.getTaskId())
              .getMiniAppContext());
    } catch (Exception ex) {
   ...
   * @param experimentTaskRunnableSettings
   */
  private void setActivityRunnableStrategy(
      ExperimentTaskRunnableSettings experimentTaskRunnableSettings) {
    experimentTaskRunnableSettings.enableMiniAppBatchRunning();
  }

  private void setExperimentTaskDO(
      ExperimentTaskRunnableSettings experimentTaskRunnableSettings,
      ActivityTaskDO activityTaskDO) {
  ...
  private void fillUserId(
    ...
    if (experimentTaskRunnableSettings.getUserId() == null) {
      experimentTaskRunnableSettings.setUser(new ChaosUser(experimentTaskDO.getUserId()));
    }
  }
