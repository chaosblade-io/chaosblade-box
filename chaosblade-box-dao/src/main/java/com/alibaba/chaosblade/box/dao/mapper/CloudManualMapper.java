...
public interface CloudManualMapper {
  ...
  @Select(
      "SELECT app_code ,COUNT(id) as total from t_chaos_activity_task where phase =\"ATTACK\" GROUP BY app_code  "
  ...
  @Select(
      "select DATE_FORMAT(gmt_create, '%Y-%m-%d') as date,\n"
          + "       count(DISTINCT(experiment_task_id)) as total \n"
          + "  from t_chaos_app_execute_result\n"
          ...
          + "group by date")
  List<Map<String, Object>> countInvocationByHostAndDate(
      @Param("configurationId") String configurationId, @Param("mountDate") Date mountDate);
  ...
  @Select(
      "SELECT app_code,\n"
          ...
          + "GROUP BY app_code")
  List<Map<String, Object>> countAppCodeByUserId(
      @Param("userId") String userId, @Param("mountDate") Date mountDate);
  ...
  @Select(
      "select DATE_FORMAT(gmt_create, '%Y-%m-%d') as date,\n"
          ...
          + "group by date")
  List<Map<String, Object>> countExperimentTaskByDate(
      @Param("userId") String userId, @Param("mountDate") Date mountDate);
}
