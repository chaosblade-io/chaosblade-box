...
public abstract class BaseChaosMethodDescriptor {
    ...
    return pairs.stream()
        .flatMap(
            pair -> {
              ...
              Optional<Annotation> argsOptional =
              ...
              if (argsOptional.isPresent()) {
                ...
                if (Primitives.allPrimitiveTypes().contains(parameterType)
                    ...
                    || ClassUtils.isAssignable(parameterType, List.class)) {
                  return Stream.of(
                      ChaosFunctionArgumentDescriptor.of(
                          chaosArgs, ChaosAppArgumentType.USER_ARGS));
                } else {
                  if (chaosArgs.converters().length > 0) {
                    ...
                    if (null != type) {
                      ...
                      List<ChaosFunctionArgumentDescriptor> chaosFunctionArgumentDescriptorStream =
                          Arrays.stream(getAllDeclaredFields(type, parameterType))
                              .filter(filed -> filed.isAnnotationPresent(ChaosArgs.class))
                              .map(
                                  filed -> {
                                    ChaosArgs args = filed.getAnnotation(ChaosArgs.class);
                                    return ChaosFunctionArgumentDescriptor.of(
                                        filed, args, argType0);
                                  })
                              .collect(Collectors.toList());
                      if (chaosFunctionArgumentDescriptorStream.isEmpty()) {
                        log.warn(
                            "argument descriptor parser result is 0 for parameter:{},type:{}",
                            chaosArgs.alias(),
  ...
  static List<ChaosFunctionDependencyDescriptor> getFunctionDependencyDescriptors(
      Method method, ChaosFunction function) {
    ChaosDependency[] chaosDependencies = function.dependencies();
    return Arrays.stream(chaosDependencies)
        .map(ChaosFunctionDependencyDescriptor::of)
        .collect(Collectors.toList());
  }
}
