...
public class SceneFunctionRepository implements IRepository<Long, SceneFunctionDO> {
  ...
  @Resource private SceneFunctionMapper sceneFunctionMapper;
  ...
  public List<SceneFunctionDO> findBySceneId(String sceneId) {
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().eq("scene_id", sceneId).eq("is_delete", false);
  ...
  public Optional<SceneFunctionDO> findByFunctionId(String functionId) {
    ...
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().eq("function_id", functionId).eq("is_delete", false);
  ...
  public List<SceneFunctionDO> findByFunctionIds(List<String> functionIds) {
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>()
  ...
  public List<SceneFunctionDO> findByFunctionCodes(List<String> codes) {
    ...
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().in("code", codes).eq("is_delete", false);
  ...
  public List<SceneFunctionDO> findByFunctionCodeRightLike(String likeWord) {
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().likeRight("code", likeWord).eq("is_delete", false);
    return sceneFunctionMapper.selectList(queryWrapper);
  }

  public Optional<SceneFunctionDO> findByCode(String code) {
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().eq("code", code).eq("is_delete", false);
  ...
  public boolean updateByFunctionId(SceneFunctionDO sceneFunction) {
    UpdateWrapper<SceneFunctionDO> updateWrapper =
        new UpdateWrapper<SceneFunctionDO>()
            ...
            .set(
                !Strings.isNullOrEmpty(sceneFunction.getDescription()),
                "description",
                sceneFunction.getDescription())
  ...
  public boolean deleteByFunctionId(String functionId) {
    ...
    UpdateWrapper<SceneFunctionDO> updateWrapper =
        new UpdateWrapper<SceneFunctionDO>().eq("function_id", functionId);
  ...
  public List<SceneFunctionDO> findInFunctionIds(List<String> functionIds) {
    ...
    QueryWrapper<SceneFunctionDO> queryWrapper =
        new QueryWrapper<SceneFunctionDO>().in("function_id", functionIds).eq("is_delete", false);
    return sceneFunctionMapper.selectList(queryWrapper);
  }
  ...
  public boolean deleteInFunctionIds(List<String> functionIds) {
    ...
    for (SceneFunctionDO exist : exists) {
      exist.setIsDelete(true);
      UpdateWrapper<SceneFunctionDO> updateWrapper =
          new UpdateWrapper<SceneFunctionDO>().eq("function_id", exist.getFunctionId());
  ...
  public List<SceneFunctionDO> findAvailableFunctions() {
    return sceneFunctionMapper.selectList(
        new QueryWrapper<SceneFunctionDO>().eq("is_delete", false));
  }

  public PageableResponse<SceneFunctionDO> findByPage(
      PageableQueryWrapper<SceneQueryRequest> pageableQueryWrapper) {
    ...
    QueryWrapper<SceneFunctionDO> sceneFunctionDOQueryWrapper =
        new QueryWrapper<SceneFunctionDO>()
            .eq("is_delete", false)
    ...
    if (null != query.getOsType() && query.getOsType() == 0) {
      ...
      sceneFunctionDOQueryWrapper.and(
          wrapper ->
              wrapper
                  .like("support_os_types", query.getOsType().toString())
                  .or()
                  .isNull("support_os_types"));
    }
    ...
    IPage<SceneFunctionDO> result =
        sceneFunctionMapper.selectPage(
