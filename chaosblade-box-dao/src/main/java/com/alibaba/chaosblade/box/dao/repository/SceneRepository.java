...
public class SceneRepository implements IRepository<Long, SceneDO> {
  ...
  @Resource private SceneMapper sceneMapper;
  ...
  public Optional<SceneDO> findBySceneId(String sceneId) {
    QueryWrapper<SceneDO> queryWrapper = new QueryWrapper<SceneDO>().eq("scene_id", sceneId);

    return Optional.ofNullable(sceneMapper.selectOne(queryWrapper));
  }

  public Optional<SceneDO> findByCode(String code) {
    QueryWrapper<SceneDO> queryWrapper =
        new QueryWrapper<SceneDO>().eq("code", code).eq("is_delete", false);
  ...
  public Optional<List<SceneDO>> findByUserIdOrPublic(String empId) {
    QueryWrapper<SceneDO> queryWrapper =
        new QueryWrapper<SceneDO>()
            .eq("is_delete", false)
            .and(wrapper -> wrapper.eq("user_id", empId).or().eq("is_public", 1));
  ...
  public boolean updateBySceneId(SceneDO scene) {
    UpdateWrapper<SceneDO> updateWrapper =
        new UpdateWrapper<SceneDO>()
            ...
            .set(
                !Strings.isNullOrEmpty(scene.getDescription()),
                "description",
  ...
  public boolean updateByCode(SceneDO scene) {
    UpdateWrapper<SceneDO> updateWrapper =
        new UpdateWrapper<SceneDO>().eq("code", scene.getCode()).eq("is_delete", false);
  ...
  public Optional<List<SceneDO>> queryAll() {
    QueryWrapper<SceneDO> queryWrapper = new QueryWrapper<SceneDO>().eq("is_delete", false);
  ...
  public boolean deleteBySceneId(String sceneId) {
    ...
    UpdateWrapper<SceneDO> updateWrapper = new UpdateWrapper<SceneDO>().eq("scene_id", sceneId);

    int rows = sceneMapper.update(exist, updateWrapper);
    return rows > 0;
  }

  public PageableResponse<SceneDO> getScenes(
    ...
    IPage<SceneDO> result =
        sceneMapper.selectPage(
  ...
  private QueryWrapper<SceneDO> buildQueryWrapper(
      PageableQueryWrapper<SceneQueryRequest> pageableQueryWrapper) {
