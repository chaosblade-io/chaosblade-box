...
public class LogStoreTriggerListener extends BaseJob implements TriggerListener {
  @Autowired private SchedulerJobTriggerLogMapper schedulerJobTriggerLogMapper;

  private SchedulerJobTriggerLogDO findByFireInstanceId(String fireInstanceId) {
    QueryWrapper<SchedulerJobTriggerLogDO> schedulerJobTriggerLogDOQueryWrapper =
        new QueryWrapper<>();
  ...
  public void triggerComplete(
    ...
    SchedulerJobTriggerLogDO schedulerJobTriggerLogDO =
        findByFireInstanceId(context.getFireInstanceId());
    if (schedulerJobTriggerLogDO != null) {
      schedulerJobTriggerLogDO.setEndTime(
          new Date(context.getFireTime().getTime() + context.getJobRunTime()));
