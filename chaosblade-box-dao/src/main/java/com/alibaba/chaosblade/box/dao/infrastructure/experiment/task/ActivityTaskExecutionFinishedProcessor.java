...
public class ActivityTaskExecutionFinishedProcessor {
  ...
  @Autowired
  private List<ActivityTaskExecutionFinishedInterceptor> activityTaskExecutionFinishedInterceptors;
  ...
  public void afterFinished(
    ...
    if (ignoreFinishedEvent(
        activityTaskDO, activityExecuteResult, experimentTaskRunnableSettings)) {
      return;
    }
    ...
    experimentTaskPusher.push(
        activityTaskDO,
        experimentTaskRunnableSettings.getExperimentTaskDO(),
        experimentTaskRunnableSettings);
  }

  private boolean ignoreFinishedEvent(
      ActivityTaskDO activityTaskDO,
      ActivityExecuteResult activityExecuteResult,
      ExperimentTaskRunnableSettings experimentTaskRunnableSettings) {
    return activityTaskExecutionFinishedInterceptors.stream()
        .anyMatch(
            (Predicate<ActivityTaskExecutionFinishedInterceptor>)
                input ->
                    input.ignoreFinished(
                        activityTaskDO, activityExecuteResult, experimentTaskRunnableSettings));
  }
  ...
  private void setActivityResult(
      ActivityTaskDO activityTaskDO, ActivityExecuteResult activityExecuteResult) {
    ResultEnum resultEnum = ResultEnum.SUCCESS;
    ActivityRunParam activityRunParam = activityTaskDO.getRunParam();
    Integer failedTaskCount =
        activityTargetExecutionResultRepository.countFailedTask(activityTaskDO.getTaskId());
    if (failedTaskCount > 0) {
      ...
      if (tolerance <= 0) {
        ...
        Integer totalTaskCount =
            activityTargetExecutionResultRepository.countTotalTask(activityTaskDO.getTaskId());
        if (totalTaskCount > 0) {
    ...
    if (activityExecuteResult.getError() != null) {
      activityTaskDO.setErrorMessage(
          ExceptionHelper.detailedMessage(activityExecuteResult.getError()));
    }
  ...
  protected boolean waitingUserChecked(ActivityTaskDO activityTaskDO) {
    ...
    return !experimentTaskDO.isStopping()
        && experimentActivityDO.needUserCheck()
        && activityTaskDO.getUserCheckState() == null;
  }

  private void saveExperimentTaskContext(
      ActivityTaskDO activityTaskDO, ActivityExecuteResult activityExecuteResult) {
    try {
      ExperimentTaskContext experimentTaskContext =
          experimentTaskContextManager.getExperimentTaskContext(
              activityTaskDO.getExperimentTaskId());
      experimentTaskContext.setMiniAppContext(activityExecuteResult.getMiniAppContextData());
      experimentTaskContextManager.updateExperimentTaskContext(
          activityTaskDO.getExperimentTaskId(), experimentTaskContext);
    } catch (Exception ex) {
