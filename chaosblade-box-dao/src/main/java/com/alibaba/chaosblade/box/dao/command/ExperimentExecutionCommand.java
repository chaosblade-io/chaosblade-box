...
/** @author haibin */
@Component
public class ExperimentExecutionCommand
    extends SpringBeanCommand<ExperimentRunRequest, Response<ExperimentRunResult>> {

  @Autowired private ExperimentTaskManager experimentTaskManager;

  @Resource private ChaosEventDispatcher chaosEventDispatcher;
  ...
  public Response<ExperimentRunResult> execute(ExperimentRunRequest experimentRunRequest) {
    ...
    ChaosError ChaosError =
        experimentChecker.checkExperimentRunnable(experimentRunRequest, response.getResult());
    if (ChaosError != null) {
    ...
    ExperimentTaskCreateRequest experimentTaskCreateRequest =
        ExperimentTaskCreateRequest.buildExperimentTaskCreateRequest(
            experimentRunRequest, experimentDO);
    // 创建演练任务
    Response<ExperimentTaskDO> createResponse =
        experimentTaskManager.createTask(experimentTaskCreateRequest);
    ...
    ActivityTaskDO activityTaskDO =
        activityTaskRepository
            .findById(createResponse.getResult().getActivityTaskId())
            .orElse(null);
  ...
  private void sendNotification(
    ...
    chaosEventDispatcher.fireEvent(
        new ExperimentTaskStartedEvent(user, createResponse.getResult()));
  }

  private void runTask(
      ExperimentRunRequest experimentRunRequest, ChaosUser user, ActivityTaskDO activityTaskDO) {
