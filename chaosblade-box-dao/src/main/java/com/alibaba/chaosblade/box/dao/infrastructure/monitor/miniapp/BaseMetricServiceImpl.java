...
public abstract class BaseMetricServiceImpl implements MetricService {
  ...
  @Autowired private MetricDefinitionRepository metricDefinitionRepository;
  ...
  public Response<List<ChaosMetricEntity>> getTargetMetric(MetricRequest metricRequest) {
    ...
    MetricDefinition metricDefinition =
        metricDefinitionRepository.findByMetric(metricKey).orElse(null);
  ...
  protected abstract List<ChaosMetricEntity> getMetrics(
  ...
  public Response<List<String>> getMetricKeysByAppCode(String metricAppCode) {
    ...
    return Response.okWithData(
        metricDefinitions.stream()
            .map(
                new Function<MetricDefinition, String>() {
                  @Override
  ...
  public Response<List<ChaosMetricEntity>> getUserDefinitionMetric(
    ...
    HttpResponseFuture<ArrayList<ChaosMetricEntity>> httpResponseFuture =
    ...
    httpResponseFuture =
        HttpClient.postJsonString(
            url, JSON.toJSONString(postBody), new TypeReference<ArrayList<ChaosMetricEntity>>() {});
    httpResponseFuture
        .onThrowable(
            new Consumer<Throwable>() {
              ...
              public void accept(Throwable throwable) {
                response.setError(
                    new ChaosError(
                        CommonErrorCode.S_REQUEST_OUTER_SYSTEM_FAILED, throwable.getMessage()));
                response.setSuccess(false);
              }
            })
        .onResponse(
            new Consumer<ArrayList<ChaosMetricEntity>>() {
              @Override
              public void accept(ArrayList<ChaosMetricEntity> chaosMetricEntities) {
                chaosMetricEntities.forEach(
                    new Consumer<ChaosMetricEntity>() {
                      @Override
