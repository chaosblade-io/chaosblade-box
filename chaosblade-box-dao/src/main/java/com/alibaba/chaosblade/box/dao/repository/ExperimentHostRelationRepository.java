...
public class ExperimentHostRelationRepository
  ...
  @Autowired private ExperimentHostRelationMapper experimentHostRelationMapper;
  ...
  public List<ExperimentHostRelationDO> findByQuery(
      QueryWrapper<ExperimentHostRelationDO> queryWrapper) {
    return experimentHostRelationMapper.selectList(queryWrapper);
  }

  public List<ExperimentHostRelationDO> findRunningScopesByAppId(
      String appId, String injectionType) {
  ...
  public List<ExperimentHostRelationDO> findByExperimentTaskAndInjectionType(
      String experimentTaskId, String injectionType) {
    ExperimentHostRelationDO experimentHostRelationDO = new ExperimentHostRelationDO();
  ...
  public ExperimentHostRelationDO findByConfigurationIdAndExperimentTaskId(
      String configurationId, String experimentTaskId) {
    QueryWrapper<ExperimentHostRelationDO> queryWrapper = new QueryWrapper<>();
  ...
  // select * from t_chaos_experiment_host_relation where
  // app_configuration_id="fb0c2ac36f56bae75f279090a24e40ac" order by
  ...
  public IPage<ExperimentHostRelationDO> findByAppConfigurationId(
    ...
    experimentHostRelationDO.setRelationType(ExperimentHostRelationDO.RELATION_EXPERIMENT_TASK);
    experimentHostRelationDO.setInjectionTargetType(injectionType);
    return experimentHostRelationMapper.selectPage(
        MyBatisUtil.getPage(page, size),
        new QueryWrapper<>(experimentHostRelationDO)
            .orderByAsc(COLUMN_EXPERIMENT_TASK_STATE)
  ...
  public IPage<ExperimentHostRelationDO> findExperimentTaskByConfigurationIdOrderByGmtCreateDesc(
      String configurationId, int page, int size) {
    ...
    return experimentHostRelationMapper.selectPage(
        MyBatisUtil.getPage(page, size), experimentRelationDOQueryWrapper);
  }

  public ExperimentHostRelationDO
      findByAppConfigurationIdAndExperimentTaskIdAndInjectionTypeAndInjectionTargetName(
    ...
    experimentRelationDOQueryWrapper.eq(
        COLUMN_RELATION_TYPE, ExperimentHostRelationDO.RELATION_EXPERIMENT_TASK);
