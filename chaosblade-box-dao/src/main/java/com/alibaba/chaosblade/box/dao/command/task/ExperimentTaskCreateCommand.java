...
public class ExperimentTaskCreateCommand
  ...
  @Autowired private List<ExperimentTaskInterceptor> experimentTaskInterceptors;

  @Override
  public Response<ExperimentTaskDO> execute(
    ...
    try {
      ...
      experimentTaskDO.setExperimentTaskContext(
          buildExperimentTaskContext(experimentTaskCreateRequest));
      ...
      if (experimentTaskCreateRequest.getExperimentRunParam() != null) {
        experimentTaskDO.setOuterUserId(
            experimentTaskCreateRequest.getExperimentRunParam().getEmpId());
      }
      ...
      List<ActivityRunParam> activityRunParams =
          experimentTaskCreateRequest.getExperimentRunParam() != null
              ? experimentTaskCreateRequest.getExperimentRunParam().getActivities()
              : null;
      // 创建每个活动的任务
      ...
      List<ActivityTaskDO> activityTaskDOS =
          activityTaskManager.createActivityTasks(activityTasksCreateRequest);
      ...
      return Response.failedWith(
          throwableChaosErrorWrappers.wrapper(
              ex, new ChaosError(CommonErrorCode.B_CREATE_EXPERIMENT_TASK_FAILED)));
    }
  }

  private ExperimentTaskContext buildExperimentTaskContext(
      ExperimentTaskCreateRequest experimentTaskCreateRequest) {
    ExperimentTaskContext experimentTaskContext = new ExperimentTaskContext();
    experimentTaskContext.setSchedulerConfig(
        experimentTaskCreateRequest.getExperimentDO().getSchedulerConfig());
    return experimentTaskContext;
  }

  private void beforeSaveExperimentTaskDO(
    ...
    for (ExperimentTaskInterceptor experimentTaskInterceptor : experimentTaskInterceptors) {
      experimentTaskInterceptor.beforeSaveExperimentTaskDO(
          experimentTaskCreateRequest, experimentTaskDO);
    }
  }
}
