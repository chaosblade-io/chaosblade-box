...
public class WorkspaceRelationRepository implements IRepository<String, WorkspaceRelationDO> {
  ...
  @Resource private WorkspaceRelationMapper workspaceRelationMapper;
  ...
  private QueryWrapper<WorkspaceRelationDO> buildQuery(WorkspaceRelationQuery query) {
    return new QueryWrapper<WorkspaceRelationDO>()
        .in(
            null != query.getRelationIds() && !query.getRelationIds().isEmpty(),
            "relation_id",
            query.getRelationIds())
        .in(
            null != query.getOuterIds() && !query.getOuterIds().isEmpty(),
            "outer_id",
            query.getOuterIds())
        //            .eq(null != query.getUserRole(), "user_role", query.getUserRole())
        //            .eq(null != query.getMemberRole(), "member_role", query.getMemberRole())
        .in(
            null != query.getWorkspaceIds() && !query.getWorkspaceIds().isEmpty(),
            "workspace_id",
            query.getWorkspaceIds())
        .eq(
            !Strings.isNullOrEmpty(query.getRelationType()),
            "relation_type",
            query.getRelationType())
        .eq(!Strings.isNullOrEmpty(query.getNamespace()), "namespace", query.getNamespace())
        .eq(
            !Strings.isNullOrEmpty(query.getExperimentCreator()),
            "experiment_creator",
            query.getExperimentCreator())
        .eq(query.isExcludeDeleted(), "is_delete", false);
  }

  public int deleteWorkspaceRelation(
      WorkspaceRelationQuery workspaceRelationQuery, boolean logical) {
  ...
  public Integer count(WorkspaceRelationQuery query) {
    QueryWrapper<WorkspaceRelationDO> queryWrapper =
        new QueryWrapper<WorkspaceRelationDO>()
            .in(
                null != query.getRelationIds() && !query.getRelationIds().isEmpty(),
                "relation_id",
                query.getRelationIds())
  ...
  public boolean addExperimentCreator(
      String workspaceId, String experimentId, String experimentCreator) {
    QueryWrapper<WorkspaceRelationDO> queryWrapper =
        new QueryWrapper<WorkspaceRelationDO>()
            .eq("workspace_id", workspaceId)
            .eq("relation_type", WorkspaceRelationTypes.EXPERIMENT)
  ...
  public boolean update(WorkspaceRelationDO workspaceRelation) {
    UpdateWrapper<WorkspaceRelationDO> updateWrapper =
        new UpdateWrapper<WorkspaceRelationDO>()
