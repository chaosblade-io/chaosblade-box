...
public class ListExperimentOperationLogCommand
    extends SpringBeanCommand<
        ExperimentPageableQueryRequest, Response<PageableResponse<ExperimentOperationLog>>> {
  ...
  public Response<PageableResponse<ExperimentOperationLog>> execute(
    ...
    PageableResponse<ChangelogDO> pageableResponse =
        changelogService.getChangelogs(
            experimentPageableQueryRequest.getPage(),
            experimentPageableQueryRequest.getSize(),
            changelogQuery);
    PageableResponse<ExperimentOperationLog> experimentOperationLogPageableResponse =
        PageableResponse.of(
                ...
                .map(
                    changelogDO -> {
                      ...
                      experimentOperationLog.setChangeType(
                          BaseChangeLogTracker.getChangeActionTypeDescription(
                              changelogDO.getChangeType()));
                      experimentOperationLog.setPropertyId(changelogDO.getPropertyId());
                      return experimentOperationLog;
                    })
                .collect(Collectors.toList()),
            pageableResponse.getPages(),
            pageableResponse.getTotal());
    return Response.okWithData(experimentOperationLogPageableResponse);
  }

  private void fillOperatorInfo(
      ExperimentOperationLog experimentOperationLog, ChangelogDO changelogDO) {
    for (ChangelogTypes.ChangeOperatorType changeOperatorType :
        ChangelogTypes.ChangeOperatorType.values()) {
