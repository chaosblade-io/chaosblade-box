...
public class ExperimentTaskPusher {
  ...
  @Autowired private List<ExperimentTaskPushInterceptor> experimentTaskPushInterceptors;
  ...
  public void push(
    ...
    if (ignorePush(activityTaskDO, experimentTaskDO, taskRunnableSettings)) {
      log.info("Not Push experiment task:" + experimentTaskDO.getTaskId());
      return;
    }
    ActivityTaskDO nextActivityTaskDO =
        acquireNextTask(experimentTaskDO, activityTaskDO, taskRunnableSettings);
    if (nextActivityTaskDO != null) {
      log.info("Push experiment task:" + experimentTaskDO.getTaskId());
   ...
   */
  private void executeNextActivityTask(
      ActivityTaskDO nextActivityTaskDO,
      ExperimentTaskRunnableSettings experimentTaskRunnableSettings) {
    ChaosUser chaosUser =
        experimentTaskRunnableSettings.getUser() == null
            ? new ChaosUser(experimentTaskRunnableSettings.getUserId())
            : experimentTaskRunnableSettings.getUser();
    ExperimentExecuteContext newExperimentExecuteContext =
        new ExperimentExecuteContext(
            experimentTaskRunnableSettings.getRequestId(),
            chaosUser,
            nextActivityTaskDO,
  ...
  private ActivityTaskDO acquireNextTask(
      ExperimentTaskDO experimentTaskDO,
  ...
  public ActivityTaskDO findNextRecoveryActivityTaskDO(ActivityTaskDO currentActivityTask) {
    return activityTaskRepository
        .findByExperimentTaskIdAndPhase(
            currentActivityTask.getExperimentTaskId(), PhaseType.RECOVER)
        .stream()
        .filter(
  ...
  private boolean ignorePush(
      ...
      ExperimentTaskDO experimentTaskDO,
      ExperimentTaskRunnableSettings taskRunnableSettings) {
    return experimentTaskPushInterceptors.stream()
        .anyMatch(
            new Predicate<ExperimentTaskPushInterceptor>() {
              @Override
              public boolean test(ExperimentTaskPushInterceptor experimentTaskPushInterceptor) {
                return experimentTaskPushInterceptor.ignorePush(
                    activityTaskDO, experimentTaskDO, taskRunnableSettings);
              }
            });
  }

  public void pushByAsyncWay(ExperimentMiniAppTaskDO activityTargetTaskDO) {
    if (!activityTargetTaskDO.isFinished()) {
      return;
    }
    String activityTaskId = activityTargetTaskDO.getActivityTaskId();
    try {
      ActivityTaskDO activityTaskDO =
          activityTaskRepository.findById(activityTargetTaskDO.getActivityTaskId()).orElse(null);
      ...
      boolean allFinished = miniAppTaskDOS.stream().allMatch(BaseTaskDO::isFinished);
      ...
      if (allFinished) {
        ...
        activityTaskExecutionFinishedProcessor.afterFinished(
            activityTaskDO, activityExecuteResult, hashMapSettings);
      }
  ...
  private boolean isActivityTaskSuccess(List<ExperimentMiniAppTaskDO> experimentMiniAppTaskDOS) {
    return experimentMiniAppTaskDOS.stream()
        .allMatch(
            experimentMiniAppTaskDO ->
                ResultEnum.SUCCESS.equals(experimentMiniAppTaskDO.getResultEnum()));
  }
}
