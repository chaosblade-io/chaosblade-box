...
public class ArgumentDefinitionConvertor {
  ...
  @Autowired private ApplicationSceneFunctionParamFilter applicationSceneFunctionParamFilter;

  public ExperimentNodeArgumentsDefinition mergeArgumentDefinitions(
      String appCode, List<SceneArgumentDefinition> arguments) {
    ExperimentNodeArgumentsDefinition experimentNodeArgumentsDefinition =
        new ExperimentNodeArgumentsDefinition();
    ...
    for (SceneArgumentDefinition sceneArgumentDefinition : arguments) {
      ...
      experimentNodeArgumentsDefinition
          .getArgsComponents()
          .put(sceneArgumentDefinition.getAlias(), sceneArgumentDefinition.getComponent());
    }
  ...
  public List<SceneArgumentDefinition> convertToArgumentDefinitions(
      String appCode, List<SceneFunctionParameterDO> sceneFunctionParameterDOS, boolean fromApp) {
    return sceneFunctionParameterDOS.stream()
        .filter(
            new Predicate<SceneFunctionParameterDO>() {
              @Override
              public boolean test(SceneFunctionParameterDO sceneFunctionParameterDO) {
                if (fromApp) {
                  return !applicationSceneFunctionParamFilter.skip(
                      appCode, sceneFunctionParameterDO.getAlias());
                }
                if (sceneFunctionParameterDO.isInvisibleWithNoDep()) {
                  return false;
                }
                return true;
              }
            })
        .sorted(getSceneFunctionParameterDOComparator())
        .map(
  ...
  public List<SceneArgumentDefinition> splitExperimentNodeArgumentsDefinition(
      String appCode, ExperimentNodeArgumentsDefinition experimentNodeArgumentsDefinition) {
    ...
    return sceneFunctionParameterDOS.stream()
        ...
        .map(
            sceneFunctionParameterDO -> {
              if (argAlias.contains(sceneFunctionParameterDO.getAlias())) {
                return null;
              }
              argAlias.add(sceneFunctionParameterDO.getAlias());
              SceneArgumentDefinition sceneArgumentDefinition = new SceneArgumentDefinition();
              sceneArgumentDefinition.setAlias(sceneFunctionParameterDO.getAlias());
              sceneArgumentDefinition.setValue(
                  userArguments.get(sceneFunctionParameterDO.getAlias()));
              sceneArgumentDefinition.setEnabled(true);
              ...
              sceneArgumentDefinition.setFunctionId(sceneFunctionParameterDO.getFunctionId());
              mergeComponent(
                  sceneFunctionParameterDO.getComponent(),
                  experimentNodeArgumentsDefinition
                      .getArgsComponents()
                      .get(sceneFunctionParameterDO.getAlias()));
              sceneArgumentDefinition.setComponent(sceneFunctionParameterDO.getComponent());
              return sceneArgumentDefinition;
            })
        .filter(sceneArgumentDefinition -> sceneArgumentDefinition != null)
        .collect(Collectors.toList());
  }

  private void mergeComponent(
      SceneFunctionParameterComponent latestComponent,
