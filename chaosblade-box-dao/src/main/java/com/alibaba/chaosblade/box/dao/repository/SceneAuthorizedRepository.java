...
/** @author sunju Date 2019-08-20 */
@Slf4j
@Repository
public class SceneAuthorizedRepository implements IRepository<String, SceneAuthorizedDO> {

  @Resource private SceneAuthorizedMapper sceneAuthorizedMapper;

  @Override
  public Optional<SceneAuthorizedDO> findById(String authorizedId) {
    return Optional.ofNullable(
        sceneAuthorizedMapper.selectOne(
            new QueryWrapper<SceneAuthorizedDO>().eq("authorized_id", authorizedId)));
  }
  ...
  public boolean update(SceneAuthorizedDO sceneAuthorized) {
    return sceneAuthorizedMapper.update(
            sceneAuthorized,
            new UpdateWrapper<SceneAuthorizedDO>()
                .eq("authorized_id", sceneAuthorized.getAuthorizedId()))
        > 0;
  }

  public boolean updateByAuthorizedId(SceneAuthorizedUpdateRequest request) {
    if (Strings.isNullOrEmpty(request.getAuthorizedId())
        && CollectionUtil.isNullOrEmpty(request.getAuthorizedIds())) {
      return false;
    }
  ...
  public boolean updateByFunctionId(SceneAuthorizedUpdateRequest request) {
    if (Strings.isNullOrEmpty(request.getFunctionId())
        && CollectionUtil.isNullOrEmpty(request.getFunctionIds())) {
      return false;
    }
  ...
  public PageableResponse<SceneAuthorizedDO> getAuthorizedRecords(
    ...
    IPage<SceneAuthorizedDO> result =
        sceneAuthorizedMapper.selectPage(
  ...
  private QueryWrapper<SceneAuthorizedDO> buildQueryWrapper(SceneAuthorizedQueryRequest query) {
    ...
    if (null != query.getPermission()) {
      queryWrapper.apply(
          String.format("(permission & %d) = %d", query.getPermission(), query.getPermission()));
    }
    ...
    if (null != query.getSupportScopeType()) {
      if (Objects.equals(
          query.getSupportScopeType(), ChaosFunctionConstant.SUPPORT_SCOPE_TYPE_HOST)) {
        queryWrapper.eq("support_host", true);
      }
      if (Objects.equals(
          query.getSupportScopeType(), ChaosFunctionConstant.SUPPORT_SCOPE_TYPE_K8S)) {
  ...
  private void appendK8sResourceQuery(
    ...
    if (ChaosFunctionConstant.K8_RESOURCE_TYPE_POD == k8sResourceType) {
      queryWrapper.and(
          wrapper ->
              wrapper
                  .likeRight("function_code", "chaos.pod-")
                  .or()
    ...
    if (ChaosFunctionConstant.K8_RESOURCE_TYPE_NODE == k8sResourceType) {
      queryWrapper.and(
          wrapper ->
              wrapper
                  .likeRight("function_code", "chaos.node-")
                  .or()
    ...
    if (ChaosFunctionConstant.K8_RESOURCE_TYPE_CONTAINER == k8sResourceType) {
      queryWrapper.and(
          wrapper ->
              wrapper
                  .likeRight("function_code", "chaos.container-")
                  .or()
                  .like("function_code", "litmuschaos.container-")
                  .or()
                  .in("function_code", "chaos.pod-pod.delete"));
    }
  }
