...
public class FeedbackProcessor implements InitializingBean {
  ...
  @Autowired private List<ExtraFeedbackComponentPostProcessor> extraFeedbackComponentPostProcessors;

  public Response<ExperimentTaskDO> checkTaskAllowFeedback(String experimentTaskId) {
    Response<ExperimentTaskDO> response =
        experimentChecker.assertExperimentTaskExist(experimentTaskId);
  ...
  public ExperimentTaskFeedbackDO createExperimentTaskFeedback(
    ...
    // 默认是符合预期，不影响业务
    newExperimentFeedTaskDO.setBusinessStatus(
        FeedBackConstant.BusinessImpactStatus.BUSINESS_IMPACT_STATUS_NO.getValue());
    newExperimentFeedTaskDO.setExpectationStatus(
        FeedBackConstant.ExpectationStatus.EXPECTATION_STATUS_YES.getValue());
    if (experimentTaskDO.isFinished()) {
      feedbackRepository.add(newExperimentFeedTaskDO);
      updateTaskFeedbackStatus(
          experimentTaskDO, FeedBackConstant.FeedbackStatus.FEEDBACK_STATUS_WAITING);
    }
  ...
  public Response<FeedbackSubmitResult> submitExperimentTaskFeedback(
    ...
    boolean allowFeedback =
        experimentTaskDO.getFeedBackStatus() != null || experimentTaskDO.isFinished();
    if (!allowFeedback) {
    ...
    return transactionUtil.execute(
        new TransactionCallback<Response<FeedbackSubmitResult>>() {
          @Override
          public Response<FeedbackSubmitResult> doInTransaction(
            ...
            updateTaskFeedbackStatus(
                experimentTaskDO, FeedBackConstant.FeedbackStatus.FEEDBACK_STATUS_FINISHED);
            try {
            ...
            if (experimentTaskFeedback.getExtra() != null) {
              feedbackSubmitResult.setRedirectUrl(
                  experimentTaskFeedback.getExtra().getRedirectUrl());
            }
  ...
  private void updateTaskFeedbackStatus(
      ExperimentTaskDO experimentTaskDO, FeedBackConstant.FeedbackStatus feedbackStatus) {
    experimentTaskRepository.updateFeedbackStatus(experimentTaskDO.getTaskId(), feedbackStatus);
  }

  private ExperimentTaskFeedbackDO initExperimentTaskFeedbackDO(
      ChaosUser chaosUser,
      ExperimentTaskDO experimentTaskDO,
      ExperimentTaskFeedback experimentTaskFeedback) {
    ExperimentTaskFeedbackDO experimentTaskFeedbackDO =
        feedbackRepository.findByExperimentTaskId(experimentTaskDO.getTaskId()).orElse(null);
    ...
    for (ExtraFeedbackComponentPostProcessor extraFeedbackComponentPostProcessor :
      ...
      try {
        extraFeedbackComponent =
            extraFeedbackComponentPostProcessor.acquireExtraFeedbackComponent(experimentTaskDO);
        if (extraFeedbackComponent != null) {
          return extraFeedbackComponent;
        }
      } catch (Exception exception) {
        log.error(
            "provide extraFeedbackComponent failed,class:{}",
            extraFeedbackComponentPostProcessor.getClass().getName(),
            exception);
        break;
      }
    }
  ...
  private void consumeExtraFeedbackComponent(
      ExperimentTaskDO experimentTaskDO, ExtraFeedbackComponent extraFeedbackComponent)
      throws Throwable {
    for (ExtraFeedbackComponentPostProcessor extraFeedbackComponentPostProcessor :
        extraFeedbackComponentPostProcessors) {
      if (extraFeedbackComponentPostProcessor.submitExtraFeedbackComponent(
          experimentTaskDO, extraFeedbackComponent)) {
        break;
      }
    }
  }
