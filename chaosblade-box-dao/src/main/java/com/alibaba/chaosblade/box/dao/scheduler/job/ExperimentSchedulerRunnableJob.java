...
public class ExperimentSchedulerRunnableJob extends BaseJob implements Job {
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    ...
    if (schedulerJobDO.getEnabled()) {
      ...
      ExperimentDO experimentDO =
          experimentRepository.findById(schedulerJobDO.getBusinessId()).orElse(null);
      ...
      Stream.of(experimentDO)
          .filter(
              experimentDO1 -> {
                boolean isLastExperimentFailed =
                    isLastExperimentTaskFailedFrequently(experimentDO1);
          ...
          .forEach(
              experimentDO12 -> {
                ...
                Response<ExperimentRunResult> resultResponse =
                    commandBus.syncRun(ExperimentExecutionCommand.class, experimentRunRequest);
                if (!resultResponse.isSuccess()) {
                  logger.warn(
                      "run experiment failed:experimentId:{},{}",
                      schedulerJobDO.getBusinessId(),
  ...
  private void trackers(ExperimentDO experimentDO, ChaosUser chaosUser) {
    trackers.trackExperimentOperation(
        ChangelogTypes.ChangeActionType.RUN,
        ChangelogTypes.ChangeOperatorType.SCHEDULER,
        chaosUser,
  ...
  private boolean isLastExperimentTaskFailedFrequently(ExperimentDO experimentDO) {
    ...
    List<String> taskIds =
        experimentTaskRepository
            .findByExperimentTasksOrderByExperimentDesc(
                experimentDO.getExperimentId(), FAILED_TASK_TOLERANCE)
            .stream()
            .map(ExperimentTaskDO::getTaskId)
