...
public class SceneFunctionCategoryManager {
  ...
  public void rebindFunctionCategories(List<SceneFunctionDO> sceneFunctionDOS) {
    sceneFunctionDOS.forEach(
        sceneFunctionDO -> {
          ...
          if (CollectionUtil.isNullOrEmpty(sceneFunctionDO.getCategoryList())) {
            return;
          }
          List<SceneFunctionCategoryRelationDO> sceneFunctionCategoryRelationDOS =
              sceneFunctionDO.getCategoryList().stream()
                  .map(
                      s -> {
                        ...
                        sceneFunctionCategoryRelationDO.setFunctionId(
  ...
  public List<String> getFunctionIdByCategoryId(String categoryId) {
    return sceneFunctionCategoryRelationRepository.findByCategoryId(categoryId).stream()
        .map(SceneFunctionCategoryRelationDO::getFunctionId)
        .collect(Collectors.toList());
  }
}
