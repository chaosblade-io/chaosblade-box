...
public class StatusInitActivityExecutionInterceptor implements ActivityExecutionInterceptor {
  ...
  private void initRecoveryPhase(ActivityTaskDO activityTaskDO) {
    ...
    List<ActivityTaskDO> activityTaskDOS =
        activityTaskRepository.findByAttackTaskId(activityTaskDO.getTaskId());
    if (activityTaskDOS.isEmpty()) {
      return;
    }
    activityTaskDOS.forEach(
        activityTaskDO1 -> {
          ...
          activityTargetExecutionResultRepository
              .findByActivityTaskId(activityTaskDO1.getTaskId())
              .forEach(
                  experimentActivityTargetTaskDO -> {
                    if (experimentActivityTargetTaskDO.isFinished()) {
                      ...
                      activityTargetExecutionResultRepository.update(
                          experimentActivityTargetTaskDO);
                    }
                  });
        });
