...
public class PrivateCloudDeviceRepository {
  ...
  public List<DeviceDO> getAliveDevices(CloudDeviceQuery query) {
    QueryWrapper<DeviceDO> queryWrapper =
  ...
  public List<DeviceDO> findByUserIdAndDeviceTypeAndConfigurationId(
      String userId, int deviceType, String cid) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
    queryWrapper.eq("user_id", userId).eq("configuration_id", cid).eq("device_type", deviceType);
    return deviceMapper.selectList(queryWrapper);
  }
  ...
  public boolean updateHeartBeatTimeByDeviceDO(
      String configurationId, String version, String chaosVersion, Date heartBeatTime, String ip) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
