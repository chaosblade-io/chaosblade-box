...
@Repository
@Slf4j
public class SceneFunctionParameterRepository
    implements IRepository<Long, SceneFunctionParameterDO> {

  @Resource private SceneFunctionParameterMapper sceneFunctionParameterMapper;
  ...
  public SceneFunctionParameterDO findByParameterId(String parameterId) {
    QueryWrapper<SceneFunctionParameterDO> queryWrapper =
        new QueryWrapper<SceneFunctionParameterDO>().eq("parameter_id", parameterId);
  ...
  public List<SceneFunctionParameterDO> findByParameterIds(List<String> parameterIds) {
    QueryWrapper<SceneFunctionParameterDO> queryWrapper =
        new QueryWrapper<SceneFunctionParameterDO>().in("parameter_id", parameterIds);
  ...
  public boolean existParam(String functionId, String alias) {
    ...
    return sceneFunctionParameterMapper.selectCount(new QueryWrapper<>(sceneFunctionParameterDO))
        > 0;
  }

  public Optional<List<SceneFunctionParameterDO>> findAllParamsByFunctionId(String functionId) {
    QueryWrapper<SceneFunctionParameterDO> queryWrapper =
        new QueryWrapper<SceneFunctionParameterDO>()
            .eq("is_delete", false)
            .eq("function_id", functionId);
    return Optional.ofNullable(sceneFunctionParameterMapper.selectList(queryWrapper));
  }

  public List<SceneFunctionParameterDO> findFilterParamsByFunctionId(
    ...
    return sceneFunctionParameterInterceptors.doUserParameterFilter(
        sceneFunctionDO, sceneFunctionParameterDOS);
  }
  ...
  public boolean updateByParameterId(SceneFunctionParameterDO sceneFunctionParameter) {
    ...
    int rows =
        sceneFunctionParameterMapper.update(sceneFunctionParameter, new QueryWrapper<>(whereRule));
    return rows > 0;
  }
  ...
  public boolean add(SceneFunctionParameterDO sceneFunctionParameter) {
    ...
    log.info(
        "add param,functionId:{},alias:{},id:{},rows:{}",
        sceneFunctionParameter.getFunctionId(),
        sceneFunctionParameter.getAlias(),
        sceneFunctionParameter.getParameterId(),
        rows);
    return rows > 0;
  }
  ...
  public boolean deleteByParameterId(String functionId, String parameterId) {
    ...
    return sceneFunctionParameterMapper.update(update, new QueryWrapper<>(sceneFunctionParameterDO))
        > 0;
  }
  ...
  public boolean deleteByFunctionId(String functionId) {
    ...
    QueryWrapper<SceneFunctionParameterDO> queryWrapper =
        new QueryWrapper<SceneFunctionParameterDO>().eq("function_id", functionId);
    SceneFunctionParameterDO sceneFunctionParameterDO = new SceneFunctionParameterDO();
