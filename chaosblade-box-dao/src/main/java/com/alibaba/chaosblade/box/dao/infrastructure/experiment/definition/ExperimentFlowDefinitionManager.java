...
public class ExperimentFlowDefinitionManager implements InitializingBean {
  ...
  @Autowired private MiniAppCodeHelper miniAppCodeHelper;
  ...
  public List<Host> findAllHostsByExperimentId(String experimentId) {
    return experimentMiniFlowGroupRepository.findByExperimentId(experimentId).stream()
        .flatMap(
            (Function<MiniFlowGroupDO, Stream<Host>>)
                miniFlowGroupDO -> miniFlowGroupDO.getHosts().getContent().stream())
        .distinct()
   ...
   * @return
   */
  public void addFlowDefinition(
      ExperimentDO experimentDO, ExperimentFlowDefinition experimentFlowDefinition) {
    List<ExperimentActivityDO> experimentActivityDOS =
        parseFlow(experimentDO, experimentFlowDefinition, true);
   ...
   * @return
   */
  public boolean updateFlowDefinition(
      ExperimentDO experimentDO, ExperimentFlowDefinition experimentFlowDefinition) {
    String experimentId = experimentDO.getExperimentId();
    List<ExperimentActivityDO> newExperimentActivityDOS =
        parseFlow(experimentDO, experimentFlowDefinition, false);
    Map<String, ExperimentActivityDO> newIdToExperimentActivityDO =
        newExperimentActivityDOS.stream()
            .collect(
                Collectors.toMap(
                    ExperimentActivityDO::getActivityId,
                    experimentActivityDO -> experimentActivityDO));
    List<ExperimentActivityDO> oldExperimentExperimentActivityDOs =
        activityRepository.findActivities(experimentId);
    Map<String, ExperimentActivityDO> oldIdToExperimentActivityDO =
        oldExperimentExperimentActivityDOs.stream()
            .collect(
                Collectors.toMap(
                    ExperimentActivityDO::getActivityId,
                    experimentActivityDO -> experimentActivityDO));
   ...
   * @return
   */
  private List<ExperimentActivityDO> parseFlow(
    ...
    ExperimentFlowDefinitionMeta.ExperimentActivitiesMap experimentActivitiesMap =
        new ExperimentFlowDefinitionMeta.ExperimentActivitiesMap();
    experimentFlowDefinitionMeta.setActivitiesMap(experimentActivitiesMap);
    // 先解析attack环节
    parsePhase(
        experimentFlowDefinitionMeta, experimentFlowDefinition.getAttack(), PhaseType.ATTACK);

    // 解析准备环节
    parsePhase(
        experimentFlowDefinitionMeta, experimentFlowDefinition.getPrepare(), PhaseType.PREPARE);
    ...
    // 解析Recovery环节
    parsePhase(
        experimentFlowDefinitionMeta, experimentFlowDefinition.getRecover(), PhaseType.RECOVER);
  ...
  private void parserExperimentDefinition(
      ExperimentFlowDefinitionMeta experimentFlowDefinitionMeta,
      List<ExperimentActivityDefinition> activityDefinitions,
      PhaseType phaseType) {
    ...
    if (!CollectionUtil.isNullOrEmpty(activityDefinitions)) {
      for (ExperimentActivityDefinition experimentActivityDefinition : activityDefinitions) {
        ...
        count++;
        preHandleExperimentActivityDefinition(
            experimentActivityDefinition, phaseType, experimentFlowDefinitionMeta);
        ExperimentActivityDO experimentActivityDO =
            buildExperimentActivityDO(
                count, experimentId, experimentActivityDefinition, phaseType, create);
        experimentFlowDefinitionMeta
            .getActivitiesMap()
            .addExperimentActivityDO(phaseType, experimentActivityDO);
      }
    }
  }
  ...
  private void handleAttackPhase(ExperimentFlowDefinitionMeta experimentFlowDefinitionMeta) {
    experimentFlowDefinitionMeta.setIncludeJvm(
        experimentFlowDefinitionMeta.getActivitiesMap()
            .getExperimentActivityDOSByPhase(PhaseType.ATTACK).stream()
            .anyMatch(
                experimentActivityDO ->
                    MiniAppUtils.isJvm(experimentActivityDO.getExecutableAppCode())));
    if (experimentFlowDefinitionMeta.isIncludeJvm()) {
      experimentFlowDefinitionMeta
          .getExperimentDO()
          .putAttribute(
              ExperimentAttributes.ATTR_JVM_INCLUDE,
              ActivityTaskExecutionAttributes.ATTRIBUTE_VALUE_TRUE);
    } else {
      experimentFlowDefinitionMeta
          .getExperimentDO()
          .removeAttribute(ExperimentAttributes.ATTR_JVM_INCLUDE);
    }
  }
  ...
  private void handleRecoveryPhase(ExperimentFlowDefinitionMeta experimentFlowDefinitionMeta) {
    experimentFlowDefinitionMeta
        .getActivitiesMap()
        .getExperimentActivityDOSByPhase(PhaseType.ATTACK)
        .forEach(
            experimentActivityDO ->
                handleRecoveryActivity(experimentFlowDefinitionMeta, experimentActivityDO));
    experimentFlowDefinitionMeta
        .getActivitiesMap()
        .getExperimentActivityDOSByPhase(PhaseType.PREPARE)
        .forEach(
            experimentActivityDO ->
                handleRecoveryActivity(experimentFlowDefinitionMeta, experimentActivityDO));
  }
  ...
  private void handleRecoveryActivity(
    ...
    if (MiniAppUtils.isFromChaosBlade(attackExperimentActivityDO.getExecutableAppCode())) {
      String recoverCode =
          MiniAppUtils.getRecoverCode(attackExperimentActivityDO.getExecutableAppCode());
      // 找出所有的恢复节点
      List<ExperimentActivityDO> recoveryActivities =
          findRecoveryActivities(
              experimentFlowDefinitionMeta, attackExperimentActivityDO, recoverCode);
      if (recoveryActivities.isEmpty()) {
        // 如果恢复节点为空，那么自动添加
        experimentFlowDefinitionMeta
            .getActivitiesMap()
            .addExperimentActivityDO(
                PhaseType.RECOVER,
                buildRecoverExperimentActivityDO(
                    recoverCode,
                    attackExperimentActivityDO,
                    experimentFlowDefinitionMeta.isCreate()));
      } else {
        ...
        if (experimentFlowDefinitionMeta.getExperimentDO().isNewExperimentDefinition()
          ...
          recoveryActivities.forEach(
              experimentActivityDO ->
                  experimentActivityDO.setAttackActivityId(
                      attackExperimentActivityDO.getActivityId()));
        } else {
          // 老流程的话，因为没办法区分哪一个恢复节点属于哪一个攻击节点
          recoveryActivities.removeIf(
              experimentActivityDO ->
                  experimentActivityDO.getAppCode().equals(recoverCode)
  ...
  private List<ExperimentActivityDO> findRecoveryActivities(
      ExperimentFlowDefinitionMeta experimentFlowDefinitionMeta,
      ExperimentActivityDO attackExperimentActivity,
      String recoveryCode) {
    List<ExperimentActivityDO> recoveryActivities = new ArrayList<>();
    if (experimentFlowDefinitionMeta.getExperimentDO().isNewExperimentDefinition()) {
      recoveryActivities =
          experimentFlowDefinitionMeta.getActivitiesMap()
              .getFlowIdByExperimentActivityDO(attackExperimentActivity.getFlowId()).stream()
              .filter(
                  experimentActivityDO ->
                      experimentActivityDO.getExecutableAppCode().equals(recoveryCode))
              .collect(Collectors.toList());
    } else {
      recoveryActivities =
          experimentFlowDefinitionMeta
   ...
   * @return
   */
  private ExperimentActivityDO buildRecoverExperimentActivityDO(
      String recoveryCode, ExperimentActivityDO attackExperimentActivityDO, boolean create) {
    ...
    experimentActivityDefinition.setScope(
        attackExperimentActivityDO.getActivityDefinition().getScope());
    experimentActivityDefinition.setSync(
        attackExperimentActivityDO.getActivityDefinition().isSync());
    experimentActivityDefinition.setUserCheck(
        attackExperimentActivityDO.getActivityDefinition().isUserCheck());
    ExperimentActivityDO recovery =
        buildExperimentActivityDO(
            attackExperimentActivityDO.getActivityOrder(),
   ...
   */
  private void preHandleExperimentActivityDefinition(
    ...
    for (ExperimentActivityDefinitionInterceptor experimentActivityDefinitionInterceptor :
      ...
      experimentActivityDefinitionInterceptor.preInterceptor(
          experimentActivityDefinition, phaseType, experimentFlowDefinitionMeta);
    }
  }

  private ExperimentActivityDO buildExperimentActivityDO(
      int index,
      String experimentId,
      ExperimentActivityDefinition experimentActivityDefinition,
      PhaseType phaseType,
    ...
    if (experimentActivityDefinition.getExecutableAppCode() == null) {
      ...
      experimentActivityDO.setExecutableAppCode(
          experimentActivityDefinition.getExecutableAppCode());
    }
   ...
   * @param experimentActivityDO
   */
  private void fillActivityName(
      ExperimentActivityDefinition experimentActivityDefinition,
      ExperimentActivityDO experimentActivityDO) {
    if (Strings.isNullOrEmpty(experimentActivityDefinition.getActivityName())) {
      experimentActivityDO.setActivityName(
          sceneFunctionRepository
              .findByCode(experimentActivityDefinition.getAppCode())
              .orElse(SceneFunctionDO.Empty)
              .getName());
    } else {
  ...
  public List<ExperimentActivityDefinition> getActivityDefinitions(
    ...
    return experimentActivityDOS.stream()
        .sorted(Comparator.comparingInt(ExperimentActivityDO::getActivityOrder))
        .map(
            experimentActivityDO -> {
              ExperimentActivityDefinition experimentActivityDefinition =
                  experimentActivityDO.getActivityDefinition();
              // 这里需要增加flowId
  ...
  private void fillExecutableAppCode(ExperimentActivityDefinition experimentActivityDefinition) {
    Optional<SceneFunctionDO> sceneFunctionDO =
        miniAppCodeHelper.getExecutableSceneFunction(experimentActivityDefinition.getAppCode());
    experimentActivityDefinition.setExecutableAppCode(
        sceneFunctionDO.isPresent()
            ? sceneFunctionDO.get().getCode()
