...
@Component
@Slf4j
public class ChaosBladeFunctionLoader {

  @Autowired private SceneDaoFunctionService sceneDaoFunctionService;
  ...
  public List<SceneFunctionDO> load(ChaosBladeInvoker chaosBladeInvoker, boolean forceFresh)
      throws Exception {
      ...
      if (modelSpecBean.getActions() != null) {
        for (ActionSpecBean actionSpecBean : modelSpecBean.getActions()) {
          SceneFunctionDO sceneFunctionDO =
              buildByActionSpecBean(modelSpecBean, actionSpecBean, prepareSceneFunctionDO);
          ...
          log.info(
              "load attack scene function from ChaosBlade,code:{},deps:{}",
              sceneFunctionDO.getCode(),
              sceneFunctionDO.getDependencies());
        }
      }
      if (prepareSceneFunctionDO != null) {
        SceneFunctionDO uninstallSceneFunctionDO =
            buildUninstallActionFunctionDO(prepareSpecBean, modelSpecBean, prepareSceneFunctionDO);
  ...
  private void loadChaosMetadata(List<SceneFunctionDO> sceneFunctionDOS) {
    sceneFunctionDOS.forEach(
        sceneFunctionDO -> {
          ...
          chaosBladeAction.setHasJavaInstall(
              hasAgentInstall(sceneFunctionDO, target, chaosBladeAction));
  ...
  private boolean hasAgentInstall(
      SceneFunctionDO sceneFunctionDO, String target, ChaosBladeAction chaosBladeAction) {
  ...
  private void saveSceneFunctionDOs(List<SceneFunctionDO> sceneFunctionDOS, boolean forceFresh)
    ...
    if (applicationStartUpConfig.isSync(ApplicationStartUpConfig.SyncFunctionType.ChaosBlade)) {
      try {
        TimeUnit.SECONDS.sleep(new Random().nextInt(10));
      } catch (InterruptedException e) {
        e.printStackTrace();
      }
      sceneDaoFunctionService.rebuildChaosBladeFunctions(sceneFunctionDOS, forceFresh);
    }
  }

  private SceneFunctionDO buildUninstallActionFunctionDO(
      PrepareSpecBean prepareSpecBean,
      ModelSpecBean modelSpecBean,
    ...
    sceneFunctionDO.setCode(
        buildAppCode(prepareSpecBean.getType(), modelSpecBean.getScope(), "uninstall"));
  ...
  private SceneFunctionDO buildByActionSpecBean(
      ModelSpecBean modelSpecBean, ActionSpecBean actionSpecBean, SceneFunctionDO prepareFunction) {
    SceneFunctionDO sceneFunctionDO = buildByBaseSceneFunctionDO();
    if (prepareFunction != null) {
      sceneFunctionDO.setAgentRequired(true);
      sceneFunctionDO.addDependency(
          SceneFunctionDependency.before(prepareFunction.getCode(), PhaseType.PREPARE));
    }
    ...
    sceneFunctionDO.setCode(
        buildAppCode(
            modelSpecBean.getTarget(), modelSpecBean.getScope(), actionSpecBean.getAction()));
  ...
  private SceneFunctionDO buildFromPrepareSpecBean(
      ModelSpecBean modelSpecBean, PrepareSpecBean prepareSpecBean) {
    SceneFunctionDO sceneFunctionDO = buildByBaseSceneFunctionDO();
    ...
    sceneFunctionDO.setCode(
        buildAppCode(prepareSpecBean.getType(), modelSpecBean.getScope(), "install"));
    sceneFunctionDO.setName(sceneFunctionDO.getCode());
  ...
  private SceneFunctionParameterDO buildGlobalParam() {
    ...
    SceneFunctionParameterComponent sceneFunctionParameterComponent =
        new SceneFunctionParameterComponent();
  ...
  private SceneFunctionParameterDO buildByMatcherSpecBean(
      SceneFunctionDO sceneFunctionDO, MatcherSpecBean matcherSpecBean) {
    SceneFunctionParameterDO sceneFunctionParameterDO = new SceneFunctionParameterDO();
    sceneFunctionParameterDO.setIsDelete(false);
  ...
  private SceneFunctionParameterDO buildByFlagSpecBean(
      SceneFunctionDO sceneFunctionDO, FlagSpecBean flagSpecBean) {
    SceneFunctionParameterDO sceneFunctionParameterDO = new SceneFunctionParameterDO();
    sceneFunctionParameterDO.setIsDelete(false);
  ...
  private static String buildAppCode(String target, String scope, String action) {
    ...
    return "chaos" + "." + newTarget + "." + action;
  }

  private String findOrCreateSceneIfNotExist() {
    SceneDO sceneDO =
        sceneRepository.findByCode(CommonConstant.CHAOS_BLADE_SCENE_CODE).orElse(null);
    if (sceneDO == null) {
      sceneDO = new SceneDO();
