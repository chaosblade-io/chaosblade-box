...
public class ApplicationRelationRepository implements IRepository<String, ApplicationRelationDO> {
  ...
  @Autowired private ApplicationRelationMapper applicationRelationMapper;

  public List<ApplicationRelationDO> findRelationByAppIdAndRelationType(
      Long appId, String relationType) {
  ...
  public IPage<ApplicationRelationDO> pageableFindRelationsByAppIdAndRelationType(
      Long appId, String relationType, int page, int size) {
    QueryWrapper<ApplicationRelationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("app_id", appId);
    queryWrapper.eq("relation_type", relationType);
  ...
  public long updateExperimentTaskState(String taskId, StateEnum stateEnum, ResultEnum resultEnum) {
    ...
    return applicationRelationMapper.update(
        applicationRelationDO, new QueryWrapper<>(whereApplicationDO));
  }
  ...
  public int deleteRelationByAppIdAndOuterIdAndRelationType(
      Long appId, String outerId, String relationType) {
    QueryWrapper<ApplicationRelationDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("app_id", appId);
    queryWrapper.eq("outer_id", outerId);
    queryWrapper.eq("relation_type", relationType);
