...
public interface MiniAppTaskManager {
   ...
   * @return
   */
  default ExperimentMiniAppTaskDO initExperimentMiniAppTaskDO(
    ...
    // 如果是异步下发，需要标记一下
    if (null != activityTaskDO.getRunParam()
        && null != activityTaskDO.getRunParam().getArguments()) {
      String async = activityTaskDO.getRunParam().getArguments().getAllArguments().get("async");
      experimentMiniAppTaskDO.setAsync(
          !Strings.isNullOrEmpty(async) && Boolean.parseBoolean(async));
    }
    if (host != null) {
      ...
      experimentMiniAppTaskDO.setAppConfigurationId(
          host.getAppConfigurationId() == null
