...
/** @author haibin */
@Component
public class ActivityRepository implements IRepository<String, ExperimentActivityDO> {

  @Autowired ExperimentActivityMapper experimentActivityMapper;

  @Autowired private MiniAppCodeHelper miniAppCodeHelper;

  @Override
  public Optional<ExperimentActivityDO> findById(String s) {
    return Optional.ofNullable(findLatestActivityByActivityId(s));
  }
  ...
  public List<ExperimentActivityDO> find(ExperimentActivityQuery query) {
    List<ExperimentActivityDO> experimentActivityDOS =
        experimentActivityMapper.selectList(buildExperimentActivityQueryWrapper(query));
  ...
  public List<ExperimentActivityDO> findByExperimentIdAndPhase(
      String experimentId, PhaseType phase) {
    ExperimentActivityQuery experimentActivityQuery = new ExperimentActivityQuery();
    experimentActivityQuery.setExperimentId(experimentId);
  ...
  public int deleteByNoFlowId(String experimentId) {
    ...
    return experimentActivityMapper.update(
        experimentActivityDO,
        new QueryWrapper<ExperimentActivityDO>()
            .eq("experiment_id", experimentId)
            .isNull("flow_id"));
  }

  private Wrapper<ExperimentActivityDO> buildExperimentActivityQueryWrapper(
      ExperimentActivityQuery query) {
    QueryWrapper<ExperimentActivityDO> queryWrapper = new QueryWrapper<>();
