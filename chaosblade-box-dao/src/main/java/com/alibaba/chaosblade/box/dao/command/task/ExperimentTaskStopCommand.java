...
/** @author haibin */
@Component
public class ExperimentTaskStopCommand
    extends SpringBeanCommand<ExperimentTaskStopRequest, Response<Void>> {
  ...
  public Response<Void> execute(ExperimentTaskStopRequest experimentTaskStopRequest) {
    ...
    ChaosError ChaosError =
        determineExperimentTaskCanStop(experimentTaskStopRequest, experimentTaskDO);
    if (ChaosError != null) {
  ...
  private void recoverTasks(
      ExperimentTaskStopRequest experimentTaskStopRequest, ExperimentTaskDO experimentTaskDO) {
    ChaosUser chaosUser = experimentTaskStopRequest.getUser();
    ActivityTaskDO activityTaskDO =
        activityTaskRepository.findFirstRecoveryActivityTask(experimentTaskDO.getTaskId());
    if (activityTaskDO != null) {
      ExperimentExecuteContext experimentExecuteContext =
          new ExperimentExecuteContext(
              ...
              experimentTaskStopRequest.getUser(),
              activityTaskDO,
  ...
  private void recordIfNeed(
      ExperimentTaskStopRequest experimentTaskStopRequest, ExperimentTaskDO experimentTaskDO) {}

  private ChaosError determineExperimentTaskCanStop(
      ExperimentTaskStopRequest experimentTaskStopRequest, ExperimentTaskDO experimentTaskDO) {
    // 检查当前节点是否注入中
    // 如果有正在注入中的任务
