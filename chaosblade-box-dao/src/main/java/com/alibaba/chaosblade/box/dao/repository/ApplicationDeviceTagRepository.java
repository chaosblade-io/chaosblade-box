...
public class ApplicationDeviceTagRepository implements IRepository<Long, ApplicationDeviceTagDO> {
  ...
  @Autowired private ApplicationDeviceTagMapper applicationDeviceTagMapper;
  ...
  public boolean deleteDeviceTag(
      String configurationId, String appId, String groupName, String userId) {
    QueryWrapper<ApplicationDeviceTagDO> queryWrapper = new QueryWrapper<>();
  ...
  public List<ApplicationDeviceTagDO> getTagsByAppIdAndConfigurationIdAndUserId(
      Long appId, String configurationId, String userId) {
    QueryWrapper<ApplicationDeviceTagDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("configuration_id", configurationId);
    queryWrapper.eq("app_id", appId);
    queryWrapper.eq("user_id", userId);
  ...
  public int countByAppIdAndPrivateIpAndUserIdAndTagName(
      Long appId, String groupName, String configurationId, String userId, String tagName) {
    QueryWrapper<ApplicationDeviceTagDO> queryWrapper = new QueryWrapper<>();
    queryWrapper.eq("configuration_id", configurationId);
    queryWrapper.eq("app_id", appId);
    queryWrapper.eq("group_name", groupName);
  ...
  public List<String> listApplicationGroupTags(
      Long appId, List<String> groupNames, String userId, String key) {
    QueryWrapper<ApplicationDeviceTagDO> queryWrapper = new QueryWrapper<>();
    if (null != appId) {
      queryWrapper.eq("app_id", appId);
    }
    ...
    return applicationDeviceTagMapper.selectList(queryWrapper).stream()
        .map(ApplicationDeviceTagDO::getTagName)
        .collect(Collectors.toList());
  }

  public List<String> listApplicationGroupTagsByConfigurationIds(
      List<String> configurationIds, String userId, String key) {
    QueryWrapper<ApplicationDeviceTagDO> queryWrapper = new QueryWrapper<>();
