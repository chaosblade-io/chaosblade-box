...
/** @author sunju */
@Repository
public class DeviceRepository extends ServiceImpl<DeviceMapper, DeviceDO>
    implements IRepository<String, DeviceDO> {

  @Resource @Getter private DeviceMapper deviceMapper;

  public PageableResponse<DeviceDO> getAliveDevices(
    ...
    IPage<DeviceDO> result =
        deviceMapper.selectPage(
            MyBatisUtil.getPage(pageNo, pageSize), buildQueryWrapper(pageableQueryWrapper));
    if (null == result) {
  ...
  private QueryWrapper<DeviceDO> buildQueryWrapper(
      PageableQueryWrapper<CloudDeviceQuery> pageableQueryWrapper) {
    CloudDeviceQuery query = pageableQueryWrapper.query();
    if (query.getScopeType() == ScopeTypeEnum.K8s) {
  ...
  private QueryWrapper<DeviceDO> buildQueryWrapper(CloudDeviceQuery query) {
    ...
    if (!Strings.isNullOrEmpty(query.getKey())) {
      if (query.getScopeType() == ScopeTypeEnum.K8s) {
        queryWrapper.nested(
            nestedWrapper ->
                nestedWrapper
                    .or(wrapper -> wrapper.like("cluster_name", query.getKey()))
                    .or(wrapper -> wrapper.like("device_name", query.getKey())));
      } else {
        queryWrapper.nested(
            nestedWrapper ->
                nestedWrapper
                    .or(wrapper -> wrapper.like("public_ip", query.getKey()))
    ...
    if (query.getScopeType() != null) {
      queryWrapper.in(
          "install_mode",
          query.getScopeType().getAsList().stream()
              .map(InstallMode::name)
              .collect(Collectors.toList()));
    }
    queryWrapper
        .in("device_type", DeviceType.HOST_POD.getType(), DeviceType.HOST.getType())
        .eq("status", 2)
        .isNotNull("version");
  ...
  public DeviceDO findByConfigurationIdAndUserId(String userId, String configurationId) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
    queryWrapper.eq("configuration_id", configurationId);
  ...
  public DeviceDO findByConfigurationIdAndUserIdNamespace(
      String userId, String configurationId, String namespace) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
    queryWrapper.eq("configuration_id", configurationId);
    queryWrapper.eq("user_id", userId);
  ...
  public DeviceDO findActiveByPrivateIp(String ip) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
    queryWrapper.eq("private_ip", ip).eq("status", DeviceStatus.ONLINE.getStatus()).last("limit 1");
    return deviceMapper.selectOne(queryWrapper);
  }
  ...
  public List<String> getOnlineClusterIdsByUserId(String userId, String namespace) {
    ...
    return deviceMapper.selectList(queryWrapper).stream()
        .map(DeviceDO::getClusterId)
        .filter((Predicate<String>) input -> !Strings.isNullOrEmpty(input))
        .collect(Collectors.toList());
  }
  ...
  public DeviceDO getDeviceByDeviceId(
      String uid, String namespace, String deviceId, Integer deviceType) {
    QueryWrapper<DeviceDO> queryWrapper = new QueryWrapper<DeviceDO>();
    queryWrapper.eq("user_id", uid);
    queryWrapper.eq("namespace", namespace);
