...
public class MiniAppLitmusChaosInvokeInterceptor extends BaseMiniAppInvokeInterceptor {
  ...
  public boolean preHandle(
    ...
    if (isNotLitmusChaosScene(miniAppInvokeContext)) {
      return true;
    }
  ...
  public void afterHandle(
    ...
    if (chaosAppResponse instanceof ChaosBladeAppResponse) {
      ...
      createExpRecordWhenAttackOrPreparePhase(
          miniAppInvokeContext, chaosBladeAppResponse, phaseType);
    }
  }
  ...
  private boolean setExpUidWhenRecoveryPhase(
    ...
    if (PhaseType.RECOVER.equals(activityTaskDO.getPhase())) {
      String chaosBladeExpId =
          getAttackExpId(experimentTaskId, miniAppInvokeContext.getHost(), activityTaskDO);
  ...
  private String getAttackExpId(String experimentTaskId, Host host, ActivityTaskDO activityTaskDO) {
    ...
    if (activityTaskDO.getAttackActivityTaskId() != null) {
      ...
      chaosBladeExpUidDO =
          queryExpUidDOIfNotfoundAttackActivityTaskId(experimentTaskId, host, activityTaskDO);
    }
    return Optional.ofNullable(chaosBladeExpUidDO).map(ChaosBladeExpUidDO::getExpUid).orElse(null);
  }

  private ChaosBladeExpUidDO queryExpUidIfFoundAttackActivityTaskId(
  ...
  private ChaosBladeExpUidDO queryExpUidDOIfNotfoundAttackActivityTaskId(
    ...
    if (MiniAppUtils.isChaosUninstall(executableAppCode)) {
      ...
      chaosBladeExpUidDO =
          chaosBladeExpUidRepository.findLastByExperimentTaskIdAndHostAndAppCodeAndNotExpired(
              experimentTaskId,
              host.getIp(),
              MiniAppUtils.getAttackCodeByRecoveryCode(executableAppCode));
    }
    return chaosBladeExpUidDO;
  }

  private void createExpRecordWhenAttackOrPreparePhase(
      MiniAppInvokeContext miniAppInvokeContext,
  ...
  private void updateExpRecordWhenRecoveryPhase(
      MiniAppInvokeContext miniAppInvokeContext,
      ChaosBladeAppResponse chaosAppResponse,
      PhaseType phaseType) {
    if (PhaseType.RECOVER.equals(phaseType)) {
      String attackExpId =
          miniAppInvokeContext.getTempData().getAsString(ChaosBladeAppInvoker.CHAOS_BLADE_EXP_ID);
      if (attackExpId != null) {
        ChaosBladeExpUidDO chaosBladeExpUidDO;
        ActivityTaskDO activityTaskDO =
  ...
  private void createChaosBladeExpRecord(MiniAppInvokeContext miniAppInvokeContext, String expId) {
    ...
    ActivityTaskDO activityTaskDO =
        miniAppInvokeContext.getActivityInvokeContext().getContextData().getActivityTaskDO();
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        buildChaosBladeExpUidDO(
            expId, appCode, experimentTaskId, host, appExecutionId, activityTaskId, activityTaskDO);
  ...
  private ChaosBladeExpUidDO buildChaosBladeExpUidDO(
      String expId,
      String appCode,
      String experimentTaskId,
      Host host,
    ...
    chaosBladeExpUidDO.addAttribute(
        ChaosBladeExpUidDO.ATTRIBUTE_ACTIVITY_NAME, activityTaskDO.getActivityName());
    return chaosBladeExpUidDO;
  }

  private ChaosBladeExpUidDO getChaosBladeExp(String activityTaskId, Host host) {
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        chaosBladeExpUidRepository.findLastByActivityTaskIdAndHost(activityTaskId, host.getIp());
