...
@Component
public class ExperimentGuardInstanceRepository
    implements IRepository<String, ExperimentGuardInstanceDO> {

  @Autowired private ExperimentGuardInstanceMapper experimentGuardInstanceMapper;
  ...
  public boolean updateRunningExperimentGuardInstanceDO(
    ...
    QueryWrapper<ExperimentGuardInstanceDO> experimentGuardInstanceDOQueryWrapper =
        new QueryWrapper<>();
    experimentGuardInstanceDOQueryWrapper.eq(
        "instance_id", experimentGuardInstanceDO.getInstanceId());
    experimentGuardInstanceDOQueryWrapper.notIn(
        "state", GuardRunState.FINISHED, GuardRunState.TRIGGERED);
    return experimentGuardInstanceMapper.update(
            experimentGuardInstanceDO, experimentGuardInstanceDOQueryWrapper)
        > 0;
  }
