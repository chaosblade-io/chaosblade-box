...
/** @author haibin */
public class QuartzSchedulerService extends BaseSchedulerService
  ...
  protected void internalAddSchedulerJob(SchedulerJobDO schedulerJob) throws SchedulerException {
    ...
    addSchedulerJob(
        loadClassFromSpring(schedulerJob),
        schedulerJob.getJobId(),
        applicationContext,
        schedulerJob.getCronExpression(),
        schedulerJob.getStartTime(),
        10,
        jobDataMap);
  }
      ...
      return applicationContext
          .getBean(
              Thread.currentThread()
                  ...
                  .loadClass(schedulerJobDO.getSchedulerBeanClass()))
          .getClass();
    } catch (Exception no) {
  ...
  private void initPersistenceSchedulerJob(ApplicationContext applicationContext) {
    ...
    for (SchedulerJobDO schedulerJob : schedulerJobDOS) {
      ...
      if (clazz != null) {
        if (registeredCache.contains(schedulerJob.getJobId())) {
          continue;
        }
  ...
  private void initRamSchedulerJob(ApplicationContext applicationContext) {
    ...
    for (Map.Entry<String, Object> beanMap : beans.entrySet()) {
      if (registeredCache.contains(beanMap.getKey())) {
        continue;
      }
      registeredCache.add(beanMap.getKey());
      Class beanClass = beanMap.getValue().getClass();
      SchedulerJob schedulerJob = AnnotationUtils.findAnnotation(beanClass, SchedulerJob.class);
  ...
  private void addSchedulerJob(
    ...
    try {
      ...
      scheduler.scheduleJob(jobDetail, trigger);
      log.info("add scheduler job success,job class:{},name:{}", beanClass.getName(), name);
    } catch (Exception ex) {
      if (ex instanceof ObjectAlreadyExistsException) {
        log.error(
            "init scheduler job failed,beanClass:"
                + beanClass.getName()
                + ",name:"
                + name
                + ",exist job");
  ...
  private JobDetail initJobDetail(
      Class beanClass, String name, ApplicationContext applicationContext, JobDataMap jobDataMap) {
    JobDetailFactoryBean jobDetailFactoryBean =
        QuartzConfig.createJobDetail(beanClass, name, "DEFAULT");
  ...
  private Trigger initTrigger(
      JobDetail jobDetail, String name, String cronExpression, Date startTime, long startDelay)
      throws ParseException {
    Trigger trigger = null;
    if (cronExpression != null) {
      CronTriggerFactoryBean cronTriggerFactoryBean =
          QuartzConfig.createCronTrigger(jobDetail, startTime, cronExpression, name + "_trigger");
      cronTriggerFactoryBean.setStartDelay(startDelay);
      cronTriggerFactoryBean.afterPropertiesSet();
      trigger = cronTriggerFactoryBean.getObject();
    } else {
      SimpleTriggerFactoryBean simpleTriggerFactoryBean =
          QuartzConfig.createSimpleTrigger(jobDetail, startTime, 0, name + "_trigger");
