...
public class MiniAppInvocationRecorder implements IRecord<MiniAppInvocationRecord> {
  ...
  /**
   * 用来记录远程调用,日志格式: 到cost为止必须有,experimentTaskId后面不一定有
  ...
  public void record(MiniAppInvocationRecord recordObject) {
    try {
      ...
      logger.info(
          "{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}",
          recordObject.getTraceId(),
          recordObject.getAppCode(),
          recordObject.getMiniAppType(),
          recordObject.isSuccess(),
          JSON.toJSONString(recordObject.getRequest()),
          JSON.toJSONString(recordObject.getResponse()),
          recordObject.getCost(),
          recordObject.getIp(),
          recordObject.getConfigurationId(),
          recordObject.getExperimentTaskId(),
          recordObject.getActivityTaskId(),
          recordObject.getMiniAppTaskId());
    } catch (Exception ex) {
  ...
  public void log(String message, boolean isPublic, Throwable throwable) {
    ...
    if (miniAppInvokeFlowThreadLocalContext == null) {
      return;
    }
    if (throwable == null) {
      log(
          miniAppInvokeFlowThreadLocalContext.getRequestId(),
          isPublic,
          ...
          miniAppInvokeFlowThreadLocalContext.getAppCode(),
          message);
    } else {
      log(
          miniAppInvokeFlowThreadLocalContext.getRequestId(),
  ...
  public void log(
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO, String message, boolean isPublic) {
    log(
        ...
        experimentMiniAppTaskDO.getAppCode(),
        message);
  }
