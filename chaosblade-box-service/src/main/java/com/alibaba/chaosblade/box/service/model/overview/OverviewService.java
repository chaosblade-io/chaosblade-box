...
public class OverviewService {
  ...
  @Resource private CloudManualMapper cloudManualMapper;
  ...
  @Autowired private OverviewProductLoadFactory overviewProductLoadFactory;

  @Autowired private ExperimentTaskRepository experimentTaskRepository;

  @Autowired private SceneFunctionNameParser sceneFunctionNameParser;

  private List<OverviewScene> defaultUserScene =
      new ArrayList<OverviewScene>() {
        {
          ...
          add(
              new OverviewScene(
                  "主机内Java抛出自定义异常", "chaos.jvm.throwCustomException", 3, "JAVA应用", "抛异常"));
          add(new OverviewScene("主机内Java脚本", "chaos.jvm.script", 4, "JAVA应用", "自定义故障"));
        }
      };

  private List<OverviewScene> defaultUserSceneEn =
      new ArrayList<OverviewScene>() {
        {
          add(
              new OverviewScene(
          ...
          add(
              new OverviewScene(
                  "HOST JVM THROWCUSTOMEXCEPTION",
                  "chaos.jvm.throwCustomException",
                  3,
                  "JAVA In-process",
                  "Throw Exception"));
          add(
              new OverviewScene(
                  "HOST JVM SCRIPT", "chaos.jvm.script", 4, "JAVA In-process", "custom fault"));
        }
      };

  private Cache<String, List<OverviewScene>> sceneCache =
  ...
  public Response<OverviewExperimentTask> getUserExperimentDayCount(ChaosUser chaosUser) {
    ...
    cloudManualMapper.countExperimentTaskByDate(chaosUser.getUserId(), monthDate).stream()
        .map(
            stringObjectMap -> {
              OverviewExperimentTaskDayCount experimentTaskCount =
                  new OverviewExperimentTaskDayCount();
              try {
                experimentTaskCount.setTime(fo.parse(stringObjectMap.get("date").toString()));
              } catch (ParseException e) {
              }
              experimentTaskCount.setTotalCount(
                  Integer.parseInt(stringObjectMap.get("total").toString()));
    ...
    overviewExperimentTask.setExperimentTaskDayCountList(
        result.values().stream()
            .sorted(Comparator.comparing(OverviewExperimentTaskDayCount::getTime))
            .collect(Collectors.toList()));
    ExperimentTaskDO experimentTaskDO =
        experimentTaskRepository.findUserLastFinishedExperimentTask(chaosUser.getUserId());
    overviewExperimentTask.setLastExperimentTaskTime(
        null != experimentTaskDO ? experimentTaskDO.getGmtCreate() : null);
  ...
  public Response<OverviewAgent> getUserAgentCount(ChaosUser chaosUser) {
    ...
    OverviewAgentInfo cluster =
        new OverviewAgentInfo(
            clusterTotal, clusterTotalOnline, clusterTotalNormal, clusterTotalError);
  ...
  public Response<List<OverviewScene>> getUserScene(ChaosUser chaosUser, BaseRequest baseRequest) {
    try {
      String lang =
          Strings.isNullOrEmpty(baseRequest.getLang())
              ? ChaosConstant.LANGUAGE_ZH
              : baseRequest.getLang();

      List<OverviewScene> result =
          cloudManualMapper.countAppCodeByUserId(chaosUser.getUserId(), getStartTime()).stream()
              .sorted(
                  (o1, o2) ->
                      Integer.parseInt(o2.get("total").toString())
                          - Integer.parseInt(o1.get("total").toString()))
              .limit(4)
              .map(
                  stringObjectMap -> {
                    ...
                    overviewScene.setName(
                        sceneFunctionNameParser.parseFunctionName(
      ...
      if (Strings.isNullOrEmpty(lang) || lang.equals(ChaosConstant.LANGUAGE_ZH)) {
        return Response.okWithData(
            CollectionUtil.isNullOrEmpty(result) ? defaultUserScene : result);
      } else {
        return Response.okWithData(
            CollectionUtil.isNullOrEmpty(result) ? defaultUserSceneEn : result);
      }
    } catch (Exception e) {
      return Response.failedWith(
          ChaosError.withCode(CommonErrorCode.B_OVERVIEW_USER_SCENE_QUERY_ERROR));
    }
  }

  public Response<PageQueryResponse<OverviewExpertise>> getUserExpertise(
      ChaosUser chaosUser, OverviewRequest overviewRequest) {
    IPage<ExpertiseDO> expertiseDOIPage =
        expertiseRepository.userFindPageable(
            chaosUser, null, null, null, overviewRequest.getPage(), overviewRequest.getSize());
    ...
    pageQueryResponse.setContent(
        expertiseDOIPage.getRecords().stream()
            .map(
                new Function<ExpertiseDO, OverviewExpertise>() {
                  @Override
  ...
  private Date getStartTime() {
    Date monthDate = DateUtils.truncate(DateUtils.addMonths(new Date(), -2), Calendar.MONTH);
    return monthDate;
  }
