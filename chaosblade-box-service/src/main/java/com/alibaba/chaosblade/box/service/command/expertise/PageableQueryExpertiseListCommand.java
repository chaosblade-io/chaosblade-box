...
public class PageableQueryExpertiseListCommand
    extends SpringBeanCommand<
        ExpertiseSearchRequest, Response<PageQueryResponse<AdminExpertiseView>>> {
  ...
  @Override
  public Response<PageQueryResponse<AdminExpertiseView>> execute(
      ExpertiseSearchRequest expertiseSearchRequest) {
    IPage<ExpertiseDO> expertiseDOIPage =
        expertiseRepository.findPageable(
    ...
    pageQueryResponse.setContent(
        expertiseDOIPage.getRecords().stream()
            .map(
                new Function<ExpertiseDO, AdminExpertiseView>() {
                  @Override
                  public AdminExpertiseView apply(ExpertiseDO expertiseDO) {
                    AdminExpertiseView adminExpertiseView = new AdminExpertiseView();
                    adminExpertiseView.setCreator(
                        userService.getUserByUserId(expertiseDO.getUserId()));
                    ...
                    adminExpertiseView.setExperimentCount(
                        countExperimentByExpertiseId(expertiseDO.getExpertiseId()));
                    adminExpertiseView.setTags(
  ...
  private Integer countExperimentByExpertiseId(String expertiseId) {
    return experimentRelationRepository
        .findByOuterIdAndRelationType(expertiseId, ExperimentRelationDO.RELATION_TYPE_EXPERTISE)
        .size();
  }
}
