...
public class PrivateScope implements IChaosDomain {
  ...
  @Autowired private PrivateCloudDeviceRepository privateCloudDeviceRepository;
  ...
  public DeviceDO register(RegisteredCallbackRequest registeredCallbackRequest) {
    String configurationId =
        generatorDeviceConfigurationId(
            ...
            registeredCallbackRequest.getNamespace(),
  ...
  private DeviceDO saveDevice(
      RegisteredCallbackRequest registeredCallbackRequest, String configurationId) {
    DeviceDO oldDeviceDO = constructDeviceDO(registeredCallbackRequest, configurationId);
    if (oldDeviceDO.getId() == null) {
      ChaosPreconditions.checkPersistence(
          privateCloudDeviceRepository.getCommon().add(oldDeviceDO), "add device failed");
    } else {
      ChaosPreconditions.checkPersistence(
          privateCloudDeviceRepository.getCommon().update(oldDeviceDO), "add device failed");
    }
    return oldDeviceDO;
  }

  private DeviceDO constructDeviceDO(
      RegisteredCallbackRequest registeredCallbackRequest, String configurationId) {
  ...
  public Boolean updateHeartBeatTime(HeartBeatCallbackRequest heartBeatCallbackRequest) {
    ...
    boolean success =
        privateCloudDeviceRepository.updateHeartBeatTimeByDeviceDO(
            this.configurationId,
            heartBeatCallbackRequest.getVersion(),
            heartBeatCallbackRequest.getChaosVersion(),
            heartBeatTime,
            heartBeatCallbackRequest.getIp());
    if (success) {
  ...
  public static String generatorDeviceConfigurationId(
      String userId,
      String namespace,
      String vpcId,
      String hostIp,
  ...
  public static String generatorKubernetesConfigurationId(
      String userId, String namespace, String vpcId, String uid) {
