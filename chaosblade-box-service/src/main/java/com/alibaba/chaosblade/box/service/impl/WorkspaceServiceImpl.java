...
public class WorkspaceServiceImpl implements WorkspaceService {
  private static final String MY_WORKSPACE_NAME = "我的空间";

  @Resource private Trackers trackers;

  @Autowired private CommandBus commandBus;

  @Resource private WorkspaceRelationRepository workspaceRelationRepository;
  ...
  @Override
  public Response<ExperimentStat> getGeneralWorkSpaceStatInfo(
  ...
  public WorkspaceExperimentPageableQueryResponse pageableGeneralExperiments(
    ...
    if (CollectionUtils.isEmpty(experimentIds)) {
      PageableResponse<UserExperiment> pageableResponse =
          PageableResponse.of(page, size, Collections.emptyList(), 0L, 0L);
    ...
    List<UserExperiment> contentList =
        experimentDOIPage.getRecords().stream()
            .map(experimentDO -> convert(experimentDO, permission, user))
            .collect(Collectors.toList());
    PageableResponse<UserExperiment> pageableResponse =
        PageableResponse.of(
            experimentDOIPage.getCurrent(),
            experimentDOIPage.getSize(),
            contentList,
            experimentDOIPage.getPages(),
            experimentDOIPage.getTotal());
  ...
  @Override
  public PermissionResult getWorkspaceExperimentTaskPermission(
  ...
  public PermissionResult getWorkspacePermission(ChaosUser user, String experimentId) {
    ...
    List<WorkspaceRelationDO> userRelationDOList =
        workspaceRelationRepository.find(userWorkspaceRelationQuery);

    if (CollectionUtils.isEmpty(userRelationDOList)) {
      return PermissionResult.of(PermissionTypes.NONE);
    }

    List<String> workspaceIdList =
        userRelationDOList.stream()
            .map(WorkspaceRelationDO::getWorkspaceId)
            .collect(Collectors.toList());
    WorkspaceRelationQuery workspaceRelationQuery = new WorkspaceRelationQuery();
    ...
    Set<String> workspaceIdSet =
        relationDOList.stream().map(r -> r.getWorkspaceId()).collect(Collectors.toSet());

    Optional<Integer> first =
        userRelationDOList.stream()
            .filter(r -> workspaceIdSet.contains(r.getWorkspaceId()))
            .map(WorkspaceRelationDO::getPermission)
  ...
  private UserExperiment convert(
      ExperimentDO experimentDO, Integer permission, ChaosUser chaosUser) {
    UserExperiment userExperiment = experimentDOTOUserExperimentConverter.convert(experimentDO);
    if (Objects.equals(
        chaosUser.getCurrentUserId(), experimentDO.getCreator().getCurrentUserId())) {
  ...
  @Override
  public AddExperimentResponse addWorkspaceExperiment(
      ChaosUser user, WorkspaceExperimentRequest request) {
    return ChangeLogExecutor.executeWithChangeLog(
        new Supplier<AddExperimentResponse>() {
          @Override
          ...
          public void accept(AddExperimentResponse addExperimentResponse) {
            if (addExperimentResponse.getSuccess()) {
              trackers.track(
                  ...
                  request.getWorkspaceExperimentList().stream()
                      .map(WorkspaceExperiment::getExperimentId)
  ...
  @Override
  public void addWorkspaceExperiments(
  ...
  public List<WorkspaceShortInfo> searchWorkspaces(WorkspaceRelationQuery workspaceRelationQuery) {
    ...
    query.setWorkspaceIds(
        relations.stream().map(WorkspaceRelationDO::getWorkspaceId).collect(Collectors.toList()));
    List<WorkspaceDO> workspaces = workspaceRepository.find(query);
    if (CollectionUtil.isNullOrEmpty(workspaces)) {
      return Lists.newArrayList();
    }
    Map<String, WorkspaceDO> mapping =
        workspaces.stream()
            .collect(
                Collectors.toMap(WorkspaceDO::getWorkspaceId, Function.identity(), (k1, k2) -> k2));
  ...
  @Override
  public List<WorkspaceBaseInfo> getWorkspaceByExperimentId(
      ChaosUser user, WorkspaceQueryRequest request) {
    WorkspaceRelationQuery workspaceRelationQuery = new WorkspaceRelationQuery();
    ...
    WorkspaceQuery workspaceQuery =
        WorkspaceQuery.builder()
            .type(WorkspaceTypes.GENERAL)
            .workspaceIds(
    ...
    List<WorkspaceBaseInfo> workspaceBaseInfoList =
        workspaceDOList.stream()
            .map(
                r ->
                    WorkspaceBaseInfo.builder()
                        .name(r.getName())
