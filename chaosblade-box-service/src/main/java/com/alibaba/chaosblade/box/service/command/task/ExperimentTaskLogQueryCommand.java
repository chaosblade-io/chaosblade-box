...
public class ExperimentTaskLogQueryCommand extends BasePoolCommand<Response<ExperimentTaskLog>> {
  ...
  public Response<ExperimentTaskLog> execute() {
    ...
    Response<ExperimentTask> experimentTaskResponse =
        getPool()
            .syncRun(
                ExperimentTaskQueryCommand.class, new ExperimentTaskQueryRequest(experimentTaskId));
    if (!experimentTaskResponse.isSuccess()) {
      return Response.failedWith(experimentTaskResponse.getError());
    }
