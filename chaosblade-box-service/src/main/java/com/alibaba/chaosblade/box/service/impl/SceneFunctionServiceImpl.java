...
public class SceneFunctionServiceImpl implements SceneFunctionService {
  ...
  @Resource private SceneFunctionRelationRepository sceneFunctionRelationRepository;
  ...
  @Resource private SceneService sceneService;
  ...
  private final Executor refreshCacheWorker =
  ...
  public Boolean queryIsFissionSceneFunction(String functionId) {
    SceneFunctionRelationDO relationDO =
        sceneFunctionRelationRepository.findByFunctionId(functionId);
    if (null != relationDO) {
  ...
  private String createSceneFunction(
      String sceneId, SceneFunctionCreateRequest sceneFunctionCreateRequest) throws ChaosException {
  ...
  private void addSceneFunctionAuthorizedRecord(ChaosUser user, SceneFunctionDO function)
      throws ChaosException {
    this.addSceneFunctionAuthorizedRecord(user.getUserId(), function);
  }

  private void addSceneFunctionAuthorizedRecord(String userId, SceneFunctionDO function)
      throws ChaosException {
    ...
    if (!result) {
      throw new ChaosException(
          CommonErrorCode.B_CREATE_MINIAPP_FAILED, "Add scene authorized record failed.");
    }
  }

  private void addSceneFunctionRelation(SceneFunctionDO parent, SceneFunctionDO child)
      throws ChaosException {
    ...
    boolean result =
        RetryUtil.retryIfReturnFalse(() -> sceneFunctionRelationRepository.add(relationDO));
    if (!result) {
  ...
  @Override
  public PageableResponse<SceneFunctionDO> querySceneFunctions(
    ...
    List<SceneAuthorizedDO> sceneAuthorizedResponse =
        sceneAuthorizedRepository.getAuthorizedRecordsGroupBy(sceneAuthorizedQueryRequest);
    if (CollectionUtil.isNullOrEmpty(sceneAuthorizedResponse)) {
      return PageableResponse.of(pageNo, pageSize);
    }
    List<String> functionIds =
        sceneAuthorizedResponse.stream()
            .map(SceneAuthorizedDO::getFunctionId)
            .collect(Collectors.toList());

    queryRequest.setFunctionIds(functionIds);
    PageableResponse<SceneFunctionDO> sceneFunctionDOPageableResponse =
        sceneFunctionRepository.findByPage(
            PageableQueryWrapper.of(queryRequest).pageNumber(pageNo).pageSize(pageSize));
    ...
    // 执行用户过滤
    List<SceneFunctionDO> sceneFunctionDOS =
        sceneFunctionDOPageableResponse.getData().stream()
            .filter(
