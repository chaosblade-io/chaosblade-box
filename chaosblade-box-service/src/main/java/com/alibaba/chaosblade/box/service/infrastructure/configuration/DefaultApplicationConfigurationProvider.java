...
public class DefaultApplicationConfigurationProvider implements ApplicationConfigurationProvider {
  ...
  @Autowired private DefaultApplicationConfigurationLoader defaultApplicationConfigurationLoader;
  ...
  public List<ApplicationConfiguration> provideApplicationConfigurations(Long appId) {
    ...
    List<ApplicationConfiguration> defaultApplicationConfigurations =
        defaultApplicationConfigurationLoader.loadConfigurations();
    List<ApplicationConfiguration> applicationConfigurations =
        applicationConfigurationRepository.findByApplicationId(appId).stream()
            .map(this::convertApplicationConfiguration)
            .collect(Collectors.toList());
    return mergeConfigurations(applicationConfigurations, defaultApplicationConfigurations).stream()
        .filter(
            applicationConfiguration ->
                applicationConfiguration.getAppType() == null
                    || applicationConfiguration.getAppType().equals(applicationDO.getAppType()))
        .collect(Collectors.toList());
  }
  ...
  private List<ApplicationConfiguration> mergeConfigurations(
      List<ApplicationConfiguration> applicationConfigurations,
      List<ApplicationConfiguration> defaultApplicationConfigurations) {
    ...
    Map<String, ApplicationConfiguration> defaultAliasToConfigs =
        defaultApplicationConfigurations.stream()
            .collect(Collectors.toMap(ApplicationConfiguration::getAlias, e -> e));
    Map<String, ApplicationConfiguration> aliasToApplicationConfigurations =
        applicationConfigurations.stream()
            .collect(Collectors.toMap(ApplicationConfiguration::getAlias, e -> e));
    for (Map.Entry<String, ApplicationConfiguration> stringApplicationConfigurationEntry :
        aliasToApplicationConfigurations.entrySet()) {
      // 合并原有的配置
      mergeConfiguration(
          stringApplicationConfigurationEntry.getValue(),
    ...
    for (Map.Entry<String, ApplicationConfiguration> stringApplicationConfigurationEntry :
        defaultAliasToConfigs.entrySet()) {
      // 补上新增的演练配置
      if (aliasToApplicationConfigurations.get(stringApplicationConfigurationEntry.getKey())
          == null) {
        result.add(stringApplicationConfigurationEntry.getValue());
      }
    }
