...
public class ApplicationServiceImpl implements ApplicationService {
  ...
  @Autowired private UserApplicationSummaryConvertor userApplicationSummaryConverter;

  @Autowired private ApplicationBasicInfoConvertor applicationBasicInfoConverter;

  @Autowired private ApplicationBaseExperimentTaskConvertor applicationBaseExperimentTaskConverter;

  @Autowired private ApplicationRelationRepository applicationRelationRepository;

  @Autowired private ApplicationGroupRepository applicationGroupRepository;
  ...
  @Autowired private ApplicationConfigurator applicationConfigurator;

  @Autowired private ApplicationDeviceTagRepository applicationDeviceTagRepository;

  @Autowired private TagManager tagManager;

  @Autowired private ExperimentApplicationFinder experimentApplicationFinder;
  ...
  public Response<PageableResponse<UserApplicationSummary>> getUserApplicationSummaries(
    ...
    IPage<ApplicationDO> applicationDOIPage =
        applicationRepository.findApplicationsByUserId(
            userId,
    ...
    PageableResponse<UserApplicationSummary> pageableResponse =
        PageableResponse.of(
            applicationDOIPage.getCurrent(),
            applicationDOIPage.getSize(),
            ExecutorUtil.execute(
                () ->
                    applicationDOIPage.getRecords().stream()
                        .map(
                            (Function<ApplicationDO, Callable<UserApplicationSummary>>)
                                applicationDO ->
                                    () -> userApplicationSummaryConverter.convert(applicationDO))
  ...
  public Response<List<UserApplicationSummary>> searchApplicationsForSummary(
    ...
    List<UserApplicationSummary> userApplicationSummaries =
        applicationRepository
            .fuzzySearchApplication(
                userApplicationSearchRequest.getUserId(),
                    userApplicationSearchRequest.getNamespace(),
                userApplicationSearchRequest.getKey(),
                    userApplicationSearchRequest.isFilterDisabled())
            .stream()
            .map(applicationDO -> userApplicationSummaryConverter.convert(applicationDO))
            .collect(Collectors.toList());
    return Response.okWithData(userApplicationSummaries);
  }

  @Override
  public Response<ApplicationBasicInfo> getApplicationBasic(
      UserApplicationQueryRequest userApplicationQueryRequest) throws Throwable {
    ApplicationDO applicationDO =
        applicationRepository.findByAppIdOrAppName(
            userApplicationQueryRequest.getAppId(),
            userApplicationQueryRequest.getAppName(),
            userApplicationQueryRequest.getUserId(),
  ...
  public Response<PageableResponse<BaseExperimentTask>> getApplicationExperimentTasks(
    ...
    IPage<ApplicationRelationDO> relationDOIPage =
        applicationRelationRepository.pageableFindRelationsByAppIdAndRelationType(
            userApplicationQueryRequest.getAppId(),
            ApplicationRelationDO.RELATION_EXPERIMENT_TASK,
            userApplicationQueryRequest.getPage(),
            userApplicationQueryRequest.getSize());
    List<BaseExperimentTask> baseExperimentTasks =
        relationDOIPage.getRecords().stream()
            .map(e -> applicationBaseExperimentTaskConverter.convert(e))
            .filter(Objects::nonNull)
            .collect(Collectors.toList());
    PageableResponse<BaseExperimentTask> pageableResponse =
        PageableResponse.of(
            relationDOIPage.getCurrent(),
            relationDOIPage.getSize(),
            baseExperimentTasks,
            relationDOIPage.getPages(),
            relationDOIPage.getTotal());
    return Response.okWithData(pageableResponse);
  }
  ...
  public Response<Integer> stopExperimentsByApplication(
    ...
    List<ApplicationRelationDO> applicationRelationDOS =
        applicationRelationRepository.findRunningTaskByAppId(applicationDO.getId());
    applicationRelationDOS.forEach(
        applicationRelationDO -> {
  ...
  public Response<PageableResponse<ExperimentScope>> searchApplicationHosts(
    ...
    ApplicationDeviceQuery applicationDeviceQuery = buildQuery(applicationScopeSearchRequest);
    IPage<ApplicationDeviceDO> applicationDeviceDOIPage =
        applicationDeviceRepository.searchLiveDevice(
            applicationDeviceQuery,
            applicationScopeSearchRequest.getPage(),
            applicationScopeSearchRequest.getSize());
    PageableResponse<ExperimentScope> pageableResponse =
        PageableResponse.of(
            applicationDeviceDOIPage.getCurrent(),
            applicationDeviceDOIPage.getSize(),
            applicationDeviceDOIPage.getRecords().stream()
                .map(e -> applicationDeviceToExperimentScopeConverter.convert(e))
                .filter(Objects::nonNull)
                .collect(Collectors.toList()),
            applicationDeviceDOIPage.getPages(),
  ...
  @Override
  public Response<PageableResponse<CloudDevice>> getScopesByApplication(
      ChaosUser user, PageableQueryApplicationHostRequest pageableQueryApplicationHostRequest) {

    ApplicationDO applicationDO =
        applicationRepository.findById(pageableQueryApplicationHostRequest.getAppId()).orElse(null);
    if (applicationDO == null) {
      return Response.failedWith(CommonErrorCode.P_APPLICATION_NOT_FOUND);
    }
    IPage<ApplicationDeviceDO> applicationDeviceDOIPage =
        findApplicationDeviceDOIPage(pageableQueryApplicationHostRequest, applicationDO);
    ...
    // RAM校验权限列表
    PageableResponse<CloudDevice> pageableResponse =
        PageableResponse.of(
            applicationDeviceDOIPage.getCurrent(),
            applicationDeviceDOIPage.getSize(),
            cloudDevices,
  ...
  private IPage<ApplicationDeviceDO> findApplicationDeviceDOIPage(
      ...
      ApplicationDO applicationDO) {
    IPage<ApplicationDeviceDO> applicationDeviceDOIPage =
        applicationDeviceRepository.findLiveDeviceByAppIdAndGroupAndKeyAndTags(
            applicationDO,
            pageableQueryApplicationHostRequest.getGroups(),
  ...
  @Override
  public Response<List<AppNameAndIdPair>> getUserApplicationNames(
  ...
  private List<AppNameAndIdPair> getAppNameAndIdPairs(ApplicationSearchRequest searchRequest) {
    List<AppNameAndIdPair> result =
        applicationRepository.findApplicationByUserIdAndScopeTypeReturnNames(
  ...
  @Override
  public Response<List<String>> getGroupsByApp(
      UserApplicationQueryRequest userApplicationQueryRequest) {
    return Response.okWithData(
        applicationGroupRepository.findApplicationGroupsByAppId(
            userApplicationQueryRequest.getAppId()));
  }
  ...
  public Response<List<Host>> getScopeByAppNameAndGroupsAndIp(
    ...
    List<ApplicationDeviceDO> applicationDeviceDOs =
        applicationDeviceRepository.findLiveDeviceByAppNameAndGroupAndIp(
  ...
  public Response<List<ApplicationConfiguration>> listApplicationConfigurations(
      UserApplicationQueryRequest userApplicationQueryRequest) {
    return Response.okWithData(
        applicationConfigurator.provide(userApplicationQueryRequest.getAppId()));
  }
  ...
  public Response updateApplicationConfiguration(
    ...
    boolean success =
        applicationConfigurator.updateOrCreateIfNotExistConfiguration(
  ...
  @Override
  public Response<PageableResponse<ApplicationDeviceDO>>
      pageableSearchApplicationHostsByAppIdOrAppNamePart(
          ApplicationScopeSearchRequest applicationScopeSearchRequest) {
    ApplicationDeviceQuery applicationDeviceQuery = new ApplicationDeviceQuery();
    applicationDeviceQuery.setAppId(applicationScopeSearchRequest.getAppId());
    ...
    IPage<ApplicationDeviceDO> applicationDeviceDOIPage =
        applicationDeviceRepository.searchDeviceByAppIdOrAppNamePart(
            applicationDeviceQuery,
            applicationScopeSearchRequest.getPage(),
            applicationScopeSearchRequest.getSize());
    PageableResponse<ApplicationDeviceDO> pageableResponse =
        PageableResponse.of(
            applicationDeviceDOIPage.getCurrent(),
            applicationDeviceDOIPage.getSize(),
            applicationDeviceDOIPage.getRecords(),
            applicationDeviceDOIPage.getPages(),
            applicationDeviceDOIPage.getTotal());
    return Response.okWithData(pageableResponse);
  }
  ...
  public Response<PageableResponse<ApplicationDO>> pageableSearchUserApplication(
    ...
    PageableResponse<ApplicationDO> pageableResponse =
        PageableResponse.of(
            applicationDOIPage.getCurrent(),
            applicationDOIPage.getSize(),
            applicationDOIPage.getRecords(),
            applicationDOIPage.getPages(),
            applicationDOIPage.getTotal());
    return Response.okWithData(pageableResponse);
  }
  ...
  private List<CloudDevice> mergeByIp(List<CloudDevice> cloudDevices, ApplicationDO applicationDO) {
    ...
    if (AppType.HOST.getType() == applicationDO.getAppType()) {
      ...
      Map<String, List<CloudDevice>> groupList =
          cloudDevices.stream().collect(Collectors.groupingBy(CloudDevice::getPrivateIp));
      groupList.forEach(
          (ip, deviceList) -> {
  ...
  @Override
  public Response<Boolean> updateApplicationTag(
      ApplicationTagsUpdateRequest applicationTagsUpdateRequest) {
      ...
      for (String configurationId : configurationIds) {
        List<TagDO> tagDOS =
            tags.stream()
                .map(
                    tagName ->
                        tagManager.findOrCreateIfNotExistByTagName(
                            userId, tagName, TAG_TYPE_APPLICATION_HOST))
                .collect(Collectors.toList());
        tagDOS.forEach(
            tagDO -> {
    ...
    if (!CollectionUtil.isNullOrEmpty(tags)) {
      ...
      for (TagDO tagDO : tagDOS) {
        for (String configurationId : configurationIds) {
          ...
          int count =
              applicationDeviceTagRepository.countByAppIdAndPrivateIpAndUserIdAndTagName(
  ...
  public Response<List<String>> searchDeviceTagsByAppIdAndGroupName(
    ...
    List<ApplicationDeviceDO> applicationDeviceDOS =
        applicationDeviceRepository.findByAppIdAndGroups(
            String.valueOf(applicationTagsSearchRequest.getAppId()),
            applicationTagsSearchRequest.getGroupNames());

    List<String> configurationIds =
        applicationDeviceDOS.stream()
            .map(ApplicationDeviceDO::getConfigurationId)
            .collect(Collectors.toList());

    return Response.okWithData(
        applicationDeviceTagRepository.listApplicationGroupTagsByConfigurationIds(
            configurationIds,
  ...
  public Response<Set<String>> searchClusterNamespacesByAppIdAndGroupName(
    ...
    return Response.okWithData(
        applicationDeviceDOS.stream()
            .collect(Collectors.groupingBy(ApplicationDeviceDO::getKubNamespace))
            .keySet());
  }

  @Override
  public Response<ApplicationHostsCount> countScopesByApplication(
      ChaosUser user, ApplicationHostsCountRequest applicationHostsCountRequest) {
    ...
    // 机器总数
    applicationHostsCount.setTotal(
        applicationDeviceRepository.countDeviceByAppIdAndGroupNames(
            applicationHostsCountRequest.getAppId(), applicationHostsCountRequest.getGroupNames()));
    return Response.okWithData(applicationHostsCount);
  }

  @Override
  public Response<List<Host>> getScopeByConfigurationIds(
      ApplicationGroupDeviceQuery applicationGroupDeviceQuery) {
    List<ApplicationDeviceDO> applicationDeviceDOs =
  ...
  @Override
  public Response<List<ExperimentApplication>> findApplicationsInExperimentForVision(
      String mainUserId, String namespace) {
    return Response.okWithData(
        experimentApplicationFinder.findApplicationsInExperiment(mainUserId, namespace));
  }
  ...
  public Response<PageableResponse<AppNameAndIdPair>> pageUserApplicationNames(
    ...
    if (!Strings.isNullOrEmpty(applicationSearchRequest.getAppType())
        && (String.valueOf(Scope.SCOPE_TYPE_CONTAINER).equals(applicationSearchRequest.getAppType())
            || String.valueOf(Scope.SCOPE_TYPE_K8S)
                .equals(applicationSearchRequest.getAppType()))) {
      // 应用类型 0代表主机 1代表容器环境
      applicationSearchRequest.setAppType("1");
    }
    List<ApplicationDeviceDO> applicationDeviceDOS =
        applicationDeviceRepository.findUserDeviceGroupByAppId(
            applicationSearchRequest.getUserId(), applicationSearchRequest.getAppIds());
    Map<Long, ApplicationDeviceDO> appDeviceMap =
        applicationDeviceDOS.stream()
            .filter(
                deviceDO -> {
    ...
    IPage<ApplicationDO> applicationDOIPage =
        applicationRepository.pageUserApplications(
    ...
    List<AppNameAndIdPair> appNameAndIdPairs =
        applicationDOIPage.getRecords().stream()
            .map(
                applicationDO -> {
                  AppNameAndIdPair appNameAndIdPair =
                      new AppNameAndIdPair(
                          applicationDO.getAppName(), String.valueOf(applicationDO.getId()));
                  appNameAndIdPair.setAppType(applicationDO.getAppType());
                  appNameAndIdPair.setScopeType(
                      ScopeTypeEnum.judgeScopeByAppType(applicationDO.getAppType()).getValue());
                  ApplicationDeviceDO deviceDO = appDeviceMap.get(applicationDO.getId());
                  if (deviceDO != null) {
                    appNameAndIdPair.setOsType(
                        null == deviceDO.getOsType() ? 0 : deviceDO.getOsType());
                  }
  ...
  public Response<Host> getScopeByAppConfigurationId(String appConfigurationId) {
    Optional<ApplicationDeviceDO> applicationDeviceDOOptional =
        applicationDeviceRepository.findById(appConfigurationId);
    if (!applicationDeviceDOOptional.isPresent()) {
      return Response.failedWith(CommonErrorCode.P_APP_DEVICE_NOT_FOUND);
    }
    return Response.okWithData(
        applicationDeviceConverter.convert(applicationDeviceDOOptional.get()));
  }
}
