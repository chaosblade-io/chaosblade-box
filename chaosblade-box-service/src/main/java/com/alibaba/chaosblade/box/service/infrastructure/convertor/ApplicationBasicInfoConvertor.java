...
/** @author haibin */
@Component
public class ApplicationBasicInfoConvertor
    extends BaseApplicationConvertor<ApplicationDO, ApplicationBasicInfo> {
  ...
  private void fillExperimentTaskInfo(ApplicationBasicInfo applicationBasicInfo, Long appId) {
    ApplicationRelationDO applicationRelationDO =
        applicationRelationRepository.findLatestExperimentTaskByAppId(appId);
    if (applicationRelationDO != null) {
      String experimentTaskId = applicationRelationDO.getOuterId();
      experimentTaskRepository
          .findById(experimentTaskId)
          .ifPresent(
