...
@Component
public class ActivityTaskQueryCommand
    extends SpringBeanCommand<ActivityTaskQueryRequest, ActivityTask> {
  ...
  public ActivityTask execute(ActivityTaskQueryRequest activityTaskQueryRequest) {
    ...
    if (activityTaskDO == null) {
      activityTaskDO =
          activityTaskRepository
              .findById(activityTaskQueryRequest.getActivityTaskId())
              .orElse(null);
    }
    if (activityTaskDO == null) {
      return null;
  ...
  private ActivityTask loadActivityTask(
      ActivityTaskDO activityTaskDO, ActivityTaskQueryRequest activityTaskQueryRequest) {
    ...
    Date startTime =
        activityTaskDO.getStartTime() == null
            ? activityTaskDO.getGmtCreate()
    ...
    if (activityTaskQueryRequest.isReturnMiniAppTasks()) {
      activityTask.setApps(
          miniAppTaskManager.findMiniAppTasksByActivityTaskId(activityTaskDO.getTaskId()));
      activityTask.setArguments(
          argumentDefinitionConvertor.splitExperimentNodeArgumentsDefinition(
              activityTaskDO.getAppCode(), activityTaskDO.getRunParam().getArguments()));
    }
  ...
  private void fillActivityName(ActivityTask activityTask, ActivityTaskDO activityTaskDO) {
    ...
    if (Strings.isNullOrEmpty(activityName)) {
      ExperimentActivityDO experimentActivityDO =
          activityRepository.findLatestActivityByActivityId(activityTaskDO.getActivityId());
      activityName = experimentActivityDO.getActivityName();
      if (Strings.isNullOrEmpty(experimentActivityDO.getActivityName())) {
        Optional<SceneFunctionDO> sceneFunctionDOOptional =
            sceneFunctionRepository.findByCode(experimentActivityDO.getAppCode());
