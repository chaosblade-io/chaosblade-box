...
public class ExperimentTaskGuardInfoQueryCommand
  ...
  @Override
  public Response<ExperimentTaskGuardsResult> execute(
      BaseExperimentTaskRequest baseExperimentTaskRequest) {
    Response<ExperimentTaskDO> experimentTaskDOResponse =
        experimentChecker.assertExperimentTaskExist(baseExperimentTaskRequest.getTaskId());
    ...
    List<ExperimentGuardInstanceDO> experimentGuardInstanceDOS =
        experimentGuardInstanceRepository.findByExperimentTaskId(experimentTaskDO.getTaskId());
    commandBus.syncRun(
        ExperimentGuardInstanceExecutionCommand.class,
        new ExperimentGuardInstanceExecutionRequest(
            experimentTaskDO, experimentGuardInstanceDOS, false));
  ...
  private void transferMetrics(
      ExperimentTaskGuardsResult experimentTaskGuardsResult,
      List<ExperimentGuardInstanceDO> experimentGuardInstanceDOS) {
    // 这个是Metric类型的数据结果
    List<BaseExperimentGuardResultEntity> metrics = new ArrayList<>();
    experimentTaskGuardsResult.setMetrics(metrics);
    // 这个是自我保护的结果
    List<ExperimentGuardRecoveryStrategyResultEntity> recoveryStrategyResultEntities =
        new ArrayList<>();
   ...
   * @return
   */
  private ExperimentGuardRecoveryStrategyResultEntity
      convertExperimentGuardRecoveryStrategyResultEntity(
    ...
    experimentGuardRecoveryStrategyResultEntity.setFields(
        experimentGuardInstanceDO.getArgument().getFields());
  ...
  private BaseExperimentGuardResultEntity convertBaseExperimentGuardResultEntity(
    ...
    BaseExperimentGuardResultEntity baseExperimentGuardResultEntity =
        new BaseExperimentGuardResultEntity();
