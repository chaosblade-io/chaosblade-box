...
@Component
@Slf4j
public class AgentClosedCommand
    extends SpringBeanCommand<AgentClosedCallbackRequest, Response<Boolean>> {

  private static Logger logger = LoggerFactory.getLogger("agent-close");

  @Autowired private PrivateCloudDeviceRepository deviceRepository;
  ...
  public Response<Boolean> execute(AgentClosedCallbackRequest agentClosedCallbackRequest) {
    log.info(
        "start handle client closed request,{}" + JSON.toJSONString(agentClosedCallbackRequest));
    try {
      Preconditions.checkArgument(agentClosedCallbackRequest.getUserId() != null, "uid Required");
      Preconditions.checkArgument(
          agentClosedCallbackRequest.getConfigurationId() != null, "cid Required");
      String userId = agentClosedCallbackRequest.getUserId();
      deviceRepository
          .findByUserIdAndDeviceTypeAndConfigurationId(
              userId, DeviceType.HOST.getType(), agentClosedCallbackRequest.getConfigurationId())
          .forEach(
              new Consumer<DeviceDO>() {
                @Override
