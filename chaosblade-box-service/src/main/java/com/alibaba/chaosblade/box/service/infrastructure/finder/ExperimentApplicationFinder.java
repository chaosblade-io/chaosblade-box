...
public class ExperimentApplicationFinder {
  ...
  public List<ExperimentApplication> findApplicationsInExperiment(
      String mainUserId, String namespace) {
    QueryWrapper<ExperimentHostRelationDO> queryWrapper = buildQuery(mainUserId, namespace);
    return doQuery(queryWrapper);
  }

  private List<ExperimentApplication> doQuery(QueryWrapper<ExperimentHostRelationDO> queryWrapper) {
    return experimentHostRelationMapper.selectMaps(queryWrapper).stream()
        .map(
            stringObjectMap -> {
              ExperimentApplication experimentApplication = new ExperimentApplication();
              experimentApplication.setAppId(
                  (String) stringObjectMap.get(ExperimentHostRelationDO.COLUMN_APP_ID));
              experimentApplication.setTaskCount(
                  (Long) stringObjectMap.get(ExperimentHostRelationDO.RESULT_FIELD_TOTAL));
              experimentApplication.setAppName(
                  applicationRepository
                      .findByIdWithStr(experimentApplication.getAppId())
                      .map(ApplicationDO::getAppName)
                      .orElse(null));
  ...
  private QueryWrapper<ExperimentHostRelationDO> buildQuery(String mainUserId, String namespace) {
    ...
    queryWrapper.eq(
        ExperimentHostRelationDO.COLUMN_RELATION_TYPE,
        ExperimentHostRelationDO.RELATION_EXPERIMENT_TASK);
    queryWrapper.eq(ExperimentHostRelationDO.COLUMN_NAMESPACE, namespace);
    queryWrapper.eq(
        ExperimentHostRelationDO.COLUMN_EXPERIMENT_TASK_STATE, StateEnum.RUNNING.getValue());
