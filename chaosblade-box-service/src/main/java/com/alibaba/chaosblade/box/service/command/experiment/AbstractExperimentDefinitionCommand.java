...
public abstract class AbstractExperimentDefinitionCommand
    extends SpringBeanCommand<ExperimentDefinitionRequest, Response<Void>>
    implements InitializingBean {

  @Autowired protected ExperimentChecker experimentChecker;

  @Autowired protected ChaosEventDispatcher chaosEventDispatcher;

  @Autowired protected TransactionUtil transactionUtil;

  @Autowired protected ExperimentGuardRepository experimentGuardRepository;

  @Autowired protected ExperimentMiniFlowRepository experimentMiniFlowRepository;

  @Autowired protected ExperimentMiniFlowGroupRepository experimentMiniFlowGroupRepository;

  @Autowired protected ExperimentFlowDefinitionManager experimentFlowDefinitionManager;

  @Autowired private ExperimentRepository experimentRepository;

  @Autowired private ArgumentDefinitionConvertor argumentDefinitionConvertor;

  @Autowired private List<ActivityDefinitionChecker> activityDefinitionCheckers;

  @Autowired private ExperimentRelationRepository experimentRelationRepository;

  @Autowired private ApplicationRelationBinder applicationRelationBinder;

  private BaseMiniGroupCheckNode miniGroupCheckNode;

  @Autowired private ApplicationContext applicationContext;

  @Autowired private List<StableProbeInterceptor> stableProbeInterceptors;

  @Autowired private SceneArgumentGradeConverter sceneArgumentGradeConverter;

  @Override
  public Response<Void> execute(ExperimentDefinitionRequest experimentFlowDefinitionRequest) {
    Response<ExperimentDO> experimentDOResponse = getExperimentDO(experimentFlowDefinitionRequest);
  ...
  private void handleExperiment(
      ExperimentDefinitionRequest experimentFlowDefinitionRequest,
  ...
  private void handleMiniFlowGroups(
      ExperimentDO experimentDO,
      ExperimentDefinitionContext experimentDefinitionContext,
    ...
    for (MiniFlowGroup miniFlowGroup : miniFlowGroups) {
      ...
      experimentDefinitionContext.addApplication(
          miniFlowGroup.getAppName(), miniFlowGroup.getAppId());
      miniGroupCheckNode.invoke(
          new MiniGroupCheckContext(miniFlowGroup, experimentDefinitionContext));
  ...
  protected void handleApplicationRelations(
      ExperimentDefinitionContext experimentDefinitionContext) {
  ...
  protected void handleExpertise(
      ExperimentDO experimentDO, ExperimentDefinitionRequest experimentDefinitionRequest) {
    if (Strings.isNullOrEmpty(experimentDefinitionRequest.getExpertiseId())) {
      return;
    }
    ExperimentRelationDO experimentRelationDO =
        experimentRelationRepository.findByExperimentIdAndOuterIdAndRelationType(
            experimentDO.getExperimentId(),
            experimentDefinitionRequest.getExpertiseId(),
            ExperimentRelationDO.RELATION_TYPE_EXPERTISE);
  ...
  protected Response<ExperimentDO> getExperimentDO(
    ...
    Response<ExperimentDO> experimentDOResponse =
        experimentChecker.assertExperimentExist(experimentDefinitionRequest.getExperimentId());
    if (!experimentDOResponse.isSuccess()) {
      return Response.failedWith(experimentDOResponse.getError());
    }
  ...
  private void setOrderForActivities(List<MiniFlowGroup> miniFlowGroups) {
    int count = 0;
    for (MiniFlowGroup miniFlowGroup : miniFlowGroups) {
  ...
  /** 创建演练流程定义 */
  void fillExperimentFlowDefinition(
      ExperimentDefinitionContext experimentDefinitionContext,
    ...
    for (MiniFlow miniFlow : miniFlowGroup.getFlows()) {
      ...
      experimentDefinitionContext.addDefinitions(
          PhaseType.ATTACK,
          convertNewExperimentActivityDefinition(
              experimentDefinitionContext,
              miniFlow.getAttack(),
              miniFlowId,
  ...
  /** 处理微流程 */
  protected abstract void handleMiniFlowGroup(
      ExperimentDefinitionContext experimentDefinitionContext,
      ExperimentDO experimentDO,
      MiniFlowGroup miniFlowGroup);
  ...
  protected void addMiniGroup(
    ...
    fillExperimentFlowDefinition(experimentDefinitionContext, miniFlowGroup, miniFlowGroupDO);
  }
  ...
  protected List<ExperimentActivityDefinition> convertNewExperimentActivityDefinition(
      ExperimentDefinitionContext experimentDefinitionContext,
      List<ExperimentActivityInfo> experimentActivityDefinitions,
      String flowId,
      List<Host> hosts) {
    return convertNewExperimentActivityDefinition(
        experimentDefinitionContext, experimentActivityDefinitions, flowId, hosts, null);
  }
  ...
  protected List<ExperimentActivityDefinition> convertNewExperimentActivityDefinition(
      ...
      List<Host> hosts,
      String attackName) {
    if (CollectionUtil.isNullOrEmpty(experimentActivityDefinitions)) {
    ...
    for (ExperimentActivityInfo experimentActivityDefinition : experimentActivityDefinitions) {
      ...
      if (!Strings.isNullOrEmpty(attackName)
          && MiniAppUtils.isChaosRecovery(experimentActivityDefinition.getAppCode())) {
        if (experimentDefinitionContext
            .getExperimentDefinitionRequest()
            .getLang()
            .equals(ChaosConstant.LANGUAGE_EN)) {
      ...
      temp.setArguments(
          argumentDefinitionConvertor.mergeArgumentDefinitions(
              ...
              sceneArgumentGradeConverter.tranToArgumentsList(
  ...
  protected void handleStableProbe(ExperimentDefinitionRequest experimentDefinitionRequest) {
    stableProbeInterceptors.forEach(
        stableProbeInterceptor -> {
  ...
  protected void setNewFlowTag(ExperimentDO experimentDO) {
    experimentDO.putAttribute(
        ExperimentAttributes.ATTR_NEW_DEFINITION_FLOW,
        ActivityTaskExecutionAttributes.ATTRIBUTE_VALUE_TRUE);
  }

  @Override
  public void afterPropertiesSet() throws Exception {
    this.miniGroupCheckNode =
        ChainFactory.createSpringChain(applicationContext, BaseMiniGroupCheckNode.class);
  }
}
