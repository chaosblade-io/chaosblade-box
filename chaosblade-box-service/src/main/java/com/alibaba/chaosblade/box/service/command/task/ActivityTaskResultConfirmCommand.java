...
public class ActivityTaskResultConfirmCommand
  ...
  @Resource private ActivityTaskChecker activityTaskChecker;
  ...
  public Response<Void> execute(ActivityTaskResultConfirmRequest activityTaskResultConfirmRequest) {
    ...
    Response<Void> activityTaskDOResponse =
        activityTaskChecker.isActivityTaskCanUserCheck(activityTaskDO);
    if (!activityTaskDOResponse.isSuccess()) {
      return Response.failedWith(activityTaskDOResponse.getError());
    }
    UserCheckState userCheckState =
        activityTaskResultConfirmRequest.isSuccess()
    ...
    if (UserCheckState.USER_CHECK_STATE_FAILED.equals(userCheckState)) {
      experimentTaskManager.stopTask(
          user, activityTaskDO.getExperimentTaskId(), new ExperimentTaskStopOption());
    } else {
        ...
        new Command<Void>() {
          ...
          public Void execute() {
            ...
            experimentTaskPusher.push(
                activityTaskDO, contextData.getExperimentTaskDO(), contextData);
            return null;
          }
        });
  }
