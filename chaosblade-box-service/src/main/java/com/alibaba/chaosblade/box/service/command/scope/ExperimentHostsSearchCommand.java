...
public class ExperimentHostsSearchCommand
  ...
  public ExperimentScope buildExperimentScope(DeviceDO deviceDO) {
    ...
    experimentScope.setExperimentTaskCount(
  ...
  private void canAutoInstall(ExperimentScope experimentScope, DeviceDO deviceDO) {
    if (StringUtils.isBlank(deviceDO.getInstallMode())
        || InstallMode.host.name().equalsIgnoreCase(deviceDO.getInstallMode())) {
  ...
  private PageableResponse<DeviceDO> findAliveMachines(
    ...
    cloudDeviceQuery.setScopeType(
        EnumUtil.integerValueOf(
            ScopeTypeEnum.class, experimentScopePageableRequest.getScopeType()));
    if (experimentScopePageableRequest.getFilter() != null) {
      ...
      if (ExperimentScopeFilter.FILTER_TYPE_NOT_EXPERIMENT.equals(
          experimentScopeFilter.getType())) {
        cloudDeviceQuery.setIsExperiment(false);
      }
    }
    PageableQueryWrapper<CloudDeviceQuery> pageableQueryWrapper =
        PageableQueryWrapper.of(cloudDeviceQuery);
