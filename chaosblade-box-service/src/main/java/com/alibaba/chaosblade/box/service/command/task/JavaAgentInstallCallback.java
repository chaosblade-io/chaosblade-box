    ...
    if (prepareStatusBean == null) {
      ...
      if (!prepareStatusBeanResponse.isSuccess()) {
        markInstallFailed(
            ...
            CommonErrorCode.B_JAVA_AGENT_CREATED_ERROR.name(),
    ...
    if (prepareStatusBean != null) {
      ...
      if (ChaosBladeStatus.Created.name().equals(prepareStatusBean.getStatus())) {
        if (installTimeOut(experimentMiniAppTaskDO)) {
          markInstallFailed(
              experimentMiniAppTaskDO,
              CommonErrorCode.B_JAVA_AGENT_CREATED_TIMEOUT.name(),
              CommonErrorCode.B_JAVA_AGENT_CREATED_TIMEOUT.getReadableMessage()
                  + ",超时时间:"
      ...
      if (ChaosBladeStatus.Revoked.name().equals(prepareStatusBean.getStatus())) {
        markInstallFailed(
            experimentMiniAppTaskDO,
            CommonErrorCode.B_JAVA_AGENT_REVOKED.name(),
      ...
      chaosBladeExpUidDO.addAttribute(
          ChaosBladeExpUidDO.ATTRIBUTE_PROCESS_NAME, prepareStatusBean.getProcess());
      chaosBladeExpUidDO.addAttribute(
          ChaosBladeExpUidDO.ATTRIBUTE_PORT, prepareStatusBean.getPort());
