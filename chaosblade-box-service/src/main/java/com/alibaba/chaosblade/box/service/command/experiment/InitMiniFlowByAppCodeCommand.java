...
public class InitMiniFlowByAppCodeCommand
    extends SpringBeanCommand<
        InitMiniFlowRequest, Response<Map<String, List<ExperimentActivityInfo>>>> {
  ...
  @Autowired private List<InitMiniFlowByAppCodeInterceptor> initMiniFlowByAppCodeInterceptors;

  @Autowired private SceneFunctionUpdateInternalOperator sceneFunctionUpdateInternalOperator;
  ...
  public Response<Map<String, List<ExperimentActivityInfo>>> execute(
    ...
    SceneFunctionDO sceneFunctionDO = resetFunctionAppCode(appCode);
  ...
  private void fillDependencies(
    ...
    handleDependencies(
        sceneFunctionDependencies,
        phaseToActivities,
        Sets.newHashSet(appCode),
        initMiniFlowRequest);
  }
  ...
  private void initAttachActivity(
      ...
      SceneFunctionDO sceneFunctionDO,
      Map<String, List<ExperimentActivityInfo>> phaseToActivities) {
    List<ExperimentActivityInfo> attacks =
        phaseToActivities.computeIfAbsent(
            PhaseType.ATTACK.name().toLowerCase(), phaseType -> new ArrayList<>());
    ExperimentActivityInfo experimentActivityInfo =
        createBySceneFunctionDO(sceneFunctionDO, initMiniFlowRequest);
  ...
  private void handleDependencies(
      List<SceneFunctionDependency> sceneFunctionDependencies,
      Map<String, List<ExperimentActivityInfo>> result,
      Set<String> appCodes,
      InitMiniFlowRequest initMiniFlowRequest) {
    if (CollectionUtil.isNullOrEmpty(sceneFunctionDependencies)) {
      return;
    }
    SceneFunctionDO sceneFunctionDO = null;
    for (SceneFunctionDependency sceneFunctionDependency : sceneFunctionDependencies) {
      sceneFunctionDO =
          sceneFunctionRepository.findByCode(sceneFunctionDependency.getCode()).orElse(null);
      if (sceneFunctionDO == null
          || !sceneFunctionDO.getEnabled().equals(ChaosFunctionConstant.ENABLED_ONLINE)) {
        continue;
      }
      ExperimentActivityInfo experimentActivityInfo =
          createBySceneFunctionDO(sceneFunctionDO, initMiniFlowRequest);
      ...
      result
          .computeIfAbsent(
              PhaseType.ofType(sceneFunctionDependency.getPhase()).name().toLowerCase(),
              phaseType -> new ArrayList<>())
          .add(experimentActivityInfo);
      List<SceneFunctionDependency> secondSceneFunctionDependencies =
          sceneFunctionDO.getDependencyList().stream()
              .filter(
                  sceneFunctionDependency1 ->
                      !appCodes.contains(sceneFunctionDependency1.getCode()))
  ...
  public void checkChaosBladeRecoveryAttack(
      InitMiniFlowByAppCodeInterceptorContext initMiniFlowByAppCodeInterceptorContext) {
    if (!MiniAppUtils.isFromChaosBlade(initMiniFlowByAppCodeInterceptorContext.getAppCode())
        && !MiniAppUtils.isFromLitmusChaos(initMiniFlowByAppCodeInterceptorContext.getAppCode())) {
      return;
    }
    ...
    List<ExperimentActivityInfo> experimentActivityInfos =
        response.computeIfAbsent(PhaseType.RECOVER.name().toLowerCase(), s -> new ArrayList<>());
    String recoveryCode = MiniAppUtils.getRecoverCode(appCode);
    boolean existRecovery =
        experimentActivityInfos.stream()
            .anyMatch(
                experimentActivityInfo ->
                    Objects.equals(experimentActivityInfo.getAppCode(), recoveryCode));
    if (!existRecovery) {
      sceneFunctionRepository
          .findByCode(recoveryCode)
          .ifPresent(
              sceneFunctionDO -> {
                ExperimentActivityInfo recovery =
                    createBySceneFunctionDO(
                        sceneFunctionDO,
  ...
  private ExperimentActivityInfo createBySceneFunctionDO(
      SceneFunctionDO sceneFunctionDO, InitMiniFlowRequest initMiniFlowRequest) {
    ...
    List<SceneFunctionParameterDO> sceneFunctionParameterDOS =
        sceneFunctionParameterRepository.findFilterParamsByFunctionId(sceneFunctionDO);
    experimentActivityInfo.setArguments(
        sceneArgumentGradeConverter.tranToArgumentsGrade(
            argumentDefinitionConvertor.convertToArgumentDefinitions(
                sceneFunctionDO.getCode(),
                sceneFunctionParameterDOS,
