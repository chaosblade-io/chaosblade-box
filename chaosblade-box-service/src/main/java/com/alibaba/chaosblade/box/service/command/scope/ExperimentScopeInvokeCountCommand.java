...
public class ExperimentScopeInvokeCountCommand
  ...
  @Autowired private CloudManualMapper cloudManualMapper;
  ...
  public List<ExperimentScopeInvokeCount> execute(ScopeInfoQueryRequest scopeInfoQueryRequest) {
    ...
    cloudManualMapper.countInvocationByHostAndDate(configurationId, monthDate).stream()
        .map(
            stringObjectMap -> {
              ExperimentScopeInvokeCount experimentScopeInvokeCount =
                  new ExperimentScopeInvokeCount();
              try {
                experimentScopeInvokeCount.setTime(
                    DateUtils.parseDate(
                        stringObjectMap.get("date").toString(),
                        DateFormatUtils.ISO_8601_EXTENDED_DATE_FORMAT.getPattern()));
              } catch (ParseException e) {
              }
              experimentScopeInvokeCount.setTotal(
                  Integer.parseInt(stringObjectMap.get("total").toString()));
              return experimentScopeInvokeCount;
            })
        .forEach(
            experimentScopeInvokeCount -> {
    ...
    return result.values().stream()
        .sorted(Comparator.comparing(ExperimentScopeInvokeCount::getTime))
