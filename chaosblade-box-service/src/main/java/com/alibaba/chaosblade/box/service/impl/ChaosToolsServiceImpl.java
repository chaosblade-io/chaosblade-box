...
public class ChaosToolsServiceImpl implements ChaosToolsService {
  ...
  @Autowired private DefaultScenarioYamlProvider scenarioYamlProvider;
  ...
  public List<ToolsOverview> toolsOverviewList(ChaosToolsRequest chaosToolsRequest) {
    ...
    return Arrays.stream(ChaosTools.values())
        .map(
            chaosTools -> {
              String configuration =
                  scenarioYamlProvider.overview(
                      ScenarioRequest.builder().chaosTools(chaosTools.getName()).build());
              ...
              if ("host".equals(chaosToolsRequest.getInstallMode())) {
                chaosToolsDO =
                    chaosToolsRepository.selectByConfigurationId(chaosToolsRequest.getOperateId());
              } else {
                chaosToolsDO =
                    chaosToolsRepository.selectByClusterId(chaosToolsRequest.getOperateId());
              }
  ...
  @Override
  public com.alibaba.chaosblade.box.common.common.domain.Response<Boolean> installChaosTools(
      ChaosToolsRequest chaosToolsRequest) {

    if ("host".equals(chaosToolsRequest.getInstallMode())) {
      return com.alibaba.chaosblade.box.common.common.domain.Response.failedWith(
          CommonErrorCode.B_CHAOS_TOOLS_INSTALL_NOT_SUPPORT_HOST);
    }
    UserDo userDo =
        licenseRepository
            .getUserLicense(chaosToolsRequest.getUserId())
            .orElseThrow(() -> new ChaosException(CommonErrorCode.P_ARGUMENT_ILLEGAL, "无效的集群"));

    IPage<DeviceDO> devices =
        deviceRepository.getDevicesByClusterId(chaosToolsRequest.getOperateId(), 1, 1);
    ...
    String proxyTag =
        ProxyHelper.buildProxyTag(
            PluginTypeUtil.getPluginTypeByDeviceType(deviceDO.getDeviceType()),
            deviceDO.getPrivateIp(),
            null);

    Response<String> response =
        litmusChaosForCloud.installLitmusOperatorForCloud(
            deviceDO.getPrivateIp(),
            String.valueOf(deviceDO.getDeviceType()),
            userDo.getLicense(),
            userDo.getSecretKey(),
            deviceDO.getVpcId(),
            proxyTag,
            chaosToolsRequest.getVersion());

    if (!response.isSuccess()) {
      throw new ChaosException(CommonErrorCode.B_CHAOS_TOOLS_INSTALL_ERROR, response.getError());
    } else {
      chaosToolsRepository.add(
          ChaosToolsDO.builder()
  ...
  @Override
  public com.alibaba.chaosblade.box.common.common.domain.Response<Boolean> uninstallChaosTools(
      ChaosToolsRequest chaosToolsRequest) {
    ...
    Response<String> response =
        litmusChaosForCloud.uninstallLitmusOperatorForCloud(
            deviceDO.getPrivateIp(),
            String.valueOf(deviceDO.getDeviceType()),
            userDo.getLicense(),
            userDo.getSecretKey(),
