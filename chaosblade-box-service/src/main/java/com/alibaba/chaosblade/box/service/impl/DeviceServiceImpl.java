...
public class DeviceServiceImpl implements DeviceService {
    ...
    public void deviceRegister(DeviceRegisterRequest deviceRegisterRequest) {
        ...
        ProbesInstallModel installModel = EnumUtil.fromString(ProbesInstallModel.class,
            deviceRegisterRequest.getInstallMode().toUpperCase());
        switch (installModel) {
            case ANSIBLE:
            case SSH:
                ...
                Long groupId = applicationGroupRepository.selectOneByAppIdAndGroupName(appId,
                    deviceRegisterRequest.getAppGroup())
                ...
                DeviceDO deviceDO = DeviceDO.builder()
                    ...
                    .memorySize(deviceRegisterRequest.getMemorySize() == null ? null
                        : deviceRegisterRequest.getMemorySize().intValue())
                ...
                Long deviceId = deviceRepository.selectOneByUnique(DeviceType.HOST.getCode(),
                    deviceRegisterRequest.getHostName())
                    .map(device -> {
    ...
    public List<DeviceNodeResponse> getMachinesForNode(DeviceNodeRequest deviceNodeRequest) {
        ...
        Map<Long, DeviceDO> deviceDOMap = deviceRepository.selectBatchIds(
            deviceNodeDOS.stream().map(DeviceNodeDO::getDeviceId)
                .collect(Collectors.toList()))
    ...
    public List<DevicePodResponse> getMachinesForPod(DevicePodRequest devicePodRequest) {
        ...
        Map<Long, DeviceDO> deviceDOMap = deviceRepository.selectBatchIds(
            devicePodDOS.stream().map(DevicePodDO::getDeviceId)
                .collect(Collectors.toList()))
            .stream().collect(Collectors.toMap(DeviceDO::getId, u -> u));
        ...
        return devicePodDOS.stream().map(devicePodDO ->
            {
                ...
                List<ContainerBO> filter = containers.stream().filter((containerBO) -> {
                        if (containerBO.getContainerName().contains("check")) {
                            return false;
                        } else {
                            return true;
                        }
