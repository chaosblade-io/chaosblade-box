...
@Slf4j
@Component
public class ExperimentAppRiskCommand
    extends SpringBeanCommand<String, Response<List<ExperimentAppRisk>>> {

  @Autowired private ActivityQueryManager activityQueryManager;

  @Autowired private AppRiskMessageLoadFactory appRiskMessageLoadFactory;
  ...
  public Response<List<ExperimentAppRisk>> execute(String experimentId) {
    ...
    List<ExperimentAppRisk> result =
        experimentActivities.stream()
            .map(
                experimentActivityDO -> {
                  AppRiskMessage appRiskMessage =
                      appRiskMessageLoadFactory.getAppRiskMessage(
