...
public class ExperimentFlowDefinitionQueryCommand
  ...
  @Autowired private ExperimentFlowDefinitionManager experimentFlowDefinitionManager;

  @Autowired private ExperimentMiniFlowGroupRepository miniFlowGroupRepository;
  ...
  @Autowired private MiniFlowGroupHostInterceptor miniFlowGroupHostCheckInterceptor;
  ...
  public ExperimentFlowInfo execute(ExperimentDO experimentDO) {
    ...
    List<ExperimentActivityDO> experimentActivityDOS =
        activityQueryManager.findActivitiesByExperimentId(experimentDO.getExperimentId());
  ...
  private List<MiniFlowGroup> queryNewMiniFlowGroups(
      List<ExperimentActivityDO> experimentActivityDOS, ExperimentDO experimentDO) {
    // 按照flowId来进行分组
    Map<String, List<ExperimentActivityDO>> FlowIdToExperimentActivityDO =
        experimentActivityDOS.stream()
            .collect(Collectors.groupingBy(ExperimentActivityDO::getFlowId));
    // 查找所有的miniFlowGroup
    List<MiniFlowGroupDO> miniFlowGroupDOS =
        miniFlowGroupRepository.findByExperimentId(experimentDO.getExperimentId());
    return miniFlowGroupDOS.stream()
        .map(
   ...
   * @return 微流程组
   */
  private MiniFlowGroup handleMiniFlowGroupDO(
        ...
        miniFlowDOS.stream()
            .map(
                miniFlowDO ->
                    handleMiniFlowDO(
                        miniFlowDO,
                        flowIdToActivityDefinitions.getOrDefault(
                            miniFlowDO.getFlowId(), new ArrayList<>())))
            .filter(
   ...
   * @return 微流程
   */
  private MiniFlow handleMiniFlowDO(
      MiniFlowDO miniFlowDO, List<ExperimentActivityDO> experimentActivityDOS) {
    ...
    Map<PhaseType, List<ExperimentActivityDO>> phaseToActivity =
        experimentActivityDOS.stream()
            .collect(Collectors.groupingBy(ExperimentActivityDO::getPhase));
    miniFlow.setRecover(
        handleExperimentActivityDOs(
            phaseToActivity.getOrDefault(PhaseType.RECOVER, new ArrayList<>())));
    miniFlow.setPrepare(
        handleExperimentActivityDOs(
            phaseToActivity.getOrDefault(PhaseType.PREPARE, new ArrayList<>())));
    miniFlow.setCheck(
        handleExperimentActivityDOs(
            phaseToActivity.getOrDefault(PhaseType.CHECK, new ArrayList<>())));
    miniFlow.setAttack(
        handleExperimentActivityDOs(
            phaseToActivity.getOrDefault(PhaseType.ATTACK, new ArrayList<>())));
  ...
  /**
   * 校验一下微流程的参数，因为存在有空的微流程这样的脏数据 在查询微流程信息时，过滤掉这种空的微流程数据
   *
   * @param miniFlow
   * @return
   */
  ...
  private List<ExperimentActivityInfo> handleExperimentActivityDOs(
      List<ExperimentActivityDO> experimentActivityDOS) {
    if (CollectionUtil.isNullOrEmpty(experimentActivityDOS)) {
      return new ArrayList<>();
    }
    return experimentFlowDefinitionManager.getActivityDefinitions(experimentActivityDOS).stream()
        .map(
            experimentActivityDefinition -> {
              ExperimentActivityInfo experimentActivityInfo = new ExperimentActivityInfo();
              BeanUtils.copyProperties(
                  experimentActivityDefinition, experimentActivityInfo, "arguments");
              experimentActivityInfo.setArguments(
                  sceneArgumentGradeConverter.tranToArgumentsGrade(
                      argumentDefinitionConvertor.splitExperimentNodeArgumentsDefinition(
  ...
  public void adapt(List<ExperimentActivityDO> experimentActivityDOS, ExperimentDO experimentDO) {
    Map<Hosts, List<ExperimentActivityDO>> hostsToActivities =
        experimentActivityDOS.stream()
            .collect(
    ...
    List<MiniFlowGroup> miniFlowGroups = new ArrayList<>();
    int count = 1;
    for (Map.Entry<Hosts, List<ExperimentActivityDO>> hostsListEntry :
        hostsToActivities.entrySet()) {
      if (hostsListEntry.getValue().isEmpty()) {
        continue;
      }
      MiniFlowGroup miniFlowGroup = new MiniFlowGroup();
  ...
  private void createNewExperimentDefinition(
      ExperimentDO experimentDO, List<MiniFlowGroup> miniFlowGroups) {
    ...
    Response response =
        commandBus.syncRun(ExperimentDefinitionCreateCommand.class, experimentDefinitionRequest);
    if (!response.isSuccess()) {
      if (!CommonErrorCode.B_DUMP_EXPERIMENT_DEFINITION.equals(
  ...
  private void checkHosts(MiniFlowGroup miniFlowGroup, MiniFlowGroupDO miniFlowGroupDO) {
    ...
    List<Host> hosts =
        miniFlowGroupDO.getHosts().getContent().stream()
            .peek(
                host -> {
                  // 2代表k8s,0代表host
                  ...
                  configurationIdList.add(
                      host.isK8s()
                          ? host.getAppConfigurationId()
    ...
    if (!CollectionUtil.isNullOrEmpty(appId) && !CollectionUtil.isNullOrEmpty(groupName)) {
      List<String> existConfigurationIdList =
          miniFlowGroupHostCheckInterceptor.batchCheck(configurationIdList, appId, groupName);
      miniFlowGroup.setHosts(
          hosts.stream()
              .peek(
                  host -> {
