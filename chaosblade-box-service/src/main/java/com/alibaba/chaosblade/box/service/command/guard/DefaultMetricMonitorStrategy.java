...
public class DefaultMetricMonitorStrategy implements MonitorStrategy, GuardExecutableInterface {
  ...
  public ExperimentGuardMonitorMetricResultEntity monitor(
    ...
    return requireExperimentGuardMonitorMetricResultEntity(
        guardResultLoadRequest.getExperimentGuardInstanceDO(), guardResultLoadRequest);
  }
  ...
  private ExperimentGuardMonitorMetricResultEntity requireExperimentGuardMonitorMetricResultEntity(
    ...
    Date startTime =
        experimentGuardInstanceDO.getValue().getLastRecordPoint() == null
            ? experimentTaskDO.getGmtCreate()
            : new Date(experimentGuardInstanceDO.getValue().getLastRecordPoint());
    Date toTime = experimentTaskDO.getGmtEnd() == null ? new Date() : experimentTaskDO.getGmtEnd();
    Map<Scope, List<ChaosMetricEntity>> metricEntities =
        invokeMetricMiniApp(
            experimentGuardResultLoadRequest.getExperimentTaskDO(),
            experimentGuardInstanceDO.getAppCode(),
            experimentGuardInstanceDO,
            experimentGuardInstanceDO.getArgument().getArguments(),
            startTime,
            DateUtils.addSeconds(toTime, METRIC_DATE_COLLECT_TIME_OFFSET),
            experimentGuardResultLoadRequest.getHosts());
    // 结果转换成list
    List<ExperimentGuardMetricDataItem> data =
        metricEntities.values().stream()
            .flatMap(
                (Function<List<ChaosMetricEntity>, Stream<ExperimentGuardMetricDataItem>>)
                    chaosMetricEntities ->
                        chaosMetricEntities.stream()
                            .map(
                                chaosMetricEntity -> {
                                  ...
                                  experimentGuardMetricDataItem.setValue(
                                      chaosMetricEntity.getValue());
                                  experimentGuardMetricDataItem.setTimestamp(
                                      chaosMetricEntity.getTimestamp());
                                  experimentGuardMetricDataItem.setGroup(
                                      chaosMetricEntity.getHost().getIp());
                                  experimentGuardMonitorMetricResultEntity.setUnit(
                                      chaosMetricEntity.getUnit());
  ...
  private void fillSubName(
      ExperimentGuardMonitorMetricResultEntity experimentGuardMonitorMetricResultEntity,
      List<SceneArgumentDefinition> arguments) {
    arguments.stream()
        .filter(sceneArgumentDefinition -> "metric".equals(sceneArgumentDefinition.getAlias()))
        .forEach(
            new Consumer<SceneArgumentDefinition>() {
              @Override
              public void accept(SceneArgumentDefinition sceneArgumentDefinition) {
                experimentGuardMonitorMetricResultEntity.setSubName(
  ...
  @Override
  public ChaosAppResponse internalInvoke(
      ChaosAppContext chaosAppContext,
      ChaosAppRequest chaosAppRequest,
      String appCode,
      ExperimentGuardInstanceDO experimentGuardInstanceDO) {
    if (MiniAppUtils.JVM_HIT_COUNT.equals(appCode)) {
      return handleJvmHitCount(
          chaosAppContext, chaosAppRequest, appCode, experimentGuardInstanceDO);
    } else {
      return handleCommonInternalInvoke(
          chaosAppContext, chaosAppRequest, appCode, experimentGuardInstanceDO);
    }
  }
   ...
   * @return
   */
  private ChaosAppResponse handleJvmHitCount(
      ChaosAppContext chaosAppContext,
      ChaosAppRequest chaosAppRequest,
      String appCode,
      ExperimentGuardInstanceDO experimentGuardInstanceDO) {
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        chaosBladeExpUidRepository.findLastByActivityTaskIdAndHost(
            experimentGuardInstanceDO.getActivityTaskId(), chaosAppRequest.getScope().getIp());
  ...
  public ChaosAppResponse handleCommonInternalInvoke(
      ChaosAppContext chaosAppContext,
      ChaosAppRequest chaosAppRequest,
      String appCode,
      ExperimentGuardInstanceDO experimentGuardInstanceDO) {
    return ChaosAppInvoker.invokeByGuard(
        chaosAppContext, chaosAppRequest, appCode, experimentGuardInstanceDO);
  }
}
