...
public class QueryActivityTaskMetricCommand
  ...
  @Override
  public Response<List<MetricResultEntity>> execute(
      BaseActivityTaskRequest baseActivityTaskRequest) {
    ...
    experimentExecuteContext
        .getContextData()
        .add(
            ActivityTaskExecutionAttributes.ATTRIBUTE_KEY_METRIC_RELOAD,
  ...
  private Stream<MetricResultEntity> getMetricStream(String data) {
    Map<String, List<ChaosMetricEntity>> result =
        JSON.parseObject(data, new TypeReference<Map<String, List<ChaosMetricEntity>>>() {});
    if (result == null) {
      return Stream.empty();
    }
    return result.getOrDefault("response", new ArrayList<>()).stream()
        .map(
            new Function<ChaosMetricEntity, MetricResultEntity>() {
              @Override
  ...
  private Stream<MetricResultEntity> getHitCountStream(String data) {
    Map<String, List<RequestHit>> result =
        JSON.parseObject(data, new TypeReference<Map<String, List<RequestHit>>>() {});
    return result.getOrDefault("response", new ArrayList<>()).stream()
        .map(
            new Function<RequestHit, MetricResultEntity>() {
              @Override
