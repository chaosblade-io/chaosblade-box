...
public class ActivityTaskBatchCreatedCommand
  ...
  @Autowired private ExperimentTaskHostRecorder experimentTaskHostRecorder;

  @Autowired private ExperimentMiniFlowGroupRepository experimentMiniFlowGroupRepository;

  @Autowired private ActivityTaskHostPercentIntercept activityTaskHostPercentIntercept;
  ...
  public List<ActivityTaskDO> execute(ActivityTasksCreateRequest activityTasksCreateRequest) {
    ...
    for (ExperimentActivityDO experimentActivityDO : experimentActivityDOS) {
      ActivityTaskDO activityTaskDO =
          buildBaseActivityTaskDO(
              experimentTaskDO,
              experimentActivityDO,
              activityIdToRunParam.get(experimentActivityDO.getActivityId()),
              activityTaskOrder);
    ...
    for (ActivityTaskDO activityTaskDO : activityTaskDOS) {
      ...
      if (PhaseType.ATTACK.equals(activityTaskDO.getPhase())
          || PhaseType.PREPARE.equals(activityTaskDO.getPhase())) {
      ...
      experimentMiniAppTaskDOS.addAll(
          batchCreateExperimentMiniAppTasks(activityTaskDO, uniqueHosts));
    }
  ...
  private List<ExperimentMiniAppTaskDO> batchCreateExperimentMiniAppTasks(
      ActivityTaskDO activityTaskDO, Set<Scope> uniqueHosts) {
    ActivityRunParam activityRunParam = activityTaskDO.getRunParam();
    if (activityRunParam == null) {
      return new ArrayList<>();
    }
    return Optional.ofNullable(activityRunParam.getScope()).orElse(new ArrayList<>()).stream()
        .map(
            host -> {
              ExperimentMiniAppTaskDO experimentMiniAppTaskDO =
                  miniAppTaskManager.initExperimentMiniAppTaskDO(activityTaskDO, host);
   ...
   */
  private List<ExperimentActivityDO> sortExperimentActivityDOS(
  ...
  private List<ExperimentActivityDO> sortByActivityMode(
      ExperimentDO experimentDO,
      List<ExperimentActivityDO> experimentActivityDOS,
    ...
    if (!experimentDO.isNewExperimentDefinition()) {
      return experimentActivityDOS.stream()
          .sorted((o1, o2) -> ActivityComparator.INSTANCE.compare(o1, o2))
          .collect(Collectors.toList());
    } else {
      // 新流程的话排序比较麻烦
      List<MiniFlowDO> miniFlowDOS =
          experimentMiniFlowRepository.findByExperimentId(experimentDO.getExperimentId());
      Map<String, MiniFlowDO> flowIdToFlow =
          miniFlowDOS.stream().collect(Collectors.toMap(MiniFlowDO::getFlowId, e -> e));
          ...
          .sorted(
              (o1, o2) -> {
                ...
                if (experimentRunModeEnum.equals(ExperimentRunModeEnum.PHASE)) {
                  ...
                  if (compareResult == 0) {
                    compareResult =
                        CompareUtils.integerCompare(o1Flow.getGroupOrder(), o2Flow.getGroupOrder());
                    if (compareResult == 0) {
                      compareResult =
                          CompareUtils.integerCompare(o1Flow.getFlowOrder(), o2Flow.getFlowOrder());
                    }
                  }
                } else {
                  compareResult =
  ...
  private Map<String, ActivityRunParam> buildActivityIdToRunParamMap(
  ...
  private ActivityTaskDO buildBaseActivityTaskDO(
      ExperimentTaskDO experimentTaskDO,
      ExperimentActivityDO experimentActivityDO,
      ActivityRunParam activityRunParam,
      int activityOrder) {
