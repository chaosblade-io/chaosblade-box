...
public class ExpertiseSearchCommand
  ...
  @Override
  public Response<PageQueryResponse<ExpertiseView>> execute(
    ...
    pageQueryResponse.setContent(
        expertiseDOIPage.getRecords().stream()
            .map(
                new Function<ExpertiseDO, ExpertiseView>() {
                  @Override
  ...
  private ExpertiseView convertExpertiseView(ExpertiseDO expertiseDO) {
    ...
    expertiseView.setTags(
        tagManager.findTagsByExpertiseId(expertiseDO.getExpertiseId()).stream()
            .map(TagDO::getName)
  ...
  private ExpertiseFlowView buildExpertiseFlowView(ExpertiseDO expertiseDO) {
    ...
    if (experimentFlowInfo == null) {
      log.info(
          "not exist or error when find flow info by experiment by expertiseId:{},experimentId:{}",
  ...
  private List<ExpertiseFlowActivity> buildActivityList(ExperimentFlowInfo experimentFlowInfo) {
    ...
    if (ExperimentRunModeEnum.PHASE.equals(experimentRunModeEnum)) {
      Map<PhaseType, List<ExperimentActivityInfo>> flowActivitiesByPhaseType =
          groupFlowActivityByPhaseType(miniFlowGroups);
      flowActivitiesByPhaseType
          .getOrDefault(PhaseType.PREPARE, new ArrayList<>())
          .forEach(
              experimentActivityInfo ->
                  appendActivity(
                      expertiseFlowActivities, PhaseType.PREPARE, experimentActivityInfo));
      flowActivitiesByPhaseType
          .getOrDefault(PhaseType.ATTACK, new ArrayList<>())
          .forEach(
              experimentActivityInfo ->
                  appendActivity(
                      expertiseFlowActivities, PhaseType.ATTACK, experimentActivityInfo));
      flowActivitiesByPhaseType
          .getOrDefault(PhaseType.CHECK, new ArrayList<>())
          .forEach(
              experimentActivityInfo ->
                  appendActivity(expertiseFlowActivities, PhaseType.CHECK, experimentActivityInfo));
      flowActivitiesByPhaseType
          .getOrDefault(PhaseType.RECOVER, new ArrayList<>())
          .forEach(
              experimentActivityInfo ->
                  appendActivity(
                      expertiseFlowActivities, PhaseType.RECOVER, experimentActivityInfo));
    }
        ...
        for (MiniFlow miniFlow : miniFlowGroup.getFlows()) {
          ...
          miniFlow
              ...
              .forEach(
                  experimentActivityInfo ->
                      appendActivity(
                          expertiseFlowActivities, PhaseType.CHECK, experimentActivityInfo));
          miniFlow
              .getRecover()
              .forEach(
                  experimentActivityInfo ->
  ...
  private Map<PhaseType, List<ExperimentActivityInfo>> groupFlowActivityByPhaseType(
    ...
    for (MiniFlowGroup miniFlowGroup : miniFlowGroups) {
      for (MiniFlow miniFlow : miniFlowGroup.getFlows()) {
        phaseTypeListMap
            .computeIfAbsent(PhaseType.PREPARE, phaseType -> new ArrayList<>())
            .addAll(miniFlow.getPrepare());
        phaseTypeListMap
            .computeIfAbsent(PhaseType.ATTACK, phaseType -> new ArrayList<>())
            .addAll(miniFlow.getAttack());
        phaseTypeListMap
            .computeIfAbsent(PhaseType.CHECK, phaseType -> new ArrayList<>())
            .addAll(miniFlow.getCheck());
        phaseTypeListMap
            .computeIfAbsent(PhaseType.RECOVER, phaseType -> new ArrayList<>())
            .addAll(miniFlow.getRecover());
      }
    }
  ...
  private void appendActivity(
      List<ExpertiseFlowActivity> expertiseFlowActivities,
      PhaseType phaseType,
