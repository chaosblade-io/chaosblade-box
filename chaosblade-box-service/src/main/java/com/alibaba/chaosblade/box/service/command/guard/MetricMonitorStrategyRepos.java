...
public class MetricMonitorStrategyRepos {
  ...
  @Autowired private List<MonitorStrategy> monitorStrategyList;
  ...
  public Optional<MonitorStrategy> select(
      ExperimentGuardResultLoadRequest experimentGuardResultLoadRequest) {
    return monitorStrategyList.stream()
        .filter(
            new Predicate<MonitorStrategy>() {
              @Override
