...
public class SettingServiceImpl implements SettingService {
  ...
  @Autowired private AgentForChaos agentForChaos;
  ...
  public Map<String, String> queryAgentInstallCommandByMode(
    ...
    if (InstallMode.host.name().equalsIgnoreCase(mode.name())) {
      ...
      StringBuilder sb =
          buildAgentInstallCommandByHelmVersion(userId, namespace, helmVersion, lang);
      result.put(COMMAND_INSTALL, sb.toString());
      return result;
    }
    return null;
  ...
  private StringBuilder buildBasicAgentInstallCommandForHost(
      String userId, String namespace, String lang) {
  ...
  private StringBuilder buildAgentInstallCommandByHelmVersion(
      String userId, String namespace, String helmVersion, String lang) {
    UserDo userDo = userRepository.getById(userId);
    if (userDo == null) {
      return new StringBuilder();
    }
      ...
      switch (helmVersion) {
        ...
        case "v3":
          ...
          if (lang.equals(ChaosConstant.LANGUAGE_EN)) {
            ...
            sb.append(
                ",controller.cluster_id={replace with cluster id, value No special requirements},controller.cluster_name={replace with cluster name, value No special requirements}");
          } else {
            sb.append(",transport.endpoint={替换为box的 ip:port}");

            sb.append(
                ",controller.cluster_id={替换为集群id，取值无特殊要求},controller.cluster_name={替换为集群名字，取值无特殊要求}");
          }
          break;
        default:
  ...
  private StringBuilder buildAgentInstallCommandByHelm(
      String userId, String namespace, String license) {
    StringBuilder sb = new StringBuilder("helm install");
    sb.append(" --set");
    sb.append(" env.name=");
    sb.append(namespace);
  ...
  private String buildAgentUnInstallCommand(
      String version, String installMode, DeviceOsType osType) {
    // host
  ...
  public Boolean uninstallAgent(String userId, SettingRequest settingRequest) {
    DeviceDO deviceDO =
        deviceRepository.findByConfigurationIdAndUserIdNamespace(
            userId, settingRequest.getConfigurationId(), settingRequest.getNamespace());
  ...
  public Response<String> installAgentForHost(
    ...
    DeviceDO deviceDO =
        deviceRepository.findActiveByPrivateIp(installAgentForHostRequest.getInstanceId());
    if (null != deviceDO) {
    ...
    String command =
        String.format(
            ...
            mgrRequest.getAppName(),
            mgrRequest.getAppGroupName(),
            mgrRequest.getChaosboxEndpoint());
  ...
  public PluginDTO queryAgentPluginDetail(String userId, String namespace, String deviceId) {
    DeviceDO deviceDO =
        deviceRepository.getDeviceByDeviceId(
            userId, namespace, deviceId, DeviceType.HOST.getType());
    if (deviceDO != null) {
      Date deviceOfflineTime = DateUtils.addMinutes(new Date(), -1);

      return convertHostDeviceToPlugin(deviceDO, PluginType.CHAOS_AGENT.name(), deviceOfflineTime);
    }

    return null;
  }

  private PluginDTO convertHostDeviceToPlugin(
      DeviceDO deviceDO, String pluginType, Date deviceOfflineTime) {
    ...
    if (deviceDO.getLastHealthPingTime() != null
        && deviceDO.getLastHealthPingTime() < deviceOfflineTime.getTime()) {
