...
public class ExperimentTaskQueryCommand
  ...
  public Response<ExperimentTask> execute(ExperimentTaskQueryRequest experimentTaskQueryRequest) {
    ...
    experimentTaskUtil.fillBaseInfo(
        experimentTask, experimentTaskDOResponse.getResult(), experimentDO);
  ...
  private void fillPhaseInfo(ExperimentTask experimentTask) {
    List<ActivityTaskDO> activityTaskDOS =
        activityTaskRepository.findByExperimentTaskId(experimentTask.getTaskId());
    Map<String, List<ActivityTaskDO>> activityIdToTasks =
        activityTaskDOS.stream().collect(Collectors.groupingBy(ActivityTaskDO::getActivityId));
    List<ActivityTask> activityTasks =
        activityIdToTasks.values().stream()
            .map(
                taskDOS -> {
                  ActivityTaskDO activityTaskDO =
                      taskDOS.stream()
                          .max(Comparator.comparingLong(o -> o.getGmtCreate().getTime()))
                          .orElse(null);
                  if (activityTaskDO == null) {
                    return null;
                  }
                  return activityTaskManager
                      .querySimpleActivityTaskInfo(activityTaskDO)
                      .getResult();
                })
    ...
    List<PhaseInfo> phaseInfos =
        activityTasks.stream()
            .collect(Collectors.groupingBy(ActivityTask::getPhase))
            .entrySet()
            .stream()
            .map(
                phaseTypeListEntry ->
                    new PhaseInfo(phaseTypeListEntry.getKey(), phaseTypeListEntry.getValue()))
            .sorted(Comparator.comparingInt(o -> o.getPhase().getType()))
            .collect(Collectors.toList());
