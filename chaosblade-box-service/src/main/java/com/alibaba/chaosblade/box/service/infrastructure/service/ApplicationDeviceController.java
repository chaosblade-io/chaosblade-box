...
public class ApplicationDeviceController {
  ...
  @Resource private ApplicationDeviceRepository applicationDeviceRepository;

  @Resource private UserTopologyConfigService userTopologyConfigService;
   ...
   * @param resources
   */
  public Map<String, String> insertOrUpdateByPod(
      String userId, String environment, List<DeviceRealDTO> resources) {
    if (CollectionUtils.isNotEmpty(resources)) {
      Map<String, String> podToAppNameMap = new HashMap<>(resources.size());
      try {
        UserTopologyConfig userTopologyConfig =
            userTopologyConfigService.queryConfigContentWithCache(userId, environment);
        resources.stream()
            .forEach(
                podInfo -> {
                  String appName =
                      handlerPodAppInfo(userId, podInfo, userTopologyConfig.getUserAppConfig());
  ...
  private String handlerPodAppInfo(
      String userId, DeviceRealDTO podRealDTO, UserAppConfig userAppConfig) {
    KubPodExtInfoDTO kubPodExtInfoDTO =
        JSON.parseObject(podRealDTO.getExtInfo(), KubPodExtInfoDTO.class);
   ...
   */
  private void persistencePodAppInfo(
      String userId,
      String appName,
      String appGroupName,
      DeviceRealDTO podRealDTO,
      KubPodExtInfoDTO kubPodExtInfoDTO) {
    if (podRealDTO.getStatus() != null
        && DeviceStatus.ONLINE.getStatus() == podRealDTO.getStatus()) {
   ...
   * @param appGroupName
   */
  public void insertOrUpdateByHost(
      String appName, String appGroupName, List<DeviceDO> deviceLists) {
