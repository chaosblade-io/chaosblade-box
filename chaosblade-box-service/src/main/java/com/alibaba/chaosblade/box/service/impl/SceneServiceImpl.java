...
public class SceneServiceImpl
    implements SceneService, ApplicationListener<ContextRefreshedEvent>, InitializingBean {

  @Resource protected SceneRepository sceneRepository;

  @Resource protected SceneFunctionRepository sceneFunctionRepository;

  @Resource protected SceneFunctionParameterRepository sceneFunctionParameterRepository;

  @Resource private ChaosAppSynchronizer chaosAppSynchronizer;

  @Resource private Trackers trackers;

  @Resource private ApplicationStartUpConfig applicationStartUpConfig;
  ...
  public SceneDO queryScene(String sceneId) {
    return sceneRepository
        ...
        .map(
            scene -> {
              scene.setFunctions(
                  sceneFunctionRepository.findBySceneId(scene.getSceneId()).stream()
                      .peek(
                          function ->
  ...
  @Override
  public PageableResponse<SceneDO> queryScenes(
      int pageNo, int pageSize, SceneQueryRequest queryRequest) {
    PageableQueryWrapper<SceneQueryRequest> query =
