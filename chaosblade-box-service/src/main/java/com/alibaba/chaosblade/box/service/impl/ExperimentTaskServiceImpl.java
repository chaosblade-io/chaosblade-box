...
public class ExperimentTaskServiceImpl implements ExperimentTaskService, InitializingBean {
  ...
  @Autowired private ExperimentTaskManager experimentTaskManager;

  @Autowired private ExperimentTaskSimpleInfoFinder experimentTaskSimpleInfoFinder;
  ...
  @Resource private ExperimentRepository experimentRepository;

  @Autowired private CommandBus commandBus;

  @Resource private ExperimentChecker experimentChecker;
  ...
  public Response<ExperimentTask> getExperimentTaskFullInfo(String taskId) {
    return commandBus.syncRun(
        ExperimentTaskQueryCommand.class, new ExperimentTaskQueryRequest(taskId));
  }
  ...
  public Response<ExperimentTaskSummary> getExperimentTaskSummary(String taskId) {
    return commandBus.syncRun(
        ExperimentTaskSummaryQueryCommand.class, new ExperimentTaskQueryRequest(taskId));
  }

  @Override
  public Response<List<ExperimentTask>> getExperimentTasksByOuterId(String outerId) {
    List<ExperimentTask> experimentTasks =
        experimentTaskRepository.findByOuterId(outerId).stream()
            .map(
                experimentTaskDO ->
                    getExperimentTaskFullInfo(experimentTaskDO.getTaskId()).getResult())
  ...
  public Response<List<ExperimentTask>> getExperimentTasksByExperimentId(String experimentId) {
    List<ExperimentTask> experimentTasks =
        experimentTaskRepository.findByExperimentId(experimentId).stream()
            .map(
                experimentTaskDO ->
                    getExperimentTaskFullInfo(experimentTaskDO.getTaskId()).getResult())
            .collect(Collectors.toList());
  ...
  public Response<ExperimentTaskSimple> getExperimentTaskSimpleInfo(String taskId) {
    Response<ExperimentTaskDO> experimentTaskDOResponse =
        experimentChecker.assertExperimentTaskExist(taskId);
    ...
    return Response.okWithData(
        experimentTaskSimpleInfoFinder.findByExperimentTaskDO(
            experimentTaskDOResponse.getResult()));
  }
  ...
  public Response<List<ExperimentTaskSimple>> getExperimentTasksSimpleByExperimentId(
    ...
    if (null != startTime && null != endTime) {
      experimentTaskDOS =
          experimentTaskRepository.findFinishedTasksBetweenByExperimentId(
    ...
    List<ExperimentTaskSimple> experimentTaskSimples =
        experimentTaskDOS.stream()
            .map(
                experimentTaskDO ->
                    experimentTaskSimpleInfoFinder.findByExperimentTaskDO(
                        experimentDO, experimentTaskDO))
            .collect(Collectors.toList());
  ...
  @Override
  public Response<Map<String, Integer>> getExperimentTaskResultCount(
      ExperimentTaskRequest experimentTaskRequest) {
    List<ExperimentTaskSimple> experimentTaskSimples =
        this.getExperimentTasksSimpleByExperimentId(experimentTaskRequest).getResult();
    int successCount = 0;
  ...
  @Override
  public Response<Map<Long, Integer>> getExperimentTaskCount(
      ExperimentTaskRequest experimentTaskRequest) {
    Date startTime = experimentTaskRequest.getStartTime();
    Date endTime = experimentTaskRequest.getEndTime();
  ...
  @Override
  public Response<ExperimentTaskControl> getControlList(
      ExperimentTaskRequest experimentTaskRequest) {
    ExperimentTaskResult experimentTaskResult =
        this.getExperimentTasksResultByExperimentId(experimentTaskRequest).getResult();

    ExperimentTaskControl experimentTaskControl = new ExperimentTaskControl();
    experimentTaskControl.setResultStats(
        Lists.newArrayList(
            ResultEnum.SUCCESS, ResultEnum.FAILED, ResultEnum.ERROR, ResultEnum.STOPPED));
    ...
    experimentTaskControl.setAppDescs(
        sceneFunctionRepository
            .findNamesByCode(Lists.newArrayList(experimentTaskResult.getAppDescs()))
            .orElse(null));
    return Response.okWithData(experimentTaskControl);
  }

  @Override
  public Response<Map<Long, Double>> getExperimentTaskCost(
      ExperimentTaskRequest experimentTaskRequest) {
    Date startTime = experimentTaskRequest.getStartTime();
    Date endTime = experimentTaskRequest.getEndTime();
    ...
    if (null == experimentTaskSimples || 0 == experimentTaskSimples.size()) {
      ...
      for (ExperimentTaskSimple experimentTaskSimple : experimentTaskSimples) {
        ...
        BigDecimal bd =
            BigDecimal.valueOf(
                (experimentTaskSimple.getEndTime().getTime()
                        - experimentTaskSimple.getStartTime().getTime())
                    / 60000.0);
        result.put(
            experimentTaskSimple.getStartTime().getTime() + cnt,
  ...
  @Autowired private Trackers trackers;

  @Override
  public Response<Void> stopTask(
  ...
  @Override
  public Response<Void> adminStopTask(
      ChaosUser user, String experimentTaskId, ExperimentTaskStopOption experimentTaskStopOption) {
  ...
  @Override
  public Response<Void> stopAllTasks(
      ChaosUser user, BaseExperimentTaskRequest baseExperimentTaskRequest) {
    ...
    experimentTaskQuery.setStates(
        Lists.newArrayList(StateEnum.RUNNING.getValue(), StateEnum.SUSPEND.getValue()));
    ...
    experimentTaskDOS.forEach(
        experimentTaskDO ->
            experimentTaskStopThreadPool.submit(
                () -> {
                  experimentTaskManager.stopTask(
                      user, experimentTaskDO.getTaskId(), experimentTaskStopOption);
                }));
  ...
  public PageQueryResponse<BaseExperimentTask> pageableQueryExperimentTaskSummary(
    ...
    return commandBus.syncRun(
        ExperimentTaskPageableQueryCommand.class, experimentPageableQueryRequest);
  }
  ...
  public PageQueryResponse<BaseExperimentTask> pageableQueryExperimentTaskByUserId(
    ...
    return commandBus.syncRun(
        ExperimentTaskPageableQueryByUserIdCommand.class, experimentTaskPageableQueryRequest);
  }

  @Override
  public void afterPropertiesSet() throws Exception {
    experimentTaskStopThreadPool =
  ...
  private List<ExperimentTaskSimple> filterTaskSimples(
      List<ExperimentTaskSimple> experimentTaskSimples,
      ExperimentTaskRequest experimentTaskRequest) {
    ...
    return experimentTaskSimples.stream()
        .filter(
            experimentTaskSimple -> {
              if (!StateEnum.FINISHED.equals(experimentTaskSimple.getState())) {
                return false;
              }
              ResultEnum result = experimentTaskSimple.getResult();
              if (!CollectionUtil.isNullOrEmpty(aoneApps)
                  && !judgeCross(experimentTaskSimple.getAoneApps(), aoneApps)) {
                return false;
              }
              if (!CollectionUtil.isNullOrEmpty(appDescs)
                  && !judgeCross(experimentTaskSimple.getAppDescs(), appDescs)) {
                return false;
              }
              if (!CollectionUtil.isNullOrEmpty(resultEnums)) {
                if (ResultEnum.FAILED.equals(result)
                    || ResultEnum.SOPPED_FAILED.equals(result)
