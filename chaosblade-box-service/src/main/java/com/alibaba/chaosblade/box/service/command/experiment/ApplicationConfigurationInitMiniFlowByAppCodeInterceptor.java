...
@Slf4j
@Component
public class ApplicationConfigurationInitMiniFlowByAppCodeInterceptor
    implements InitMiniFlowByAppCodeInterceptor {
  ...
  public void afterInit(
    ...
    if (initMiniFlowRequest.getAppId() == null) {
      return;
    }
    Long appId = initMiniFlowRequest.getAppId();
    List<ApplicationConfiguration> applicationConfigurationList =
        getApplicationConfigurationList(initMiniFlowRequest, appId);
    Map<String, List<ExperimentActivityInfo>> phaseToExperimentActivityInfo =
        initMiniFlowByAppCodeInterceptorContext.getResponse();
    for (Map.Entry<String, List<ExperimentActivityInfo>> entry :
        phaseToExperimentActivityInfo.entrySet()) {
      PhaseType phaseType = PhaseType.valueOf(entry.getKey().toUpperCase());
      List<ApplicationConfiguration> configurationsSupportPhases =
          filterConfigurationsByPhase(applicationConfigurationList, phaseType);
      if (configurationsSupportPhases.isEmpty()) {
        continue;
      }
      for (ExperimentActivityInfo experimentActivityInfo : entry.getValue()) {
        List<ApplicationConfiguration> configurationsSupportAppCode =
            getConfigurationsSupportAppCode(configurationsSupportPhases, experimentActivityInfo);
        if (configurationsSupportAppCode.isEmpty()) {
          continue;
        }
        for (SceneArgumentDefinition sceneArgumentDefinition :
            sceneArgumentGradeConverter.tranToArgumentsList(
                experimentActivityInfo.getArguments())) {
          List<ApplicationConfiguration> configurationsSupportAlias =
              getConfigurationsSupportAlias(configurationsSupportAppCode, sceneArgumentDefinition);
          if (configurationsSupportAlias.isEmpty()) {
            continue;
          }
          for (ApplicationConfiguration applicationConfiguration : configurationsSupportAlias) {
            if (!Strings.isNullOrEmpty(applicationConfiguration.getValue())) {
              sceneArgumentDefinition
                  .getComponent()
                  .setDefaultValue(applicationConfiguration.getValue());
            }
          }
        }
      }
   ...
   * @return
   */
  @NotNull private List<ApplicationConfiguration> getApplicationConfigurationList(
      InitMiniFlowRequest initMiniFlowRequest, Long appId) {
    return applicationConfigurationProvider.provideApplicationConfigurations(appId).stream()
        .filter(
            new Predicate<ApplicationConfiguration>() {
              ...
              public boolean test(ApplicationConfiguration applicationConfiguration) {
                if (null == applicationConfiguration.getScope()
                    || null == applicationConfiguration.getScope().getNodeGroups()) {
                  return true;
                }
   ...
   * @return
   */
  @NotNull private List<ApplicationConfiguration> getConfigurationsSupportAlias(
      List<ApplicationConfiguration> configurationsSupportAppCode,
      SceneArgumentDefinition sceneArgumentDefinition) {
    return configurationsSupportAppCode.stream()
        .filter(
            new Predicate<ApplicationConfiguration>() {
              @Override
              public boolean test(ApplicationConfiguration applicationConfiguration) {
                return applicationConfiguration
                    .getFunctionParamAlias()
                    .equals(sceneArgumentDefinition.getAlias());
              }
            })
        .collect(Collectors.toList());
  }
   ...
   * @return
   */
  @NotNull private List<ApplicationConfiguration> getConfigurationsSupportAppCode(
      List<ApplicationConfiguration> configurationsSupportPhases,
      ExperimentActivityInfo experimentActivityInfo) {
    return configurationsSupportPhases.stream()
        .filter(
            new Predicate<ApplicationConfiguration>() {
              @Override
              public boolean test(ApplicationConfiguration applicationConfiguration) {
  ...
  @NotNull private List<ApplicationConfiguration> filterConfigurationsByPhase(
      List<ApplicationConfiguration> applicationConfigurationList, PhaseType phaseType) {
    return applicationConfigurationList.stream()
        .filter(
            new Predicate<ApplicationConfiguration>() {
              @Override
              public boolean test(ApplicationConfiguration applicationConfiguration) {
