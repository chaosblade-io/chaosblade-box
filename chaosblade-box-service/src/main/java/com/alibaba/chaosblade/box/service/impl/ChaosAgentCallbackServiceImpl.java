...
public class ChaosAgentCallbackServiceImpl implements ChaosAgentCallbackService {
  ...
  @Autowired private CommandBus commandExecutorPool;
  ...
  @Autowired private PodReportHandler podReportHandler;
  ...
  public Response<String> handleJavaAgentInstall(AgentMetricRequest agentMetricRequest) {
    ...
    // 根据uid查询当前java安装命令
    log.info(
        "receive java agent install response,expuid:{},status:{},error:{}", uid, status, error);
    if (uid == null) {
    ...
    if (ChaosBladeInvoker.STATUS_RUNNING.equals(status)) {
      ...
      Response<PrepareStatusBean> response =
          chaosBladeInvoker.getPrepareStatus(
    ...
    // 获取小程序的执行记录
    Optional<ExperimentMiniAppTaskDO> activityTargetTaskDOOptional =
        activityTargetExecutionResultRepository.findById(chaosBladeExpUidDO.getAppExecutionId());
    if (activityTargetTaskDOOptional.isPresent()) {
      commandExecutorPool.syncRun(
          JavaAgentInstallCheckCommand.class, activityTargetTaskDOOptional.get());
      return Response.ofSuccess(null);
    } else {
      log.warn(
          "not found activity target task  record by taskid:{}",
          chaosBladeExpUidDO.getAppExecutionId());
      return Response.ofSuccess(null);
    }
  }
  ...
  public Response<String> handlerChaosbladeAsync(ChaosbladeAsyncRequest chaosbladeAsyncRequest) {
    ...
    log.info(
        "[ActivityAsyncCallbackHandler] receive async request,requestId:{},configurationId:{}",
        requestId,
        chaosbladeAsyncRequest.getConfigurationId());

    if (uid.isEmpty() || status.isEmpty()) {
      log.warn(
          "[ActivityAsyncCallbackHandler] failed, requestId:{}, error:{}",
          requestId,
          "uid || status Required");
    ...
    ExperimentMiniAppTaskDO experimentMiniAppTaskDO =
        activityTargetExecutionResultRepository.findById(requestId).orElse(null);
    if (experimentMiniAppTaskDO == null) {
      log.warn(
          "[ActivityAsyncCallbackHandler] Failed,requestId:{},error:{}",
          requestId,
          "not found miniAppTask record");
      return Response.ofFailure(Response.Code.Parameter_Empty, "not found injection record");
    }
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        getChaosBladeExpUid(uid, requestId, experimentMiniAppTaskDO);
    if (chaosBladeExpUidDO == null) {
      log.warn(
          "[ActivityAsyncCallbackHandler] Failed,requestId:{},error:{}",
          requestId,
          "not found injection record");
      return Response.ofFailure(Response.Code.Parameter_Empty, "not found injection record");
    }
    consumeRecord(
            uid,
            ...
            status,
            chaosbladeAsyncRequest.getError(),
            chaosbladeAsyncRequest.getToolType())
        .accept(chaosBladeExpUidDO);
    return Response.ofSuccess(null);
  }

  private Consumer<ChaosBladeExpUidDO> consumeRecord(
      String uid,
      ExperimentMiniAppTaskDO experimentMiniAppTaskDO,
      String status,
      String error,
    ...
    return chaosBladeExpUidDO ->
        doCallback(chaosBladeExpUidDO, experimentMiniAppTaskDO, uid, status, error, toolType);
  }
  ...
  private void doCallback(
      ...
      String toolType) {
    threadPool
        .executor(THREAD_AGENT_ASYNC_CALLBACK)
        .execute(
            new Runnable() {
              @Override
              public void run() {
                commandBus.syncRun(
                    ActivityAsyncCheckCommand.class,
                    new AsyncCallBackContext(
                        experimentMiniAppTaskDO, chaosBladeExpUidDO, uid, status, error, toolType));
              }
            });
  }

  /**
   * 1.首先根据UID查询 2.根据UID查询不到,则有可能下发规则超时导致没获取到,因此要通过requestId和cid来获取
   *
   * @param uid agent的注入ID，类似于expId
   * @param requestId 请求id
   * @return
   */
  private ChaosBladeExpUidDO getChaosBladeExpUid(
      String uid, String requestId, ExperimentMiniAppTaskDO experimentMiniAppTaskDO) {
    ChaosBladeExpUidDO chaosBladeExpUidDO =
        chaosBladeExpUidRepository.findForAsyncCallback(uid, requestId).orElse(null);
    if (chaosBladeExpUidDO == null) {
      log.warn("not found chaosblade expId record by uid:{} or request id:{}", uid, requestId);
      DeviceDO deviceDO = getDevice(experimentMiniAppTaskDO);
      if (deviceDO == null) {
        return null;
      }
      chaosBladeExpUidDO =
          chaosBladeExpUidRepository.createChaosBladeExpRecord(
              uid, experimentMiniAppTaskDO, deviceDO.getPrivateIp(), true);
  ...
  private DeviceDO getDevice(ExperimentMiniAppTaskDO experimentMiniAppTaskDO) {
    DeviceDO deviceDO =
        deviceRepository.findByConfigurationId(experimentMiniAppTaskDO.getDeviceConfigurationId());
    if (deviceDO == null) {
      log.error(
          "not found device by configurationId:{}",
