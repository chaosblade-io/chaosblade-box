...
class SettingController extends BaseController {
  ...
  public RestResponse<Map<String, String>> queryInstallCommand(
    ...
    Map<String, String> returnT =
        settingService.queryAgentInstallCommandByMode(
            userId,
            settingQueryInstallRequest.getNamespace(),
            installMode,
            deviceOsType,
            settingQueryInstallRequest.getHelmVersion(),
            settingQueryInstallRequest.getLang());

    return RestResponseUtil.okWithData(returnT);
  }

  @RequestMapping(value = "QueryHelmPackageAddress", method = RequestMethod.POST)
  public RestResponse<String> queryHelmPackageAddress(
      @LoginUser ChaosUser user, HttpServletRequest request) {
    String userId = user.getUserId();
  ...
  public RestResponse<Map<String, String>> queryUninstallCommand(
      @LoginUser ChaosUser user, @RequestBody SettingRequest settingRequest) {

    String userId = user.getUserId();
    Map<String, String> uninstallCommand =
        settingService.queryAgentUnInstallCommand(
            userId, settingRequest.getNamespace(), settingRequest.getConfigurationId());
    return RestResponseUtil.okWithData(uninstallCommand);
  }

  @ApiOperation(value = "主机探针卸载")
  @PostMapping("Uninstall")
  public RestResponse<Boolean> uninstall(
      @LoginUser ChaosUser user, @RequestBody SettingRequest settingRequest) {
  ...
  public Response<String> installPlugin(
      @LoginUser ChaosUser user,
      @RequestBody InstallAgentForHostRequest installAgentForHostRequest) {
  ...
  public RestResponse<Boolean> uninstallPlugin(
      @LoginUser ChaosUser user, @RequestBody SettingRequest settingRequest) {
    ...
    return RestResponseUtil.failed(
        ChaosError.withCode(CommonErrorCode.B_CHAOS_TOOLS_UNINSTALL_ERROR));
  }

  @PostMapping(value = "QueryPluginStatus")
  public RestResponse<PluginDTO> queryPluginStatus(
      @LoginUser ChaosUser user, @RequestBody SettingQueryPluginStatus settingQueryPluginStatus) {
    String userId = user.getUserId();
    PluginDTO pluginDTOReturnT =
        settingService.queryAgentPluginDetail(
            userId,
            settingQueryPluginStatus.getNamespace(),
            settingQueryPluginStatus.getInstanceId());
