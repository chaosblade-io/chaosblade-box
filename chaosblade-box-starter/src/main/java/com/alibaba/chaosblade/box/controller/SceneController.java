...
public class SceneController extends BaseController {
  ...
  @Resource private SceneFunctionService sceneFunctionService;

  @Resource private SceneFunctionParameterService sceneFunctionParameterService;

  @Autowired private SceneDescriptionParser sceneDescriptionParser;

  @Autowired private SceneFunctionNameParser sceneFunctionNameParser;

  @PostMapping("QuerySceneFunctionByCategoryId")
  public RestResponse<PageableResponse<SceneFunctionVO>> querySceneFunctionByCategoryId(
      @LoginUser ChaosUser user, @RequestBody PageableSceneQueryRequest pageableRequest) {
  ...
  public RestResponse<List<SceneFunctionParameterVO>> getAllFunctionParameters(
      @LoginUser ChaosUser user, @RequestBody SceneQueryRequest sceneQueryRequest)
      throws PermissionDeniedException {
    return wrapResponse(
        sceneFunctionParameterService
            .queryFilterParametersByFunctionId(sceneQueryRequest.getFunctionId()).stream()
  ...
  public RestResponse<PageableResponse<SceneFunctionVO>> queryUserSceneFunctions(
      @LoginUser ChaosUser user, @RequestBody PageableRequest pageableRequest) {
    SceneQueryRequest queryRequest = new SceneQueryRequest();
    ...
    PageableResponse<SceneFunctionVO> functions =
        convertSceneFunctions(
            sceneFunctionService.querySceneFunctions(
                pageableRequest.getPage(), DEFAULT_PAGE_SIZE, queryRequest),
            pageableRequest.getLang());
  ...
  private List<SceneFunctionVO> convertSceneFunctions(List<SceneFunctionDO> functions) {
    ...
    return functions.stream().map(SceneFunctionVO::from).collect(Collectors.toList());
  }
  ...
  private PageableResponse<SceneFunctionVO> convertSceneFunctions(
    ...
    return PageableResponse.clone(pageableResponse, convertSceneFunctions(functions));
  }
