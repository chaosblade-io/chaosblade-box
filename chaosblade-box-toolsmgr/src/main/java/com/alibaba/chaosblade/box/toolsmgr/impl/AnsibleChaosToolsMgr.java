...
public class AnsibleChaosToolsMgr implements ChaosToolsMgr<MgrRequest> {
  ...
  public Response<String> deployAndInstallAgent(MgrRequest mgrRequest) {
    ...
    return ansibleExecutor(
        ansibleCommand.getCommand(mgrRequest), ansibleCommand, 3 * 60000, TimeUnit.MICROSECONDS);
  }
  ...
  public Response<String> testConnection(MgrRequest mgrRequest) {
    if (checkLocalHostAndAppend(mgrRequest)) {
      ...
      Response<String> pingResult1 =
          ansibleExecutor(ansiblePingCommand.getCommand(mgrRequest), ansiblePingCommand);
    ...
    return Response.ofFailure(
        Response.Code.INVALID_Parameter, "can not append to /etc/ansible/hosts");
  }
  ...
  private static Response<String> sendSSHKeyIfNotExitOnPassword(MgrRequest mgrRequest) {
    ...
    String port =
        mgrRequest.getInstancePort() == null
            ? ANSIBLE_LINUX_DEFAULT_PORT.toString()
            : mgrRequest.getInstancePort();

    String[] cmds =
        new String[] {
          "/usr/bin/expect",
          SSH_FILE_COPY_DEFAULT_PATH,
          mgrRequest.getInstanceIp(),
          mgrRequest.getInstanceUser(),
          mgrRequest.getInstancePassword(),
          port
        };
  ...
  private static boolean checkLocalHostAndAppend(MgrRequest mgrRequest) {
    ...
    Response<String> returnT =
        ansibleExecutor(
            ansibleAllListHostCommand.getCommand(mgrRequest), ansibleAllListHostCommand);
    if (returnT.isSuccess() && !returnT.getResult().isEmpty()) {
      ...
      Response<String> returns =
          ansibleExecutor(
              ansibleAppendHostCommand.getCommand(mgrRequest), ansibleAppendHostCommand);
  ...
  private static Response<String> ansibleExecutor(
      String ansibleCommand, AnsibleCommand ansibleCommandd) {
    return ansibleExecutor(
        ansibleCommand, ansibleCommandd, DEFAULT_TIME_OUT, TimeUnit.MICROSECONDS);
  }

  private static Response<String> ansibleExecutor(
      String ansibleCommand, AnsibleCommand ansibleCommandd, long timeout, TimeUnit unit) {
    try {
      Process process =
          Runtime.getRuntime().exec(new String[] {"/bin/sh", "-c", ansibleCommand}, null, null);
      process.waitFor(timeout, unit);

      /**
       * [WARNING]: No inventory was parsed, only implicit localhost is available [WARNING]:
       * provided hosts list is empty, only localhost is available. Note that the implicit localhost
       * does not match 'all' [WARNING]: Could not match supplied host pattern, ignoring:
       * 47.99.143.216
       */
      if (process.getErrorStream().available() > 0) {
        log.warn(
            "[ansible executor] failed, command: {}, err: {}",
            ansibleCommand,
            IoUtil.read(
                process.getErrorStream(), SystemPropertiesUtils.getPropertiesFileEncoding()));
        Response<String> re =
            Response.ofFailure(
                Response.Code.Deploy_Error,
                IoUtil.read(
                    process.getErrorStream(), SystemPropertiesUtils.getPropertiesFileEncoding()));
        return re;
      } else {
