...
public class ChaosSettings {
  ...
  public void registerSettings(Object settingObject) {
    ReflectionUtils.doWithFields(
        settingObject.getClass(),
        field -> {
          ChaosSettingDescriptor chaosSettingDescriptor =
              field.getAnnotation(ChaosSettingDescriptor.class);
          ChaosSettingInfo chaosSettingInfo = null;
          try {
            chaosSettingInfo = initConfig(settingObject, field, chaosSettingDescriptor);
            chaosSettingGroupItemInfoMap.put(chaosSettingInfo.getName(), chaosSettingInfo);
   ...
   */
  private ChaosSettingInfo initConfig(
    ...
    if (chaosSettingGroupItemInfoMap.containsKey(name)) {
      throw new IllegalArgumentException("chaos settings duplicate,key:" + name);
    }
    ...
    ownerInfo.setConverter(
        chaosSettingDescriptor.converters().length == 0
            ? null
    ...
    for (ConfigUpdateListener configUpdateListener : configUpdateListeners) {
      if (configUpdateListener.support(chaosSettingInfo)) {
        try {
          ...
          log.warn(
              "handler config key failed,key:{},newValue:{}",
              chaosSettingInfo.getName(),
              chaosSettingInfo.getValue(),
              exception);
        }
      }
    }
