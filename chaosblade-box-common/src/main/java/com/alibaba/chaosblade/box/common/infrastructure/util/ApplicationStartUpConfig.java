...
public class ApplicationStartUpConfig {
  ...
  public boolean isSync(SyncFunctionType syncFunctionType) {
    if (functionSyncTypes.contains(SyncFunctionType.ALL.name())) {
      return true;
    }
    boolean isSync = functionSyncTypes.contains(syncFunctionType.name());
    if (isSync) {
      log.info(
          "config[chaos.function.sync.type:{}],sync function for type:{}",
          functionSyncTypes,
          syncFunctionType.name());
    } else {
      log.info(
          "config[chaos.function.sync.type:{}],skip sync function for type:{}",
          functionSyncTypes,
          syncFunctionType.name());
    }
    return isSync;
