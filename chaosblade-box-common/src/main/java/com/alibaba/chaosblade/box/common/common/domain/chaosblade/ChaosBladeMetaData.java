...
public class ChaosBladeMetaData {
  ...
  private ChaosBladeMetaData() {
    InputStream inputStream =
        Thread.currentThread()
            ...
            .getResourceAsStream("chaos-blade/function_word_segmentation_dict_up.json");
    try {
      functionWordDict =
          JSON.parseObject(
              inputStream, new TypeReference<Map<String, Map<String, String>>>() {}.getType());
    } catch (IOException e) {
  ...
  public String getFunctionNameByChaosBladeAction(
      String originName, ChaosBladeAction chaosBladeAction) {
    if (ChaosBladeActionType.ATTACK.equals(chaosBladeAction.getActionType())) {

      return chaosBladeAction.getScope().trim().toUpperCase()
          + " "
          + chaosBladeAction.getSubTarget().trim().toUpperCase()
          + " "
  ...
  public String getFunctionNameParser(String originName) {
    ...
    String parserdFunctionName =
        scopeDict.get(originNames[0])
