...
public final class ResourceBundleHolder implements Serializable {
  ...
  private final transient Map<Locale, ResourceBundle> bundles =
  ...
  public ResourceBundle get(Locale locale) {
    ...
    synchronized (this) {
      ...
      // bundles.put(locale,
      // rb=ResourceBundle.getBundle(owner.getName(),locale,owner.getClassLoader()));

      Locale next = getBaseLocale(locale);
      String s = locale.toString();
      URL res =
          ResourceProvider.findResource(
      ...
      if (res != null) {
        ...
        try {
          ...
          MissingResourceException x =
              new MissingResourceException("Unable to load resource " + res, owner.getName(), null);
          x.initCause(e);
          throw x;
        }
      } else {
  ...
  /**
   * Clear the cache used by {@link #get(Class)}. This is useful in case of changes to {@link
   * ResourceProvider}.
   */
