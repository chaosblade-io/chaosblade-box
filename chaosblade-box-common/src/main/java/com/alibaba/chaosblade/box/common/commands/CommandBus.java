...
public class CommandBus implements CommandExecutable, SpringCommandExecutable {
  ...
  private Map<String, CommandExecutor> commandTypeCommandExecutorPoolMap =
  ...
  public CommandBus(List<CommandExecutor> commandExecutorMap) {
    ...
    for (CommandExecutor commandExecutor : commandExecutorMap) {
      CommandExecutor oldCommandExecutor =
          commandTypeCommandExecutorPoolMap.get(commandExecutor.getCommandExecutorName());
      if (oldCommandExecutor != null) {
        throw new IllegalStateException(
            String.format(
                "command executor for name:%s already exist,the exist one is:%s,the new one is:%s",
      ...
      commandTypeCommandExecutorPoolMap.put(
          commandExecutor.getCommandExecutorName(), commandExecutor);
    }
  }
  ...
  public CommandExecutor select(String commandExecutorName) {
    ...
    if (commandExecutor == null) {
      log.info(
          "not found command executor for command by name:"
              + commandExecutorName
              + ",use default instead");
  ...
  public List<CommandExecutorInfo> getCommandExecutorInfo() {
    return commandTypeCommandExecutorPoolMap.values().stream()
        .map(
