...
/** @author haibin */
public class CommandExecutorPoolSpringFactory
    implements FactoryBean<CommandBus>, ApplicationContextAware {

  private CommandBus commandBus;

  private ApplicationContext applicationContext;

  @Autowired private List<CommandExecutorPreProcessor> commandExecutorPreProcessors;
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    List<CommandExecutor> commandExecutors =
        initCommandExecutors(commandExecutorPoolConfig.getExecutor());
    ...
    return JSON.parseObject(
        Objects.requireNonNull(
            Thread.currentThread()
                ...
                .getResourceAsStream("command_executor_config.json")),
        CommandExecutorPoolConfig.class);
  }

  private List<CommandExecutor> initCommandExecutors(Map<String, CommandExecutorConfig> executor) {
    List<CommandExecutor> commandExecutors = new ArrayList<>();
    executor
        .entrySet()
        .forEach(
            new Consumer<Entry<String, CommandExecutorConfig>>() {
              @Override
              public void accept(
                  Entry<String, CommandExecutorConfig> stringCommandExecutorConfigEntry) {
                ...
                if (commandExecutorConfig.getExecutorClass() != null) {
                  commandExecutor =
                      initCommandExecutorByClass(
                          commandExecutorConfig.getExecutorClass(),
                          stringCommandExecutorConfigEntry.getKey(),
                          stringCommandExecutorConfigEntry.getValue());
                } else {
                  commandExecutor =
    ...
    SpringUtils.getBeans(applicationContext, CommandExecutor.class)
        .forEach(
            new Consumer<CommandExecutor>() {
              @Override
  ...
  private void initCommandDecorators(CommandBus commandBus) {
    ...
    SpringUtils.getBeans(applicationContext, CommandDecorator.class)
        .forEach(
            new Consumer<CommandDecorator>() {
              @Override
  ...
  private CommandExecutor initCommandExecutorByClass(
      String className, String name, CommandExecutorConfig commandExecutorConfig) {
    try {
      Class<?> claszz = CommandExecutor.class.getClassLoader().loadClass(className);
      try {
        Constructor<?> constructor =
            claszz.getConstructor(
                String.class, ApplicationContext.class, CommandExecutorConfig.class);
        return (CommandExecutor)
            constructor.newInstance(name, applicationContext, commandExecutorConfig);
