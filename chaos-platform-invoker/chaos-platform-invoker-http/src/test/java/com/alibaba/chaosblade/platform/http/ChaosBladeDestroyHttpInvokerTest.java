...
public class ChaosBladeDestroyHttpInvokerTest {

    @Autowired
    private ChaosBladeDestroyHttpInvoker chaosBladeDestroyHttpInvoker;
    ...
    public void pingTest() throws Throwable {
        ...
        request.setName("e867954fb2b24729");

        CompletableFuture<ResponseCommand> future = chaosBladeDestroyHttpInvoker.invoke(request);
        ResponseCommand responseCommand = future.get();
        Assert.assertTrue(responseCommand.isSuccess());
    }

    @Test
    public void pingTestWithoutUid() throws Throwable {
        HttpChannelRequest request = new HttpChannelRequest();
        request.setHost("192.168.0.1");
        request.setPort(chaosAgentPort);
        request.setSceneCode("chaosblad.cpu.fullload");
