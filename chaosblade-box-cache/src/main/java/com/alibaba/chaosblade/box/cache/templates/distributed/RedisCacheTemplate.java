  ...
  public void prefixPut(String prefixKey, String key, Serializable value) {
    try {
      if (expireAfterWrite > 0) {
        redisTemplate
            .opsForValue()
            .set(wrap(prefixKey, key), value, expireAfterWrite, TimeUnit.SECONDS);
      } else {
        redisTemplate.opsForValue().set(wrap(prefixKey, key), value);
      }
    } catch (Exception e) {
      log.error(
          "[RedisCacheTemplate] put error, key: {}, value: {}", wrap(prefixKey, key), value, e);
    }
  }
          ...
          public <K, V> List<Object> execute(RedisOperations<K, V> operations)
            ...
            for (Map.Entry<String, Serializable> entry : values.entrySet()) {
              if (expireAfterWrite > 0) {
                ...
                operations
                    .opsForValue()
                    .set((K) wrap(prefixKey, entry.getKey()), (V) entry.getValue());
              }
            }
